---
title: 云风的 BLOG
url: https://blog.codingnow.com/2012/08/
...
自从动了[重新实现 skynet](http://blog.codingnow.com/2012/08/skynet.html) 的念头，最近忙的跟狗一样。每天 10 点醒来就忙着写代码，一句废话都不想说，一直到晚上 11 点回家睡觉。连续干了快一个月了。
...
另一方面，一旦固定采用单进程多线程方案，之前的多进程共享数据的模块就显得过于厚重了。新的方案更为轻量，也更适合 lua 使用。这项工作在[上一篇 blog](http://blog.codingnow.com/2012/07/dev_note_24.html) 中提到过。这和 skynet 的重写原本是两件事情，但我强行放在一起做迁移，增加了许多难度。但考虑到，原本我就需要梳理一次我们的全部服务器端代码（包括大量我没有 review 过的），就把这两件事情同时做了。
...
目前能找到[年初的一篇记录](http://blog.codingnow.com/2012/01/dev_note_7.html) ，经过大半年的演化，已经不完全是记录的那个样子了。但大体上的思路一直沿用着。
...
首先我引入了之前实现好的 [Lua 数据序列化模块](https://github.com/cloudwu/lua-serialize) 。并[对它做了一些改动](https://github.com/cloudwu/skynet/blob/master/lualib-src/lua-seri.c) ，合并到 skynet 项目中。
...
这个模块并不复杂，我实现在了 [skynet.lua](https://github.com/cloudwu/skynet/blob/master/lualib/skynet.lua) 中，不到 100 行代码。

作为一个范例，我实现了一个叫 root 的远程对象，让它在一个[独立服务](https://github.com/cloudwu/skynet/blob/master/service/remote_root.lua) 中启动。它可以提供一个基本的名字服务。在一个[简单的 test 程序](https://github.com/cloudwu/skynet/blob/master/service/testrobj.lua) 中，我们可以看到一个远程对象把自己注册到 root 里，别的服务从 root 拿到这个对象，就可以向本地对象一样调用上面的方法了。