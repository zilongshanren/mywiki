...
url: http://www.learn-cocos2d.com/knowledge-base/cocos2d-iphone-faq/learn-cocos2d-public-content/manual/cocos2d-general/19514-how-to-fix-base-sdk-missing-error/
author: Max says
...
[FAQ: cocos2d for iPhone](http://www.learn-cocos2d.com/knowledge-base/cocos2d-iphone-faq/learn-cocos2d-public-content/manual/cocos2d-general/): How to fix "Base SDK Missing" Error
...
This is an error that happens every time Apple releases a new iOS SDK version. Cocos2d for iPhone isn't usually updated to fix this error. Luckily, the fix is straightforward.

### There is no SDK with the name or path ...

When you try to compile you may get an error like this one.

### Base SDK Missing

The Target dropdown shows you why: the Base SDK is missing. Whatever that means. Well, simply put, if you have iOS SDK 4.1 installed but cocos2d was last built with iOS SDK 4.0 then it's still set to use 4.0 but the 4.1 release doesn't include the 4.0 version. You'll need to update the project to compile with iOS SDK 4.1 instead, in this case. Version numbers may be different and depend on the iOS SDK version you're using.

### Get Info on Project

Right click the project and select Get Info.

### Locate: Base SDK for All Configurations

There it says, the SDK "iOS Device 4.0" is missing.

### Change Base SDK to your most current iOS Device version

In this case, select iOS Device 4.1 and you're good to go.

Note: don't pick any of the iOS Simulator SDKs or you will have issues deploying on the device or submitting the App to Apple or using features that are only available on the device. Xcode automatically selects the Simulator SDK if you do Simulator builds.
