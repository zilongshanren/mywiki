...
url: http://www.learn-cocos2d.com/knowledge-base/cocos2d-iphone-faq/learn-cocos2d-public-content/manual/cocos2d-general/14120-what-makes-spritesheets-faster-than-regular-sprites/
author: Max says
...
[FAQ: cocos2d for iPhone](http://www.learn-cocos2d.com/knowledge-base/cocos2d-iphone-faq/learn-cocos2d-public-content/manual/cocos2d-general/): What makes SpriteSheets faster than regular Sprites?
...
In graphics programming the rendering of textures happens so called draw calls. Each draw call sets new states in the rendering engine, which causes some overhead as parameters are changed and textures swapped in or out of video memory. In general you want as few draw calls as possible.

Now if you want to render the same texture or parts of it several times you can do this in one draw call. On the iPhone reducing draw calls is one of the important factors to getting a high framerate. While each Sprite is rendered seperately, each in its own draw call, with SpriteSheets you can render everything of the SpriteSheet's texture in just one draw call. Usually the SpriteSheet will have a Texture Atlas as texture, so you can render player animations, monster animations and level graphics from the same SpriteSheet with just one draw call.

If you get 60 fps in your game without SpriteSheets, don't worry about them. But if you want to make a game with lots of Sprites, let's say more than 50 sprites with 64x64 Pixels then you should use a Texture Atlas and SpriteSheet from the start.
