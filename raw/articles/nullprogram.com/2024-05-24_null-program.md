...
exist, or a subscript [that may fall out of bounds](https://research.google/blog/extra-extra-read-all-about-it-nearly-all-binary-searches-and-mergesorts-are-broken/). It’s easy to miss
the edge cases where results overflow, creating a nasty, subtle bug, [even
in the presence of type safety](https://blog.carlana.net/post/2024/golang-slices-concat/). Ideally such computations happen in
...
- Range check
*before* computing a result. No exceptions.
- Do not cast unless you know
*a priori* the operand is in range.
- Never mix unsigned and signed operands.
[Prefer signed.](https://www.youtube.com/watch?v=wvtFGa6XJDU) If you
need to convert an operand, see (2).
- Do not add unless you know
*a priori* the result is in range.
- Do not multiply unless you know
*a priori* the result is in range.
- Do not subtract unless you know
*a priori* both signed operands
are non-negative. For unsigned, that the second operand is not larger
than the first (treat it like (4)).
- Do not divide unless you know
*a prior* the denominator is positive.
...
software, [like glibc](https://sourcegraph.com/search?q=context:global+%22%3E+outend%22+repo:%5Egithub%5C.com/bminor/glibc%24+&patternType=keyword&sm=0), are riddled with such pointer overflows.
...
With [arena allocation](/blog/2023/09/27/) there are usually two concerns. First, will
