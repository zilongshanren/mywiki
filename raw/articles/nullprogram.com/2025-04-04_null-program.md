...
It began as a [water sort puzzle](https://www.coolmathgames.com/blog/how-to-play-lipuzz-water-sort) solver, constructed similarly to
[my British Square solver](/blog/2020/10/19/). It was nearly playable, so I added a user
interface [with SDL2](/blog/2023/01/08/). My wife enjoyed it on her desktop, but wished
...
WebAssembly (Wasm). I succeeded, and now [my game runs in browsers](/water-sort/)
([source](https://github.com/skeeto/scratch/tree/master/water-sort)). Like [before](/blog/2025/03/06/), next I ported [my pkg-config clone](/blog/2023/01/18/)
to the Wasm System Interface ([WASI](https://wasi.dev/)), whipped up a proof-of-concept UI,
...
functions emitted by the compiler. LLVM comes with `wasm-ld`
...
Wasm modules may import. Because I [think we could do better](/blog/2023/02/11/),
[as usual](/blog/2023/02/15/) [around here](/blog/2023/03/23/), in this article we’re going to
eschew the language runtime and code directly against raw WASI. You
still have [easy access hash tables and dynamic arrays](/blog/2025/01/19/).
...
Fortunately Wasm is old enough that [LLMs](/blog/2024/11/10/) are well-versed in it, and
...
We can see [the ABI](https://github.com/WebAssembly/tool-conventions/blob/main/BasicCABI.md) taking shape: Clang has predictably mapped
`float`
...
-
Pointers were mapped onto `i32`

. Pointers are a high-level concept, and
...
, which is part of the Clang ABI, not
...
purposes, and the `__stack_pointer`

global is the stack register of its
...
([Below LLVM 20](https://releases.llvm.org/20.1.0/docs/ReleaseNotes.html#changes-to-the-webassembly-backend) you will need the undocumented `-mbulk-memory`
...
[w64devkit](https://github.com/skeeto/w64devkit) has `-lmemory`
...
address space for the result. The [multi-value extension](https://github.com/WebAssembly/multi-value/blob/master/proposals/multi-value/Overview.md)
...
in MS-DOS](/blog/2018/04/13/). Instead, develop on a platform much better suited for
it, then *port* your application to Wasm after you’ve [got the issues
worked out](/blog/2025/02/05/). The less Wasm-specific code you write, the better, even
...
so [I can do as I please](/blog/2023/05/31/). I absolutely love that WASI barely uses
...
This function is shaped like [POSIX ](https://pubs.opengroup.org/onlinepubs/9799919799/functions/writev.html)`writev()`
...
value, with zero as
success rather than some kind of [in-band signaling](/blog/2016/09/23/). Hence the
...
Keep going and you’ll have [something like ](/blog/2023/02/13/)`printf`
...
That’s 9 parameters — and I had thought [Win32 ](https://learn.microsoft.com/en-us/windows/win32/api/fileapi/nf-fileapi-createfilew)`CreateFileW`

was
over the top. It’s even more complex than it looks. It works more like
[POSIX ](https://pubs.opengroup.org/onlinepubs/9799919799/functions/openat.html)`openat()`
