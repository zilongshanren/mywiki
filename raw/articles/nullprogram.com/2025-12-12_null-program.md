...
.
The procedure attaches the pointer to its window as `GWLP_USERDATA`

. This
pointer is passed indirectly, through a `CREATESTRUCT`
...
In future messages we can retrieve it with `GetWindowLongPtr`
...
like this, I’ve allocated executable memory with `VirtualAlloc`
...
This assembly program defines a new section named `.exebuf`
...
loved to define the array in C with the GCC `section`
...
From here the plan is to create a function that accepts a `Wndproc5`
...
through their fifth parameter. It turns out setting up `exebuf`

was the
more complicated part, and `make_wndproc`
...
It stores the new argument for the `Wndproc5`

just above the shadow space.
Then calls into the `Wndproc5`
