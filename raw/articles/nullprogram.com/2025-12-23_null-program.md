...
](https://pubs.opengroup.org/onlinepubs/9799919799/utilities/find.html)`find`

utility. It operates a file system hierarchies, with basic
...
parentheses for precedence. `find`

may apply the expression to a great
...
was later surprised all the real world `find`

implementations I examined
use [tree-walk interpreters](https://craftinginterpreters.com/a-tree-walk-interpreter.html) instead. This article describes how my
...
, e.g. print everything under each listed path. This prints the
...
is the logical AND binary operator. `-print`

always evaluates
to true. It’s never necessary to write `-a`

, and adjacent operations are
implicitly joined with `-a`

. We can keep chaining them, such as finding
...
(or similar side-effect extensions like
`-print0`

or `-delete`

) are present, the whole expression is wrapped in an
implicit `( expr ) -print`
...
for logical OR. To print all files with the executable bit *or*
with a `.exe`
...
I needed parentheses because `-o`

has lower precedence than `-a`

, and
because parentheses are shell metacharacters I also needed to escape them
for the shell. It’s a shame `find`
...
instead! There’s
also a unary logical NOT operator, `!`

. To print all non-executable files:
...
Only lists the sizes of directories, as the `-type d`

fails causing the
whole expression to evaluate to false without evaluating `-exec`
...
(GNU, BSD, BusyBox) have a `-regex`

extension, and eagerly compile the
...
stops the program. While I could just let it “run off the
end” it’s useful to have an actual instruction so that I can attach a
label and jump to it. The `not`

opcode negates the register. `braf`

is
“branch if false”, jumping (via relative immediate) to the labeled (in
printed form) instruction if the register is false. `brat`

is “branch if
true”. Together they implement the `-a`

and `-o`

operators. In practice
there are no loops and jumps are always forward: `find`

is [not Turing
complete](/blog/2016/04/30/).

In a real implementation each possible action (`-name`
...
, etc.) would get a dedicated opcode. This requires implementing
each operator, at least in part, in order to correctly parse the whole
`find`

expression. For now I’m just focused on the bytecode compiler, so
this opcode is a stand-in, and it kind of pretends based on looks. Each
action sets the register, and actions like `-print`

always set it to true.
My compiler is [called ](https://github.com/skeeto/scratch/blob/c142e729/parsers/findc.c)`findc`

(“find compiler”).

**Update**: Or try [the ](https://nullprogram.com/scratch/findc/)**online demo** via Wasm! This version
includes a [peephole optimizer](https://github.com/skeeto/scratch/commit/2c0a4b8f) I wrote after publishing this
article.

I assume readers of this program are familiar with `push`

macro
and `Slice`

macro. Because of the latter it requires a very
recent C compiler, like GCC 15 (e.g. via [w64devkit](https://github.com/skeeto/w64devkit)) or Clang 22. Try
out some `find`

commands and see how they appear as bytecode. The simplest
...
Inside the parentheses, if `-executable`

succeeds, the right-hand side is
skipped. Though the `brat`

jumps straight to a `braf`
...
in a row cancel out, and so these instructions could be
eliminated. Overall this compiler could benefit from a [peephole
optimizer](https://en.wikipedia.org/wiki/Peephole_optimization), scanning over the program repeatedly, making small
...
re-targets ahead one instruction, and vice versa.
...
might convert to a `brat`

, and vice versa.
- Delete side-effect-free instructions before
`halt`
...
- Exploit always-true actions, e.g.
`-print`
...
infix into a compiler-friendly postfix. That is, this:
...
array, so it’s already tokenized for us by the shell [or runtime](/blog/2022/02/18/). The
classic [shunting-yard algorithm](https://en.wikipedia.org/wiki/Shunting_yard_algorithm) solves this problem easily enough.
...
we pop the stack into the output queue until we see `(`

.


When we’re out of tokens, pop the remaining stack into the queue. My
parser synthesizes `-a`

where it’s implied, so the compiler always sees
logical AND. If the expression contains no `-exec`
...
,
after processing is complete the parser puts `-print`
...
-
For an action token, create an `action`

instruction, and push it onto
the fragment stack as a new fragment.

-
For a `!`

token, pop the top fragment, append a `not`
...
token, pop the top two fragments, join then with a `braf`
...
exactly one fragment. Append a `halt`

instruction to this fragment, and
that’s our program! If the final fragment contained a branch just beyond
its end, this `halt`

is that branch target. A few peephole optimizations
