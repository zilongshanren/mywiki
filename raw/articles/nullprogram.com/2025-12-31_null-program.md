...
concept as [last time I talked about data structures](/blog/2025/01/19/): a collection
...
It will be sourced from some string, formatted like the `env`
...
And all the parser heavy lifting will be done by [our ever-handy ](/blog/2025/03/02/)`cut`
...
objects into the front. Zero-initialized `head`

pointer, point the new
node at it, then make that node the new `head`
...
the list works even for empty lists. The `tail`

pointer is unneeded once
...
hash map, in the form of [a hash trie](/blog/2023/09/30/), by adding two more pointers
to the linked list:
...
Then we swap that into the `head`
...
constructing an index table over the list: [an MSI hash table](/blog/2022/08/08/). This
...
nodes, and so linked list nodes are untouched.
...
property is the input to `hash64`

and `equals`

. By using a different input
