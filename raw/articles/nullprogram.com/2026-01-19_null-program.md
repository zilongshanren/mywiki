---
title: null program
url: https://nullprogram.com/blog/2026/01/19/
...
nullprogram.com/blog/2026/01/19/

I came across a recent article on [making Linux system calls from a Wine
process](https://gpfault.net/posts/drunk-exe.html). Windows programs running under Wine are still normal Linux
...
application: build [my pkg-config implementation](/blog/2023/01/18/) such that on Windows
`pkg-config.exe`

behaves as a native pkg-config, but when run under Wine
...
kernel. [Cosmopolitcan Libc](https://justine.lol/cosmopolitan/) cleverly does this out-of-the-box, but
in this article we’ll mash together a couple existing sources with a bit
of glue.

The results are in [the merge-demo branch](https://github.com/skeeto/u-config/commit/e0008d7e) of u-config, and took
hardly any work:
...
A platform layer, `main_wine.c`

, is a merge of two existing platform
...
Wine, and [the best solution I found](https://web.archive.org/web/20250923061634/https://stackoverflow.com/questions/7372388/determine-whether-a-program-is-running-under-wine-at-runtime/42333249#42333249) was to locate
`ntdll!wine_get_version`

. If this function exists, we’re in Wine. That
works out to a pretty one-liner because `ntdll.dll`
...
An x86-64 Linux syscall wrapper with [thorough inline assembly](/blog/2024/12/20/):
...
I’d normally use `long`

for all these integers because Linux is [LP64](https://en.wikipedia.org/wiki/64-bit_computing#64-bit_data_models)
(`long`

is pointer-sized), but Windows is LLP64 (only `long long`
...
cross toolchain support](/blog/2021/08/21/), including Mingw-w64. There are even a few
Windows libraries in the Debian package repository, [such as zlib](https://packages.debian.org/trixie/x32/libz-mingw-w64), and
...
Note the architecture in the `-I`

and `-L`

options. It really is querying
the [cross sysroot](https://peter0x44.github.io/posts/cross-compilers/). Though these paths are in the cross sysroot,
...
A tool prefixed `x86_64-w64-mingw32-`

should not produce paths containing
`x86_64-linux-gnu`

(the host architecture in this case). Our version won’t
have these issues.
...
Platforms implement the first four functions, and call `uconfig()`

with
the platform’s configuration, context pointer (`os *`

), command line
arguments, environment, and some memory (all in the `config`

object). My
strategy is to link two platforms into the binary, and the first challenge
is they both define `os_write`

, etc. I did not plan nor intend for one
...
This dirty, but effective trick [may look familiar](/blog/2025/02/05/). It also doesn’t
...
If I were serious about keeping this experiment, I’d lift `os`

as I did
the functions (as `win32_os`

, `linux_os`

) and include `wine_detected`
...
instead of an equivalent platform-agnostic type like
`ptrdiff_t`

. I never thought this would be an issue because this source
literally contains `asm`

blocks and no conditional compilation, yet here
we are. Lesson learned. I wanted to try an extremely janky `#define`

on
`long`

to fix it, but this source file has a couple `long long`
...
is [the initial value of the stack pointer](/blog/2025/03/06/), which
[points to ](https://articles.manugarg.com/aboutelfauxiliaryvectors)`argc`
...
. We’ll need construct
an artificial “stack” for the Linux platform layer to harvest. On Windows
this is [the process entry point](/blog/2023/02/15/), and it will find the rest on its
...
Where `cmdline_to_argv8`

is my Windows argument parser, already
...
onto the fake stack. The Linux entry point doesn’t need
`auxv`

and can be omitted. Once in the Linux entry point it’s essentially
...
And that’s it! We have our platform merge. Build ([w64devkit](https://github.com/skeeto/w64devkit)):
...
On Debian use `x86_64-w64-mingw32-gcc`
...
binfmt](https://packages.debian.org/trixie/wine-binfmt), here’s how it looks on Debian:
...
argument despite it being in the cross sysroot:
...
Looking good! It passes the `pc_path`
