...
the C standard library are `qsort()`
...
result](/blog/2016/09/05/). For example, suppose I have a structure representing a
...
time](/blog/2014/10/12/). Also, on some platforms and configurations, repeatedly
accessing a global variable in a comparator [may have a significant
cost](/blog/2016/12/23/). A common workaround for thread safety is to make the
...
- Inspect some global state: the
**global variable solution**. The
caller will change this state for some other calls.
- Query its unique thread ID: the
**thread-local storage solution**.
Calls on different threads will have different thread IDs.
- Examine a context argument: the
**user pointer solution**.
...
[libjit](https://www.gnu.org/software/libjit/) documentation.
...
,
just like in [my JIT compiler](/blog/2015/03/19/). It’s initially mapped writable in
