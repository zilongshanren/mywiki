...
This article durably captures [my reddit comment](https://old.reddit.com/r/C_Programming/comments/1hrvhfl/_/m51saq2/) showing techniques
for `std::unordered_map`

and `std::vector`

equivalents in C programs. The
...
The foundation is our allocator, a simple [bump allocator](/blog/2023/09/27/), so
we’ll start there:
...
Allocating through the `new`

macro eliminates several classes of common
defects in C programs. If we get our types mixed up we get errors, or at
least warnings. Our [size calculations cannot overflow](/blog/2024/05/24/). We cannot
...
This is equivalent to a `std::string_view`

in C++. The macro allows us to
efficiently convert string literals into `Str`

objects. Because our data
...
appears superfluous, but it’s necessary: `memcmp`

[arbitrarily
forbids null pointers](/blog/2023/02/11/#strings), and we may be passed a zero-initialized
`Str`

. Though [this is scheduled to be corrected](https://developers.redhat.com/articles/2024/12/11/making-memcpynull-null-0-well-defined).
...
data is fixed to 0–255 rather than allowing the signedness of `char`
...
(MSI) hash table](/blog/2022/08/08/). I don’t think it fits our problem as well as the
...
is an empty table. They come out of an arena ready-to-use:
...
Now we leverage `equals`

and `hash64`

for a double-hashed, open address
...
entries. Per the `ENVEXP`

comment, it’s hard-coded to support up to 1,000
...
If the number of keys is unbounded, [hash tries](/blog/2023/09/30/) work better. Trees
...
Next I’d like a function that takes an `Env`

and produces an `envp`

data
structure as expected by `execve(2)`

. Then we can use this map as
the environment in a child process. We’ll need some string manipulation,
particularly [string concatenation](/blog/2024/05/25/). The core is a copy function:
...
we need to handle the arbitrary
special case around null pointers should the input be a zero `Str`
...
is special check because pointer arithmetic on
...
By virtue of the scratch arena, the `envp`

object is automatically freed
should `execve`

fail. (If that should even matter.) Considering this, if
...
To implement the `envp`

conversion for the hash trie `Env`

, let’s add one
more tool to our toolbox: dynamic arrays. Our `std::vector`

equivalent.
We’ll start with [a familiar slice header](/blog/2023/10/05/):
...
one is set up to create an `envp`

array. The good news is that manipulation
occurs through generic code, so everything else is reusable.

I want a `push`

macro that creates an empty slot in which to insert a new
value, evaluating to a pointer to this slot. Usually that means
incrementing `len`

, but when out of room it will need to expand the
...
is just like those calls to `lookup`

. If expansion is needed, the `push`
...
The internals of that helper look an awful lot like `concat`
...
[applying alignment in the second ](https://lists.sr.ht/~skeeto/public-inbox/%3CCAB2_dQWNOKCSCa8L8khH2W0eunsKK-_CkJZaDUpRAA4AFMG8Jg@mail.gmail.com%3E)`alloc`

may introduce unnecessary
fragmentation. This has been corrected above.)

For unfathomable reasons, standard C does not permit `_Alignof`

on
expressions, so slice data is simply pointer-aligned. (The more shrewd
might consider `max_align_t`

.) Like concatenation, we copy the object to
...
**Update**: [NRK points out](https://old.reddit.com/r/C_Programming/comments/1i74hii/_/m8l40fo/) we can use `__typeof__`

(extension) or
`typeof`

(C23), to work around this syntactical limitation of `_Alignof`

.
Convert the `align`

local variable into a parameter:
...
array is a form of [small-size optimization](/blog/2016/10/07/). It’s used at
first, and sufficient for nearly all inputs. So no stack litter in the
arena. If it’s not enough, then `push`

will *automatically move the stack
...
we could instead add a `next`

field to `Env`

as an intrusive linked list
that chains the nodes together in insertion order. Or another way to look
at it, `Env`

is a linked list with an *intrusive hash trie* for O(log n)
...
function blows the stack, though we already solved that
issue.
...
we can use the address of the `FlatEnv`
...
Recall it’s allocated out of our arena (via `new`

), and ASLR gives our
arena a random offset. On top of that, a `FlatEnv`

seed depends precisely
...
— each easily replaceable — we did all this without runtime
