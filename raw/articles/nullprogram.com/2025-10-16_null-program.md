...
As I [continue to reflect](/blog/2025/09/30/) on arenas and lifetimes in C++, I realized
that dealing with destructors is not so onerous. In fact, it does not even
impact [my established arena usage](/blog/2025/01/19/)! That is, implicit RAII-style
...
remains [resource management avoidance](/blog/2024/10/03/) when possible.) We can also
...
interfaces](/blog/2024/09/04/) to do so, or they’re simply ineffective at it ([pmr](https://en.cppreference.com/w/cpp/memory/polymorphic.html)) —
...
when out of memory mainly to emphasize that this works, but
...
designed for it. Also note that it’s now `raw_alloc`

, as we’ll be writing
a new, enhanced `alloc`
...
like a kind of type tag. The `dtor`

functions will be generated using a
template function:
...
would be instantiated by placement `new[]`

. It’s essentially a placement
`delete[]`

. An arena initializes with an empty list of Dtors as a new
member:
...
member directly. Updates to `dtors`

are potentially visible to
destructors, inhibiting optimization.) The new, enhanced `alloc`
...
I’m using the non-standard `__has_trivial_destructor`

built-in supported
by all major C++ implementations, meaning we still don’t need the C++
standard library, but `std::is_trivially_destructible`

is the usual
tool here. [LLVM is pushing ](https://clang.llvm.org/docs/LanguageExtensions.html#:~:text=__has_trivial_destructor)`__is_trivially_destructible`

instead,
but it’s not supported by GCC [until GCC 16](https://gcc.gnu.org/bugzilla/show_bug.cgi?id=107600).
...
If a constructor throws, placement `new[]`
...
With a little more cleverness we could omit the `objects`
...
we could probably omit `count`

, then write a single-object allocator that
...
These sockets will all be closed when `example`

exits via their singular
Dtor entry on `scratch`

. When calling this `example`
...
list to be passed into `example`

. Objects existing in `*perm`

will
not be destroyed by `example`

because `dtors`

isn’t passed in. If we had
