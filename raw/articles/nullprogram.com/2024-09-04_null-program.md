...
about wrapping `std::regex`

in a tidy C interface which not only
...
[w64devkit](https://github.com/skeeto/w64devkit), MSVC `cl`

, and `clang-cl`

: [scratch/regex-wrap](https://github.com/skeeto/scratch/tree/master/regex-wrap)
...
Longtime readers will find it familiar: [my favorite](/blog/2023/10/08/) non-owning,
counted strings form in place of null-terminated strings — similar to C++
`std::string_view`

— and [arena allocation](/blog/2023/09/27/). Yes, such fundamental
...
called [a ](/blog/2018/06/10/)*non-allocating library*. This requires a bit of C++
...
On the C++ side the first thing I do is replace `new`

and `delete`

, which
is how I force it to allocate from the arena. This replaces `new`
...
I can’t tell `std::regex`

about the arena — it calls `operator new`
...
, but it’s useful here to demonstrate and measure. Using it
...
. The [many other
variants](https://en.cppreference.com/w/cpp/memory/new/operator_new), including arrays, call these four and so may be skipped.
Allocating over-aligned objects isn’t a special case for arenas, so I
implemented plain `new`

by calling aligned `new`

. I’d prefer to [allocate
through a template](/blog/2024/04/14/) so that I can “see” the type, but that’s not an
option in this case.

After converting to signed sizes [because they’re simpler](/blog/2024/05/24/), it’s the
usual from-the-end allocation. I prefer `-fno-exceptions`
...
optional*, but I also replaced sized `delete`

for little other reason than
[sized deallocation is cool](/blog/2023/12/17/). C++ destructs in reverse order, so
...
try to free anything at all, but since it’s going to call `delete`
...
matching four times faster! I expected a *small* speedup, but
not that. In the typical case it appears to be wasting most of its time on
allocation. On the other hand, libstdc++ `std::regex`
...
“cregex” iterator, again giving it each end of the
input. I hope it’s not just making a copy (MSVC `std::regex`
...
destroyed before returning, uselessly calling `delete`
...
You might have noticed the lack of `declspec(dllexport)`

. [DEF files are
great](/blog/2023/08/27/), and I’ve come to appreciate and prefer them. GCC and MSVC
...
hides some `std::regex`

problems, trade-offs must be considered:

- No Unicode support, particularly UTF-8
`std::regex`

implementations are universally poor and slow
- libstdc++
`std::regex`

is especially slow to compile
