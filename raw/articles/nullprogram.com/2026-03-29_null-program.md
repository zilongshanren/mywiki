---
title: null program
url: https://nullprogram.com/blog/2026/03/29/
...
nullprogram.com/blog/2026/03/29/
...
these early months of [the automation of software engineering](https://shumer.dev/something-big-is-happening). I
...
is thinking](https://paulgraham.com/writes.html), and it would defeat the purpose for an AI to write
...
AI in late 2024](/blog/2024/11/10/) is, as I predicted, utterly obsolete. There’s a
...
I’ll still write code myself from time to time for fun — [minimalist](/blog/2018/06/10/),
with my [style](/blog/2023/10/08/) and [techniques](/blog/2025/01/19/) — the same way I play [shogi](https://en.wikipedia.org/wiki/Shogi) on
...
For instance, a couple weeks ago I [put AI to work on a problem](https://github.com/skeeto/w64devkit/pull/357),
and it produced a working solution for me after ~12 hours of continuous,
autonomous work, literally while I slept. The past month [w64devkit](https://github.com/skeeto/w64devkit) has
...
So far the my biggest, successful undertaking is [Quilt.cpp](https://github.com/skeeto/quilt.cpp), a C++
clone of [Quilt](https://savannah.nongnu.org/projects/quilt), an early, actively-used source control system for
patch management. Git is a glaring omission from the [almost](/blog/2020/09/25/) complete
...
Quilt is conceptually simple, and I could lean on [busybox-w32](https://frippery.org/busybox/) `diff`

and `patch`

, I’ve considered writing my own implementation, just [as I did
pkg-config](/blog/2023/01/18/), but I never found the energy to do it.

Then I got good enough with AI to knock out a near feature-complete clone
in about four days, including a built-in `diff`

and `patch`

so it doesn’t
actually depend on external tools (except invoking `$EDITOR`
...
I changed the output of `quilt mail`

to be more Git-compatible. The mbox
...
though it were in Git the whole time. `git format-patch`
...
suite](https://eli.thegreenplace.net/2026/rewriting-pycparser-with-the-help-of-an-llm/) based on the original, its documentation, and other online
documentation, validating that suite against the original implementation
(see `-DQUILT_TEST_EXECUTABLE`

). Then had another AI code to the tests, on
architectural guidance from me, with `-D_GLIBCXX_DEBUG`
...
highly error-prone](/blog/2023/02/11/). Even AI has trouble with it. In the ~9k lines
of C++ that is Quilt.cpp, I am only aware of three memory safety errors by
the AI. Two were null-terminated string issues with `strtol`

, where the AI
was essentially writing C instead of C++, after which I directed the AI to
use `std::from_chars`

and drop as much direct libc use as possible. (The
other was an unlikely branch with `std::vector::back`
...
employment change, this past December I read *Professional CMake*.
...
Update: Several folks suggested I give [OpenCode](https://opencode.ai/) another shot, and this
