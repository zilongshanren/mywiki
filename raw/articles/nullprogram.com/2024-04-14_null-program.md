...
*Update September 2025: This article *[has a followup](/blog/2025/09/30/) with
corrections.

My techniques with [arena allocation](/blog/2023/09/27/) and [strings](/blog/2023/10/08/) are
...
standard library](/blog/2023/02/11/), the C++ standard library offers me little. Its
...
, etc. It eliminates noisy, repetitive code and
interfaces — getters, setters, separate `const`
...
could be useful, but it keeps falling short when I
...
noisy, and in practice they make debug builds (i.e. `-O0`
...
C style, plus methods just look ugly without references: `obj->func()`
...
Finally, no exceptions of course. I assume `-fno-exceptions`
...
(As needed, replace the second `assert`

with whatever out of memory policy
is appropriate.) Then allocating, say, a [10k-element hash table](/blog/2023/06/26/)
(i.e. to keep it [off the stack](/blog/2024/02/05/)):
...
With C++, I initially tried [placement new](https://en.cppreference.com/w/cpp/language/new#Placement_new) with the arena as the
...
relies on complex, finicky rules governing `new`

overloads, especially for
...
I soon learned it’s better to replace the `new`

macro with a template,
which can actually see what it’s doing. I can’t call it `new`
...
could accept constructor arguments and perfect forward them to a
constructor. However, that’s too far into the dark arts for my comfort,
plus it requires a correct definition of `std::forward`

. In practice that
means `#include`

-ing it, and whatever comes in with it. Or ask an expert
...
say that [a static cast is sufficient](https://www.foonathan.net/2020/09/move-forward/). This is easy to do:
...
expand the parameter pack without casting or `&&`
...
to provide a definition. That means including `<new>`
...
calls a `std::string_view`

. In C++ we won’t need the `countof`
...
of the rare cases. The same concept applies as an `s8`
...
, e.g. an array of strings. `const`

is required because string
literals are `const`

in C++, but it’s immediately stripped off for the
...
— make a string spanning two pointers — so often appears in my
...
mistakes quickly and early, and they complement [fuzz testing](/blog/2019/01/25/).
...
specialized and less useful. (I don’t care about range-based `for`
...
qualifier in two of the three major C++ implementations. A
...
function, so it’s not required to happen at compile time.
...
On the other hand, [hash maps are mostly solved](/blog/2023/09/30/), so I wouldn’t
...
checks required in C, though not all: `memcpy`
