...
of course there’s JavaScript for web technologies. [WebAssembly](/blog/2025/04/04/)
...
and use more polished development tools — debugging, [testing](/blog/2025/02/05/), etc.
...
For preferred non-WASI Wasm runtime is Volodymyr Shymanskyy’s [wasm3](https://github.com/wasm3/wasm3).
...
bindings, [pywasm3](https://github.com/wasm3/pywasm3), but it’s distributed only in source code form. That
...
For the use cases in this article, the best option is [wasmtime-py](https://github.com/bytecodealliance/wasmtime-py). The
...
The [official examples](https://github.com/bytecodealliance/wasmtime-py/tree/main/examples) don’t do anything non-trivial or interesting,
and so to figure things out I had to study [the documentation](https://bytecodealliance.github.io/wasmtime-py/),
...
with different stores. [Use the wrong store and it panics](https://docs.wasmtime.dev/api/wasmtime/struct.Store.html#cross-store-usage-of-items): It’s
already keeping track internally! I do not understand why the interface
works this way. So to make things simpler, I use `functools.partial`

to
bind the `store`

parameter and so get the interface I expect.

The `get_buffer_ptr`

object is a buffer protocol object, and if you’re
...
memory size](/blog/2025/04/19/) you probably want to grab a fresh buffer object. For
bytes (e.g. buffers and strings) I prefer the `read`

and `write`

methods.

Because [multi-value](https://github.com/WebAssembly/multi-value/blob/master/proposals/multi-value/Overview.md) is still in an experimental state in the Wasm
...
To make matters worse, wasmtime-py adds its own footgun: The `read`

and
`write`

methods adopt the questionable Python convention of negative
indices acting from the end. If `malloc`

returns a pointer in the upper
half of memory, the negative pointer will pass the bounds check inside
`write`

because negative is valid, then quietly store to the wrong
address! Doh!
...
buffer overflow into Python’s memory space](https://github.com/paulocoutinhox/pdfium-lib/blob/139d5037/modules/wasm.py#L601-L606):
...
method returns a non-bounds-checked raw `ctypes`
...
copying plain Python values in and out. The `struct`
...
It fills a similar role to [JavaScript ](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/DataView)`DataView`
...
To copy structures back out, use `struct.unpack_from`

. If you’re moving
strings, you’ll need to `.encode()`
...
copy inputs for a function. In my examples I’ve used `malloc`

because it
requires no elaboration, but as usual [a bump allocator](/blog/2023/09/27/) solves
...
My [main experimental benchmark](https://github.com/skeeto/scratch/tree/master/wasm-bench) is a variation on [my solution to
the “Two Sum” problem](/blog/2023/06/26/), which I originally wrote for JavaScript, then
...
in JavaScript’s Wasm API. `wasmtime.Instance`
...
**Update**: Wolfgang Meier points out the `serialize`
...
Loup Vaillant’s [Monocypher](https://monocypher.org/) is a wonderful cryptography library.
...
It’s not “Wasm-aware” so I need `--export-all`

to expose the interface.
...
I’ve [discussed ](/blog/2025/04/19/)`__heap_base`

before, which is part of the ABI.
...
out all sensitive data. Often `memset`

is insufficient — typically it’s
zero-then-free, and compilers see the [lifetime](/blog/2025/09/30/) about to end — but no
...
interface](https://monocypher.org/manual/aead). We “lock” up some data in an encrypted box, write any
...
keys, which we’ll do using [the ](https://docs.python.org/3/library/secrets.html)`secrets`
...
is basically the same in reverse, but throws if the box
...
Find the **complete source **[in my scratch repository](https://github.com/skeeto/scratch/tree/master/wasm-monocypher).
