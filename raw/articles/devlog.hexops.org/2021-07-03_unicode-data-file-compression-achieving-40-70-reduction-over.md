...
url: https://devlog.hexops.org/2021/unicode-data-file-compression/
...
[Background](https://devlog.hexops.org#background)[Problem](https://devlog.hexops.org#problem)[Investigation](https://devlog.hexops.org#investigation)[Zig implementation](https://devlog.hexops.org#zig-implementation)[Results? Better than gzip/brotli; and even better](https://devlog.hexops.org#results-better-than-gzipbrotli-and-even-better-with-them)*with*them![Notable mention](../../assets/30cda17697c77b9d.img)[Conclusion](https://devlog.hexops.org#conclusion)
...
[http://www.unicode.org/Public/UCA/latest/allkeys.txt](http://www.unicode.org/Public/UCA/latest/allkeys.txt)[http://www.unicode.org/Public/UNIDATA/UnicodeData.txt](http://www.unicode.org/Public/UNIDATA/UnicodeData.txt)
...
My secondary thought was that, scrolling through these data files it was obvious most entries were derived from prior entries. Many entries were long runs of data where the next entry had the same value, plus a small increment. For example, at the start of the `allkeys.txt`
...
I did a quick hacky Go implementation of differential encoding on these files to see how well that would work. The results were pretty good, and already beat just `gzip -9`
...
- Zig has variable bit-width integers: I could just write
`u4`

and`u21`

values instead of needing to handle bit packing within larger size integers myself. That was*nice*. - In the Zig standard library it provides:
...
Using these two opcodes, I was able to effectively encode the entire file. The `set`

opcode had some associative data which effectively expressed an entire raw, uncompressed entry in the file (one line.) This increased the file size since it was effectively just adding 4 bits (the opcode) as additional overhead.

### Iteratively finding the most lucrative opcodes

To find the most lucrative (i.e. compressed) opcodes, I printed the data I would associate with an opcode (like `set`

) and then looked for repetitions. Sometimes manually, and sometimes by e.g. piping data to a combination of `sort|uniq -c|sort -r`
...
Once I had narrowed down to a larger group of opcodes that more specifically represented the data, I was able to print the number of bits required to store the change in specific fields (like `value.seq[0]`

) and add even more specific opcodes to use variable bit widths:
...
This meant I could determine the difference in the `key`

and `value`

fields (what those actually are isn’t important, just that they are all minor incremental differences on the prior entry in the file) - set an *incrementor* to do some work on each emission, such as say increment the `key`

array by `[0, 1, 5]`

each emission, and then say “now emit_32 values!”.
...
The two files went from the original 568K (with gzip) down to just 61K (with UDDC+gzip). With this, we are able to equal or match both `gzip -9`

and `brotli -9`

on their own, AND when combined with gzip or brotli we are able to reduce by 40-70%:
...
represents`brotli -9 <file>`

compression`.gz`

represents`gzip -9 <file>`
...
: 298 lines`AllKeysFile.zig`

336 lines`DecompFile.zig`
...
[https://unicode.org/reports/tr10/#Reducing_Sort_Key_Lengths](https://unicode.org/reports/tr10/#Reducing_Sort_Key_Lengths)[https://unicode.org/reports/tr10/#Run-length_Compression](https://unicode.org/reports/tr10/#Run-length_Compression)
