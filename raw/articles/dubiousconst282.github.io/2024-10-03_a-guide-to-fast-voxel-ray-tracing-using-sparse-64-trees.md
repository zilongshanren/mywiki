...
source_blog: dubiousconst282.github.io
source_site: https://dubiousconst282.github.io/
category: graphics
fetched: '2026-04-19'
---

![](../../assets/5692bbc967245ddd.png)
...
Since each node can encode population of at most 8 voxels, and each node takes at least 4 bytes, we can estimate a total of around `4/8 + 4/64 + 4/512 + ... ~ 0.57`

bytes per voxel in the best case, not taking into account materials and other attributes.
...
Applying the same formula as before, we get `12/64 + 12/4096 + 12/32768 + ... ~ 0.19`

bytes per voxel, or nearly 3x less than SVOs, even with the added luxury of absolute pointers (which makes construction and edits a bit easier).
...
Another potential micro-optimization here is to re-write the intersection formula as `sidePos * invDir + (-origin * invDir)`

, which can be evaluated through 3 FMAs rather than 3 separate subtractions and multiplies. I did not find this had any considerable impact and it will increase register usage to maintain the pre-scaled origin. (Unless the shader compiler is already doing this, which I’d find a bit surprising.)
...
[Binary representation of IEEE754 floats](https://en.wikipedia.org/wiki/Single-precision_floating-point_format), Wikipedia.
...
We can start *adapting* the *adaptive* grid ray-marcher to trees by simply inferring bounding-boxes of empty tree sub-divisions at each march step. To do this, we first need to descend down the tree from the root node until either an empty cell or a leaf is found:
...
magic number is an OR-sum of all bits in a 2³ cube at zero coordinates, following the indexing order of `x + z*4 + y*16`
...
This works because flipping the bits of an integer bound by a power of two is equivalent to reversing its value, as a bitwise operation. More precisely, `cond ? ((1<<n)-1) - x : x`
...
Although we could reverse float coordinates from range [1.0, 2.0] to [2.0, 1.0] simply by `3.0 - x`

, our upper bound is exclusive and so this will produce ever so slightly off results, which can cause some minor artifacts if the resulting hit coordinates are used for things like light bounces.
...
[Ray Tracing in One Weekend](https://raytracing.github.io/)[Coding Adventure: Ray Tracing](https://www.youtube.com/watch?v=Qz0KTGYJtUk)[Ray Tracing with Voxels in C++ Series](https://jacco.ompf2.com/2024/04/24/ray-tracing-with-voxels-in-c-series-part-1/)[Physically Based Rendering: From Theory To Implementation](https://www.pbrt.org/)- and more…

![](../../assets/32aed95bae63c9a9.jpg)


![](../../assets/6f79843171b00964.jpg)
