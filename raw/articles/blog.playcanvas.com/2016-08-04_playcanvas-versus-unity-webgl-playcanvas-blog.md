---
title: PlayCanvas versus Unity WebGL | PlayCanvas Blog
url: https://blog.playcanvas.com/playcanvas-versus-unity-webgl
...
### Download Size[](https://blog.playcanvas.com#download-size)
...
### Load Time[](https://blog.playcanvas.com#load-time)
...
**The PlayCanvas app's load times are up to 43 times faster than the Unity app.**- The Unity app fails to even load on lower end devices. The sheer amount of JavaScript causes the browser on those devices to run out of memory loading the page.
...
### Frame Rate[](https://blog.playcanvas.com#frame-rate)
...
**PlayCanvas frame rates are up to 4 times greater than Unity.**In particular, Unity seems to perform poorly in Safari on iOS.- Unity exhibits very unstable performance in Chrome for Android. Initially, the app's frame rate is in the mid to high teens for approximately 20 seconds before it starts to rise to a number in the 50s. At that point it, it regularly drops frames and never reaches a solid 60FPS.
...
### Conclusion[](https://blog.playcanvas.com#conclusion)

To summarize:

**Unity WebGL apps are up to 21 times larger.****PlayCanvas apps load up to 43 times faster.****PlayCanvas app frame rates are up to 4 times higher.**
...
**Update**[](https://blog.playcanvas.com#update)
