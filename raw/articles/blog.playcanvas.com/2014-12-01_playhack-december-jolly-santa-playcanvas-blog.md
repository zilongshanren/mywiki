---
title: 'PLAYHACK December: Jolly Santa | PlayCanvas Blog'
url: https://blog.playcanvas.com/playhack-december-jolly-santa
author: Christy O'Connor
published: '2014-12-01'
source_blog: PlayCanvas
source_site: https://blog.playcanvas.com
category: graphics
fetched: '2026-04-13'
---

![PLAYHACK Logo](../../assets/78a828d2bf3cbcba.jpg)


If spending part of your December making a festively themed game doesn't get you in the mood for the Christmas season, we don't know what will.

Its December 2014 and before the year is out we have a festive game jam for you to test your skills in.

## How does it work?[](https://blog.playcanvas.com#how-does-it-work)

PLAYHACK starts today! We'll provide you with a 3D model and your mission is to turn it into a game. The only rule: you must use the model somewhere in your game.

It works like this:

**Fork**into your account.[this month's project](https://playcanvas.com/project/333365/overview/playhack-dec-14)**Use the project and it's contents to build a new game.****Post to your**about your game throughout the month.[devlog](https://blog.playcanvas.com/the-devlog-playcanvas-community-feature/)**At the end of the month we'll look through all the projects that were forked from the original.****We'll pick our favorite and feature it on the front page of PlayCanvas for everyone to see.**

## The Model[](https://blog.playcanvas.com#the-model)

![Santa](../../assets/93c1813034e0f892.jpg)


We are giving **you the reins** (get it?), to take the game wherever you want. Well almost. Incorporating the sack and present is optional, but you must use the **Jolly Santa model** (complete with sleigh and Rudolph).

Whether it's an endless **side scroller** or an intense **racing game**, you can use this month's model to create any game that you like.

Look out for another series of tutorial blog posts coming soon to give you some ideas coming soon.

## Judging[](https://blog.playcanvas.com#judging)

The process of judging will be as broad as the rules. You won't be marked on the strength of code, graphics or theme, **the winning game will be the project that we enjoy the most**. Simple.

We will be holding things down here at HQ during the month long jam by **sharing** some of the best **screen shots**, **DevLog entries** and **general updates** of your progress.

## Featured Winner[](https://blog.playcanvas.com#featured-winner)

On the **1st of January** we will judge your projects and our favorite will be **featured on the PlayCanvas landing page!**

![SRL](../../assets/830b5aab78181244.png)


[Play SRL](https://playcanv.as/p/o5sSYIiR/) by lizzip and auroraimortalis

## Last Month[](https://blog.playcanvas.com#last-month)

With **ramps**, **barrels**, **hyper speed turrets** and **explosions**, last month's PlayHack was crazy (in the best way possible).

Our favorite entry is [SRL](https://playcanv.as/p/o5sSYIiR/) by PlayCanvas users [ lizzip](https://playcanvas.com/user/lizzip) and

[, whose sci-fi maze game really impressed us using security cameras, laser doors and the Hover Tank acting as the villain. Amazing!](https://playcanvas.com/user/auroraimortalis)

**auroraimortalis**Honorable mentions go to PlayCanvas users [ crefossus](https://playcanvas.com/user/crefossus) (

[game](https://playcanv.as/p/bAARELwk/)) and

[(](https://playcanvas.com/user/danielpettersson)

**danielpetterson**[game](https://playcanv.as/p/KRE8VnRm/)), but thank you to everyone who took part. We can't wait to see what you can come up with this month!

**Click the giant button below to begin your project**!