---
title: PLAYHACK November - Turrets and shooting bullets | PlayCanvas Blog
url: https://blog.playcanvas.com/playhack-november-turrets-and-shooting-bullets
author: Dave Evans
...
### Rotating Turret[](https://blog.playcanvas.com#rotating-turret)
...
This is the code in **turretcontrols.js**. It's very simple. In the initialize function we find the Entity called **Tank_gun_turret**. This Entity is the separate model for the gun turret. Then each frame we check to see if the A or D key is pressed and we rotate the turret to the left or right using the `rotate()`

method on the Entity.

### Shooting Bullets[](https://blog.playcanvas.com#shooting-bullets)
...
function above. This is called when the player presses the S key. It introduces a few different concepts:

`Entity.clone()`

[](https://blog.playcanvas.com#entityclone)

The `clone()`

method on Entities creates a complete copy of the Entity and its components. When you clone an Entity it is created but not added into the scene hierarchy so you have to call the `addChild()`

method and add it to the parent of your choice. In this case, we're just adding it to the root.

#### Enabling and Disabling Entities[](https://blog.playcanvas.com#enabling-and-disabling-entities)
...
### Target Practice[](https://blog.playcanvas.com#target-practice)
