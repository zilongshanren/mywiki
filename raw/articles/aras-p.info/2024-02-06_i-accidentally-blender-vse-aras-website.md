---
title: I accidentally Blender VSE · Aras' website
url: https://aras-p.info/blog/2024/02/06/I-accidentally-Blender-VSE/
...
like [libavcodec](https://en.wikipedia.org/wiki/Libavcodec) are used. Among other things, `libswscale`

is used to do
movie frame RGB↔YUV conversion. Turns out, `libswscale`

can do those parts multi-threaded for a while by now, it’s just
...
but that ends up doing the conversion completely single-threaded. There is a `"threads"`
...
which means you can no longer use `sws_getContext()`

, and instead you have to initialize the context the hard way:
...
And you’d think that’s enough? Haha, of course not. `sws_scale()`

*never* does multi-threading internally. For that, you need
to use `sws_scale_frame()`

instead. And once you do, it crashes since it turns out that you have had created your
`AVFrame`

objects just a tiny bit wrong that was completely fine for `sws_scale`

, but is very much not fine for `sws_scale_frame`
...
can be applied. Whenever that happens, the image that is normally produced by the strip is ![](../../assets/4d8233e2d13f0034.png)


*transformed*into a new one. All of that is done on the CPU, and was multi-threaded already.
...
![](../../assets/cfc615c150739efa.png)

![](../../assets/52c75e3852e838bf.png)
...
![](../../assets/2fde9c89a96ee6a4.png)

![](../../assets/ee81cc68213ecccc.png)
...
![](../../assets/e73716a2a7bca07d.png)

![](../../assets/73a7b2ebf876190c.png)


#### Transparency border around Bilinear filtering

VSE had three filtering options in Blender 4.0 and earlier: `Nearest`

, `Bilinear`

and `Subsampled3x3`

. Of those, only the Bilinear one was adding
...
![](../../assets/04144db13f91aaab.png)

![](../../assets/8c985b9f728dbca5.png)
...
![](../../assets/0e9c2758d1e54fb3.png)

![](../../assets/c285ae1517730afd.png)

![](../../assets/8bf603ac887f933f.png)
...
- Get rid of virtual functions from the inner loop, and some SIMD for bilinear filtering (
[#115653](https://projects.blender.org/blender/blender/pulls/115653)), - Simplify cubic filtering, and add some SIMD (
[#117100](https://projects.blender.org/blender/blender/pulls/117100)), - Simplify math used by Box (née Subsampled3x3) filter (
[#117125](https://projects.blender.org/blender/blender/pulls/117125)), - Fix “does a solid image that covers the whole screen, and so we can skip everything under it” optimization not working, when said image has scale
(
...
- Glow effect: multi-threaded now, 6x-10x faster (
[#115818](https://projects.blender.org/blender/blender/pulls/115818)). - Wipe effect: multi-threaded now, and simplify excessive trigonometry in Clock wipe; 6x-20x faster
(
[#115837](https://projects.blender.org/blender/blender/pulls/115837)). - Gamma Cross effect: was doing really complex table + interpolation based things just to avoid a single square root call.
Felt like the code was written before hardware floating point was invented :) 4x faster now
(
[#115801](https://projects.blender.org/blender/blender/pulls/115801)). - Gaussian Blur effect: 1.5x faster by avoiding some redundant calculations (
[#116089](https://projects.blender.org/blender/blender/pulls/116089)).
