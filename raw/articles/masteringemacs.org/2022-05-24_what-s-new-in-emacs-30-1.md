---
title: What's New in Emacs 30.1?
url: https://www.masteringemacs.org/article/whats-new-in-emacs-301
...
is a tantalizing replacement for company mode and corfu as it combines the buffer and window system (the`*Completions*`

window when you minibuffer complete with`TAB`

) with an*excellent*user experience by adding a schnazzy predictive typing that the old auto complete package did. I think`*Completions*`

system already in Emacs, combined with the preview option, makes it a useful alternative to Company / Corfu for some.You also get to leverage the full force of Emacs’s expansive set of minibuffer completion styles, orders and more, though preview mode does have a few limitations around the style you can use.

This is my favorite feature in Emacs 30.1. The user experience is superb; someone really cared for this, and it shows.
...
The org protocol feature’s not new to Emacs 30.1, but automatically configuring your Emacs on Linux so it works properly

*is*.Which-key is now built in, which is good news for people who prefer that method of help (as opposed to typing

`C-h`

in a key chord).`libjansson`

is dead; long live Emacs’s builtin JSON parser. The new parser is much faster.The tree-sitter-specific

`*-sexp`

commands (such as`forward-sexp`

) now work well.Native compilation is on by default, provided your system has the required dependencies. A massive vote of confidence for a great feature.

Full support for Emacs on Android, and a whole host of touch screen-related improvements to Emacs as a result.

You can now request a diff of your multi-file replace regexp (see

`C-h a replace diff`

)More Tramp (Emacs’s remote file editing system) connections, including support for Kubernetes pods.


I will, as per usual, begin to update [my book, Mastering Emacs](https://www.masteringemacs.org/book), so it includes these latest changes.

Until then, and for the next couple of days, you can enjoy a 30% discount on my book. Enjoy!
...
That is good news. I’m hoping by declaring that native compilation, a must-have feature that [compiles elisp into native code](https://www.masteringemacs.org/article/speed-up-emacs-libjansson-native-elisp-compilation) and greatly speeds up your Emacs, it encourage more distros to adopt the setting, if they have not already. If you’re the type to compile your own Emacs, you may want to read my article as it does require a few mandatory components that most Linux distributions do not ship with.
...
I’ve been following this development for quite a while, and it’s wonderful to see that you can run a full-blown Emacs instance on Android *and* with the support of the Emacs developers behind it. How exciting. I’ll have to find the time to play around with it.
...
JSON is a massive bottleneck when you’re using LSP servers, as LSPs are chatty and JSON is how they communicate. I wrote about the introduction of [libjansson](https://www.masteringemacs.org/article/speed-up-emacs-libjansson-native-elisp-compilation) when it was first introduced as it made a noticeable difference in performance back when it made an optional dependency in Emacs. It turns out that libjansson is not that quick. One day, a new contributor, Géza Herman, showed up with a JSON parser that was not only faster than libjansson, but more complete in its implementation.

As a result libjansson is not only deprecated but removed entirely. Excellent!
...
If you’re the type of person to compile your own Emacs, you might as well give this one a whirl. GC bugs do happen, but I’d say they’re rare enough that you can turn it off and claim your 5% speed-up voucher.
...
OK, no lie, this is a really cool feature that just got easier to use.

Org can register (on Windows, Linux, etc.) itself as a URI protocol controller. When you open a `https`

hyperlink, your OS knows it has to use your preferred browser; ditto for e-mail. Here it’s called `org-protocol`

, and with this new change (or the slightly more manual steps outlined [in the org mode manual](https://orgmode.org/worg/org-contrib/org-protocol.html)) you can request that `org-protocol://....`

contents are sent to your Emacs for processing.

That means you can send information to Org from other tools — such as the commandline or, more tantalizingly, from your web browser or even e-mails, provided your e-mail client follows hyperlinks and supports custom protocols.

Provided you’ve done the installation steps (I recommend you check out [the link](https://orgmode.org/worg/org-contrib/org-protocol.html) if the example below does not work) you can use a snippet like this in a Bookmarklet in your browser – or straight into the console for testing:
...
Replacing `template: "m"`

with the `M-x org-capture`

template key of your choice. The document’s title, URL and text selection is then sent to your org capture.

Pretty darn cool! If you’re an org mode user – I know you are! – you should get on top of this right away.

You can even register your own protocols (`org-protocol-protocol-alist`

) and build your own integrations, and it’s not like you have to feed it into org!
...
Do not sleep on Emacs’s client-server architecture, or the headless evaluation feature it has. My article on [Fuzzy Finding with Emacs Instead of fzf](https://www.masteringemacs.org/article/fuzzy-finding-emacs-instead-of-fzf) is a great example of how two dozen lines of elisp and another baker’s dozen of bash can clone the keystone feature of `fzf`
...
I mean, I’m glad they’re slowly making these changes, as the tree-sitter modes are threadbare. Aside from better (usually) syntax highlighting and indentation, they rarely offer much.

But this is yet another thing to explain to people that, yes, now your dir-locals will pick up on the TS mode you’re using, but all the other features present in the old major modes probably don’t work.

In my opinion, tree-sitter major modes should be deprecated and the syntax highlighting and indentation relegated to optional minor modes you can enable in the existing major modes (and obviously brand new ones, if no such major modes exist) and all other “tree-sitter-portable” custom commands are gated behind a feature flag that require the presence of TS and the right grammar file to work, with perhaps a fallback for things that can be implemented – or is already – with non-TS code.
...
I’ve never in my life been able to remember what the numbers map to, so I this is a useful aid to me and, no doubt, anybody else looking to remap or just figure out what a mouse event is.
...
This governs the `*Completions*`

window that may (or may not, as the case may be) pop up to offer assistance if when you are completing. If you use the default completion mechanism in Emacs, you may want to consider looking into tweaking this, if you find the current `*Completions*`

behavior is not to your liking.
...
Anything that speeds up a known and serious IO bottleneck in Emacs is welcome news to me. The fact they named the variable `fast-read-process-output`
...
I am not sure why there was ever the option of sending email addressed alongside HTTP requests. I don’t recall that ever being a thing in the 90s either, but perhaps that is just my memory failing me.
...
I recommend you try [ultra-scroll](https://github.com/jdtsmith/ultra-scroll) if you’re serious about wanting smooth scrolling in Emacs.
...
It shouldn’t come as a surprise to anybody that Emacs, a dynamic computing environment, can be coaxed into running stuff it shouldn’t. Ideally that’d never happen, but given the nature of what Emacs does – and how integrated it is in our lives – it is no surprise that on occasion its power is turned against it. What surprises me is that these issues don’t happen more frequently.
...
This obviously goes hand in hand with the Android changes. The myth that Emacs is a keyboard only system is obviously false, and it has been since Lucid/XEmacs improved mouse support in the 1990s. Now that touch screen support is improved, I’m curious to see if we’ll see the emergence of org packages/email clients on Android that lean into the new touch support.
...
This is addition to things like squiggly underlines that we’ve had for a long time now. I’m quite surprised that terminals can render anything other than a opaque line, though; that is news to me.
...
Can’t say I’ve ever had the desire to heavily mod my mode line, but I know that a lot of people care greatly about its aethestics. I’m sure this is of benefit to people who use right-to-left languages also; perhaps they are the primary users?
...
I guess this refers to the ‘window peek’ feature in Windows’ taskbar, and that it’s really more of a bugfix / tweak more than an actual feature.
...
Emacs will watch file changes using a range of mechanisms depending on the platform of choice.
...
There’s a longer-running theme to try and make the `*Completions*`

window useful in contexts outside the minibuffer’s completion mechanism. This mechanism is rather confusingly named `completion-in-region`

, even though it’s not just about acting on a region. If you use popup completion tools like company and corfu, then you should consider experimenting with this improved completion system, as it’s a “native” buffer-and-window-based approach to the same.
...
There are popular third-party packages that do much of this already, so it’s nice to see that a similar feature has made it into core. My article on [Understanding Minibuffer Completion](https://www.masteringemacs.org/article/understanding-minibuffer-completion) goes into more detail on how to configure minibuffer completion in Emacs.
...
See [my article](https://www.masteringemacs.org/article/understanding-minibuffer-completion) for more information on customizing Emacs’s minibuffer.
...
Coaxing Emacs into putting your windows where you want them to go is a common source of frustrating. I wrote [Demystifying Emacs’s Window Manager](https://www.masteringemacs.org/article/demystifying-emacs-window-manager) in part to help people wrangle the windows into place. Dedicated windows are a blunt (though useful) tool that tells Emacs not to use a window for anything other than what is in it already. This feature mirrors what you find in a lot of other IDEs and editors where most windows have a singular purpose, unlike Emacs’.
...
This is pretty advanced stuff, but if you’re unhappy with the default methods employed by Emacs (see `display-buffer-fallback-action`

) you can now tell it to more aggressively reuse windows using the “most recently used” order of windows.
...
I do not think you’re likely to encounter code that does this just yet, but I suppose this is one way of disambiguating windows that may share the same buffer name or major mode, but differ in some crucial way that only a custom cons cell `category`

can sort out.

Pretty niche. You’re probably not going to use this *really* into `display-buffer-alist`
...
Window configurations are, well, configurations of windows. They are snapshots of window layouts: `C-x r w`

puts your current window configuration into a register, and that is one such use; the other is tab bars in `tab-bar-mode`

. Both set and restore window configurations when you change tabs or explicitly change them with a register. This variable’s really quite complex, so if you have strong opinions on this (you probably do not) then you really should read the lengthy docstring for this variable.
...
You can now change the cursor type for each window. Neat. The variable `cursor-type`

used to just override it for the whole Emacs session.
...
I suppose this is what the change above is really about: to remove bespoke code and generalize it into the new category system.
...
Useful on portable devices (like Android) that may not have certain modifier keys on its native keyboard.
...
Tab bars are window configurations; tab lines are basically tabs like they appear in your browser.
...
If you have multiple tabs featuring the same buffer, you can now preserve the point location between tabs.
...
A better way of configuring tab bar/line modes’ keys. You can opt for more complex keys, and even use keyboard repeat maps if you use `C-<TAB>`
...
Other notable commands include `M-x find-variable`

, `M-x find-function`

and `M-x find-function-on-key`

to jump to their source definitions. I use ’em all the time.
...
All this does is determine whether `outline-minor-mode`

is enabled and configured to make headings collapsible.
...
This is really only likely to be of interest and use to elisp hackers who know their way around Emacs’s internals. The declared type for `+`

is `Declared type: (function (&rest (or number marker)) number)`
...
A useful change, even though I doubt I’ll remember that I can do this. For inputtable characters I normally type them then use `C-u C-x =`

to see its unicode name along with a host of other useful information.
...
This I am more likely to find a use for. `C-h b`

is a must-know command, and now it shows the unicode name for self-inserted characters.
...
Wow. This change is great. Directory-local variables are elisp variables declared in one of several files (such as `.dir-locals.el`

) that you can use to effect directory-specific changes to Emacs buffers. I really love dirlocals; however, they’re a pain to add and edit. `M-x add-dir-local-variable`

is a newbie trap, and it’s easy to type the wrong thing and cause all sorts of issues (or worse: that nothing at all happens.)

A nice customize UI will go a long way towards encouraging more people to use it.
...
One cool aspect of Customize is that you can set settings for the duration only, and then later on return to finalize them and save them to a file. All this does is add that functionality to booleans.
...
Very useful. Emacs is squeamish about dirlocals as it maintains a list of safe variables and anything else is considered unsafe, and you’re asked to approve every setting. Whitelisting certain directories is a great compromise for people who are OK with the risks that follow from such a feature.

## CL Print

This is for elisp programmers only, and governs how Elisp forms are printed on the screen for human consumption.
...
kills buffers by regexp; and now you can have it do so without asking your permission.
...
Ah, very handy. You can already diff files you want to save with `C-x s`
...
TAB, eh? I’m most assuredly going to accidentally reveal a password that way. You can edit `read-passwd-map`

to remove the TAB if you want, though.
...
Good. You can do this with `M-x remove-hook`

also. Useful if you foolishly cram lambdas into hooks or advice… and are now struggling to remove them. But you’d never make that mistake, right? :)
...
Uniquify renames buffers (but not the underlying files, if any) to sensible names instead of `foo<1>`

, `foo<2>`

, etc. It’s enabled by default, nowadays, but if you use the projects feature in Emacs, and if you frequently open the same file name in multiple projects, give this setting a try.
...
These are Tramp features, part of Emacs’s remote file editing system.
...
Emacs uses a wide range of tools to infer the major mode to use. Filename is not the only way: Emacs will scan the file to try and determine the major mode if there is no obvious choice that matches its filename. Looking at `/usr/bin/env`
...
You can probably experiment with setting this even higher to batch up output from commands. Your mileage may vary, though. Mine is set to `1048576`
...
This is worth enabling if inconsistent kill-yank indentation annoys you. Doing this correctly, especially in whitespace sensitive languages, is really important in structured editing tools such as my package, [Combobulate](https://www.masteringemacs.org/article/combobulate-structured-movement-editing-treesitter).
...
This is a very welcome addition for bulk text replacing. It’s a nervewracking thing to accept/reject changes and hope you didn’t screw it up. Having a dedicated diff window that works across multiple files is a superb addition to Emacs.
...
Advanced indeed. Registers are capable of incrementing and decrementing a number already, and kmacro (Emacs’s keyboard macro facility) has its own internal counter (see [Keyboard Macros are Misunderstood](https://www.masteringemacs.org/article/keyboard-macros-are-misunderstood)) also. Now you can switch values between them and also do simple comparisons. Combine it with the ability to physically move up or down a line; increment and decrement; and conditional termination, and I’m pretty sure kmacro close to Turing-complete.
...
Excellent. There’s already a kmacro ring for macros known to Emacs, but it’s a bit awkward. Now you can list the macros.
...
This is for `M-x electric-pair-mode`

, which automatically pairs an opening character with its constituent closing character.
...
is also Alt+Tab. The key binding `C-M-i`
...
The mode line adopts a different character for the coding system in the active buffer. You can find it in the bottom left.
...
Input methods are really neat. You can switch to a ‘virtual keyboard’ that behaves like another language, letting you enter diacritics and complex unicode characters using your current keyboard’s layout. You are not limited to actual languages, either: there are input methods to help you enter LaTeX for example.

You can toggle the input method with `C-\`

and with a prefix argument to set the input method type. I wrote about them in [Olé! Diacritics in Emacs](https://www.masteringemacs.org/article/diacritics-in-emacs). I even wrote my own input method for [Inserting Emoji](https://www.masteringemacs.org/article/inserting-emoji-input-methods) just to highlight how powerful and simple the system is.
...
Wonderful news for people who prefer the original spelling of encyclopedia, and Danes, I suppose.
...
Outline mode is a cut-down outliner for collapsing and expanding headings.
...
Excellent news, and likely to be far more robust as it uses tree-sitter’s nodes to determine what to hide and show.

### Info

Info mode is Emacs’s implementation of the TexInfo hypertext viewer.
...
GUD is a user interface for debuggers such as GDB.
...
Grep (or rather the interface for talking to and displaying the results from `grep`

) is a powerful feature in Emacs with a wide range of commands.
...
Good news if you prefer the heading system used by tools such as `rg`

. I have not tried it yet, but I wonder how easy it is to bulk edit the output of a grep command when it is in header mode.

### Compilation mode

Compilation mode (via `M-x compile`

usually) is Emacs’s interface for talking to not just compilers but any tool that emits structured output.
...
Emacs already has a huge range of compilation rules, so I’m surprised it didn’t have it for Lua, given how popular it is.

### Project

Project is one of many, many file project solutions built into Emacs, and arguably the best one to use. See `C-x p C-h`
...
If you’re not strapped for space, why not enable this?
...
Root markers are used to find the boundaries of a project (for example the `.git`

directory, or maybe a file such as a Makefile), and that works well enough for simple use cases. You can tell Emacs to look for other things in addition to its builtin rules, and this is where the contract of a root marker breaks down a little, particularly if you have a repo and another non-repo marker inside of that.
...
This is effectively a way of executing a key bindings from the root directory of the current buffer’s root project.
...
VC is Emacs’s excellent generic version control interface. If you do not use [Magit, an Emacs mode for Git](https://www.masteringemacs.org/article/introduction-magit-emacs-mode-git), or if you use a source control that is not git, then I highly recommend you sit down and learn how to use VC. It’s really excellent.
...
I’ve long advocated using the menu bar because telling beginners to turn it off is [Bad Emacs Advice](https://www.masteringemacs.org/article/bad-emacs-advice). The toolbar is harder to justify turning on as it is a little bit threadbare. Having said that, making it even more contextual is still a very positive thing indeed.
...
It is rare indeed for a command to get obsoleted and the unobsoleted.
...
Diff mode is one of several modes for interacting with and browsing `.diff`
...
Very useful in Lisp, in my opinion, where a change of indentation can easily happen when you change something else. I have git set to ignore whitespace by default to avoid indentation changes gumming up the staging area.
...
Good all-round command and saves having to do multiple keystrokes to achieve the same effect.

### Dired

Dired is Emacs’s all-singing, all-dancing directory and file editor, bound to `C-x d`
...
Oh, this is much-needed. Here’s how it works: you can insert the contents of a directory into an existing dired buffer (find a directory and tap `i`

) and act on the files across the directory boundaries as though they were one directory – very powerful, and it means you can use [dired to replace find & xargs](https://www.masteringemacs.org/article/dired-shell-commands-find-xargs-replacement) – but the `p`

and `n`

commands to move between files does not jump between these virtual directories, and so you’re stuck having to move point between a bunch of dead space. Until now, that is. I recommend you set it to `bounded`
...
If you squeeze dired into a side bar window, you can now truncate filenames to make them fit to the size of the window, which is a good quality of life improvement.
...
Emacs has several ways of guessing the right command to run on a filename, and they are annoyingly different and only available in some parts of Emacs and not others. This is a clear attempt to clean up this mess and offer a unified way of picking the right command to run.
...
Excellent addition. I used to do `!`

to do this, but now it has its own key.
...
Worth a look if you want Emacs to open your media files in other tools than its defaults.
...
Omit mode hides useless files based on regexp filters. Great way to hide backup files, compiler detritus and more. You may have to `(require 'dired-x)`
...
This relates to Tramp, Emacs’s remote file editing protocol. Because Tramp sends commands to the remote host, you can now customize how dired should query for files on the remote file system.

### Ediff

Ediff is another diff tool in Emacs, albeit a far more advanced one. It has a wide range of entrypoints: `C-h a ediff`
...
Ediff’s achilles’ heel is its awkward frame that it uses as a control interface; but now it at least grants succor to people who use tiling window managers.

### Buffer Selection

Buffer selection, not to be confused with `C-x b`

, is one of, ah, several buffer switching tools in Emacs. Try it with `M-x bs-show`
...
Eshell is an Emacs shell written entirely in Emacs. It’s worth [Mastering Eshell](https://www.masteringemacs.org/article/complete-guide-mastering-eshell) if you want an all-Emacs command line experience.
...
If you thought someone using korn or c shell wasn’t bad enough, now you can frustrate your coworkers by writing your shell scripts in Eshells’ own zsh-like language.
...
Eshell has a huge range of commands that it exposes to its shell, only to intercept them and pass the input and output to a dedicated Emacs function. See `C-u C-h a eshell/`
...
Eshell’s expansion system is inspired by zsh’s, though they’re not at all interchangeable, as Eshell effectively passes things to elisp functions internally, arguments and all, so being able to split, splice and otherwise manipulate how entries are fed to the underlying commands is very important.
...
One cool feature of Eshell is that you can `cd`

into a remote system using Tramp – for example `cd /ssh:homer@powerplant:/etc/`

– and then operate on the remote machine through Eshell and Tramp as though it were ‘local’. Really powerful, but sometimes there are cracks in the machinery, such as the case of the user and group id environment variables not being correct.
...
I long ago abandoned using `C-a`

and friends to move the beginning of line, preferring `M-m`

instead (as it skips forward past whitespace after moving to the beginning of the line) but this command obviously does not respect the prompt. So if you use `M-m`

, just rebind it to the same key as `C-a`
...
Aliases here referring to the same aliasing you can do in other shells.
...
is a command I use all the time when I want more specificity around searching than what a tool like `rg`

gives me. `grep`

already works great in Eshell, so it’s nice to see that `rgrep`
...
I added this to my bash prompt decades ago, as I find it invaluable when I want to compare exit codes in scripts.
...
Pcomplete is the completer tool originally built for Eshell, but now has uses elsewhere. It’s a pretty nifty little completion engine, as I demonstrate in [PComplete: Context-Sensitive Completion in Emacs](https://www.masteringemacs.org/article/pcomplete-context-sensitive-completion-emacs)
...
The mooted successor to `sudo`

.

### Shell mode

This is the shell I actually use the most day to day. Delightfully dumb and it behaves exactly like a souped-up Emacs buffer. See [Running Shells and Terminal Emulators in Emacs](https://www.masteringemacs.org/article/running-shells-in-emacs-overview) if you want to know what all the differences are.
...
This is a nice change as Emacs will of course keep its own history (see [Shell & Comint Secrets: History commands](https://www.masteringemacs.org/article/shell-comint-secrets-history-commands)) but that history does not persist under normal circumstances. If you’re using a non-default history file for your shell and you want Emacs to respect it, now you can.
...
Anything that speeds up comint is a good thing in my book.

### Prog mode

Prog mode is a generic major mode that programming modes should try to inherit from. It is not intended for normal users.
...
Imenu is a lookup feature for structured elements in the current buffer, such as function and class names in code, and headings and sub-headings in text.
...
If you use a completion mechanism that makes accessing nested elements hard in imenu, you can now request that it flattens the hierarchy.
...
Which function shows the current function in the mode line by default. It works anywhere `M-x imenu`
...
The mode line is crowded and often truncated if you split your windows. Having another place to put this information is useful.
...
Tramp is Emacs’s remote file editing facility.
...
Tramp supports a dizzying array of “connection” methods, including things that aren’t “connections” in the traditional sense, such as `sudo`
...
Tramp added native docker support a version or two ago, which is super handy for editing or viewing stuff inside a container from the comfort of your own Emacs.
...
Good to see more support for namespace-based container solutions other than docker.
...
Tramp can chain connections, so you can connect to a bastion host first before you connect to kubernetes pods.
...
You absolutely must enable connection sharing (controlmaster) if you use ssh. It makes a huge difference in performance.
...
Oh, cool! One common criticism I had of multi-hop/chaining connections is that you really need to know what you’re looking for when you are writing out these multi-hop connections, as the completion support is non-existent. Good to see that Tramp will now attempt to complete via the multi-hop links you have already written. Neat!
...
I’ve had a snippet of code called `sudo`

that does exactly this for what must be a decade plus now. Now it’s built in, albeit with a harder-to-remember name.
...
If you have performance issues with Tramp, you should try this out.
...
SHR is Emacs’s HTML rendering engine that powers EWW, the Emacs Web Wowser.
...
is Emacs’s builtin web browser.
...
Fields here meaning the widget system that Emacs has built in.
...
This feature has been around for ages, and it’s great if you want to remove all the chaff from a page. Note that the only real change here is that it toggles.
...
Great stuff if you regularly visit pages that require toggling on readable mode.
...
This is the tree-sitter version for Go.
...
Man referring here to the manual page tool `man`
...
DocView is a generic tool for converting complex documents into images for display in Emacs (pdf, docx, etc.)
...
Emacs’s SVG engine is quite good nowadays, and it’s definitely better than a raster image of a PDF.
...
If your theme clashes with the colors in SVG files, you can now change them to better suit your theme.
...
Shortdoc is [Emacs’s Builtin Elisp Cheat Sheet](https://www.masteringemacs.org/article/emacs-builtin-elisp-cheat-sheet).
...
Package is the package manager and installer in Emacs.
...
Oh that is very nice. I will definitely enable this now that `package`

can install from source control using `vc`
...
That’s a really cool addition, and I look forward to using it with my own packages. I normally maintain a separate Emacs installation just for this purpose, but this approach might save me the hassle of having to maintain that.

### Flymake

Flymake is Emacs’s linter/on-the-fly-compiler interface.
...
Worth enabling to see if it is something that helps or hinders. You should consider setting it in a dir locals variable or mode hook so it only applies to some buffers.

### Flyspell

Emacs’s on the fly spell checker.
...
One of a dozen modes that edit Javascript.
...
This is just standardizing on the xref interface and its default global keys.
...
The major mode for editing Python.
...
I long ago stopped worrying about this stuff when code formatting tools became prevalent. Why get angry at your coworkers’ bad code formatting habits when you can direct your anger at a faceless code formatting tool instead?
...
Inferior Python mode refers to the comint version that talks to the external python interpreter.
...
Eldoc is a generic documentation popup system.
...
This is a great addition and it rounds off the ability to manually install packages with VC that arrived in Emacs 29. You can replace a lot of the third-party package managers now if your needs are simple.
...
Gnus is an expansive mail and news agent in Emacs.
...
Rmail is one of many ways of sending and reading email in Emacs.
...
Dictionary is a lookup system for [wordsmiths](https://www.masteringemacs.org/article/wordsmithing-in-emacs).
...
Pp is the pretty printing functionality for elisp.
...
The major mode used to edit elisp code.
...
One of two Perl modes in Emacs.
...
Desktop saves and restores your Emacs buffers and files between sessions.
...
I have absolutely been caught out by this before, so that is a welcome change indeed.

### Recentf

Recentf remembers files you have visited and lets you re-visit them quickly.
...
One of several ways of displaying images in Emacs.
...
This is a test runner framework for Emacs, and a rather useful one at that.
...
SOCKS is a proxy-like protocol that, quite frankly, I don’t think I’ve used in decades.
...
This is the major mode for interactively editing keyboard macros.
...
This is Emacs’s advanced symbolic computer algebra system.
...
IELM is the interactive shell for [Evaluating Elisp in Emacs](https://www.masteringemacs.org/article/evaluating-elisp-emacs).
...
EasyPG is a wrapper for GnuPG and friends. See [Keeping Secrets in Emacs with GnuPG and Auth Sources](https://www.masteringemacs.org/article/keeping-secrets-in-emacs-gnupg-auth-sources).
...
This is the internal library used by Emacs for buttons, edit fields, etc. in places like the Customize interface.
...
A framework for plucking the thing at point: words, functions, emails, urls, you name it. A useful thing to learn about and use, as it has a wide range of applications.
...
Bug reference mode is a small minor mode that makes ‘bug references’ (such as #2823) clickable.

### Buffer-menu

Yet another buffer selection and display system in Emacs.
...
RCIRC is one of two IRC clients in Emacs, the other being ERC.
...
FFAP is Find File At Point is a system for plucking the file at point and opening it.
...
Proced’s basically Emacs’s answer to `top`

. See [Displaying and Interacting with processes using Proced](https://www.masteringemacs.org/article/displaying-interacting-processes-proced).
...
The color file has a large range of advanced color conversion routines. Worth a look if that is something you interact with a lot.
...
XWidget is a half-abandoned, half-working attempt at bringing the Webkit browser to Emacs.
...
How does dired work on platforms (such as MS-DOS or Windows) where there is no `ls`

program? The answer is simple: Emacs has an elisp emulation layer that talks to the underlying system directly.
...
There is a host of network utility wrappers in Emacs. See [Using the commandline network utilities from Emacs](https://www.masteringemacs.org/article/network-utilities-emacs).
...
Another obscure feature in Emacs. Here it’s a flat list of common URLs and a command to select and then open one of them. The fact that “Yahoo” is still in there says it all.
...
Obscure library for working with Quail, a format used in multilingual systems.
...
Xref is Emacs’s cross-referencing system, and all this change does is make it so `M-x revert-buffer`

(which I have a version of that reverts the current buffer bound to `F6`

) reverts properly in xref buffers also.
...
Good to see more tree-sitter major modes.
...
Long overdue, even if I find the capabilities of editorconfig rather basic.
...
This is a long-awaited inclusion for some people. Which-key pops up a list of key bindings if you partially type a key and a wait a little while.
...
Buried in the foot of the NEWS file is one of the best new additions in Emacs. A window-based answer to Company / Corfu, with a host of really nice ergonomic features. I really, really recommend you disable your in-line completer (corfu, company, auto-complete, whatever) and give this one a try for a little while.
...
A compromise for people who want the toolbar but only when it has something interesting to offer.
...
Excellent feature that is enabled by default, so no need to do anything.
...
A popular set of themes beloved by many.
...
Interesting inclusion and I am curious to see it in use. Emacs already has parsing tools already (Semantic Bovinator being one such example) so I guess this will find a home in Org mode as it has a range of custom query languages.
...
Compat provides backwards and forwards compatible changes for package authors so they can target older versions of Emacs, and keep older packages working in newer ones, too.
...
The heart of this is that in Lisp, code and data are the same, and when Emacs emits a readable form of a lambda or closure you just created, it does so in a way that looks like a list. In older Emacsen, you could write `(car (lambda))`

and it’d spit out `closure`

(provided `lexical-binding`

is enabled), and now you cannot do this; it’s not a list any more.

It’s very unlikely this will affect you in any way.
...
Emacs added a new advice system many versions ago; it’s better, and I should get around to removing the old advice forms…
...
Yet more complexity around how to remap major modes to accommodate tree-sitter major modes.
...
Ah good. I always found it odd that it would bind to the left-most argument, but now I can specify which one I want.
...
Ah that is very useful actually. Now to remember that it exists…
...
This complexity is yet again borne out of tree-sitter’s introduction to Emacs. I do wonder if all this complexity (remember diamond pattern inheritance in OO?) will come back to bite us in a few years… I hope not.
...
Juri Linkov did an awful lot of work to try and salvage the rather broken tree-sitter “sexp” commands and I think he’s done a stellar job, considering how hard it is to do [Structured Movement and Editing with Tree-Sitter](https://www.masteringemacs.org/article/combobulate-structured-movement-editing-treesitter) in a way that works the way users expect it work.
...
You can now declare the type of functions in Emacs, presumably to help the native compiler make informed choices about the input and return value. We are moving inexorably closer to Common Lisp…
...
A welcome bit of syntactic sugar.
...
Connection local variables are variables local to Tramp-specific connections.
...
The tree-sitter-enabled equivalent of `thing-at-point`

. This really should be merged into the existing thing at point system; if TS is available, then its settings should take precedence.
