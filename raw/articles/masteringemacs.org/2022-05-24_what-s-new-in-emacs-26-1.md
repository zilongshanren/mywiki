---
title: What's New in Emacs 26.1
url: https://www.masteringemacs.org/article/whats-new-in-emacs-26-1
...
I have no idea what prompted this feature, but color management is certainly an interesting addition. I suppose someone’s using Emacs’s `M-x image-dired`

for their photography or art, perhaps?
...
I interpret this to mean that you can now override the default behavior of always saving a buffer to a file (or ask for a filename is no such filename exists). This is only of interest to package developers where `C-x C-s`

might mean “commit to vcs” and not “save to file”.
...
Hey, neat. But I use `auto-insert-mode`

as I rarely want just the magic signature in a shell script.
...
Looks like a slightly higher level than equivalent comint functions. I played around with it and it seems to be aimed squarely at programmatic calls to functions that terminate with a status code, but where you may or may not want the output. For end users the usual `M-|`

, et al. commands are still the go-to option; this function is aimed purely at programmers.
...
This is useful. If you do multiple commands on regions then you may wish to capture the output of multiple commands. Now you can do that whereas before Emacs would clear the `*Shell Command Output*`
...
That is another welcome user experience improvement. I remember when `C-BACKSPACE`

would try to kill into the prompt and you would get an annoying error message. I find that these minor changes have a lasting impact on users’ perception of functional software.
...
I’ve had this issue. Habitually type `M-q`

on a docstring only to not have it make a change and yet when I save (I am a habitual saver also) it tells me there have been changes written to the disk.
...
That is actually very useful. I had no idea there were so many timers running at one time in Emacs. Though I question the choice of `c`
...
have greater claim to be the key that deletes, stops, or removes something?
...
Very useful. There is also a `x-<hyper/super/meta>-keysym`

variable that you can remap in the same way. Great if you want more portability between systems and operating systems, and if you want to control this behavior from inside Emacs.
...
That’s useful, I guess, but I wish it would just do this without the round-about method described here. I’ve been burnt by this more times than I can remember. At least IDO is clever enough to interpret `//`

as the root of the remote system and not to go to the local `/`
...
I’ve always liked `M-x dig`

, a feature of Emacs for many years. :
...
often but I think I could find this useful also.
...
It may not look it, but `Dz`

above is actually `Ǳ`

, a singular unicode codepoint. With that in mind, capitalizing that character correctly then makes perfectly sense.
...
Normally you would need `find-name-dired`

or its counterparts to do this, but having simple globbing available is useful indeed. Remember, `dired`

simply transplants the output of a `ls`

command (or an elisp-equivalent on other platforms) and glams it up with some interactive pizzazz.

For more information: [Working with multiple files in dired](https://www.masteringemacs.org/article/working-multiple-files-dired).
...
For more information: [Dired Shell Commands: The find & xargs replacement](https://www.masteringemacs.org/article/dired-shell-commands-find-xargs-replacement).
...
For more information: [WDired:Editable Dired Buffers](https://www.masteringemacs.org/article/wdired-editable-dired-buffers)
...
(And if you don’t use `M-x compile`
...
for readability mode (to strip out useless cruft and leave just the text that is useful) and hopefully the days of garish fonts bleeding through in EWW are over! Very welcome.
...
That’s really nifty. The key seem to be: `-/+`

to in/decrease size; `r`
...
There’s a full-fledged DNS Zone file editor in Emacs called `M-x dns-mode`

. If you edit zone files, you should check it out.
...
For more information: [Keeping Secrets in Emacs with GnuPG and Auth Sources](https://www.masteringemacs.org/article/keeping-secrets-in-emacs-gnupg-auth-sources).
