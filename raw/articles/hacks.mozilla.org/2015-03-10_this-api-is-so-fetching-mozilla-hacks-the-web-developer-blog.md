...
url: https://hacks.mozilla.org/2015/03/this-api-is-so-fetching/
...
The [Fetch API](https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API) intends to fix most of these problems. It does this by introducing the same primitives to JS that are used in the HTTP protocol. In addition, it introduces a utility function `fetch()`
...
function’s arguments are the same as those passed to the
...
. They map directly to the underlying HTTP concepts, but have
...
objects have a **guard** property. This is not exposed to the Web, but it affects which mutation operations are allowed on the Headers object.
...
with mode “no-cors”. - “response”: naturally, for Headers obtained from Response (
`Response.headers`
...
is not a [valid HTTP Header name](https://fetch.spec.whatwg.org/#concept-header-name). The mutation operations will throw TypeError if there is an immutable guard. Otherwise they fail silently. For example:
...
can only be set by passing initial
...
mode is simple, if a request is made to another origin with this mode set, the result is simply an error. You could use this to ensure that
...
mode is what you’ll usually use to make known cross-origin requests to access various APIs offered by other vendors. These are expected to adhere to
...
enumeration determines if cookies for the other domain are
...
, which is set by the UA to be

the referrer of the Request. This may be an empty string. The other is

`context`

which is a rather [large enumeration](https://fetch.spec.whatwg.org/#requestcredentials) defining what sort of resource is being fetched. This could be “image” if the request is from an tag in the controlled document, “worker” if it is an attempt to load a worker script, and so on. When used with the
...
, which is “basic”, “cors”, “default”, “error” or
...
“Set-Cookie” and “Set-Cookie2”.`"cors"`

: response was received from a valid cross-origin request.[Certain headers and the body](https://fetch.spec.whatwg.org/#concept-filtered-response-cors)may be accessed.`"error"`
...
: response for “no-cors” request to cross-origin resource.[Severely](https://fetch.spec.whatwg.org/#concept-filtered-response-opaque)
...
returns a Response resulting in
...
[ArrayBuffer](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/ArrayBuffer)[ArrayBufferView](https://developer.mozilla.org/en-US/docs/Web/API/ArrayBufferView)(Uint8Array and friends)[Blob](https://developer.mozilla.org/en-US/docs/Web/API/Blob)/[File](https://developer.mozilla.org/en-US/docs/Web/API/File)- string
[URLSearchParams](https://url.spec.whatwg.org/#interface-urlsearchparams)[FormData](https://developer.mozilla.org/en-US/docs/Web/API/FormData)– currently not supported by either Gecko or Blink. Firefox expects to ship this in version 39 along with the rest of Fetch.
...
function), will try to intelligently [determine the content type](https://fetch.spec.whatwg.org/#concept-bodyinit-extract). Request will also automatically set a “Content-Type” header if none is set in the dictionary.
...
Alexander PetrovMarch 11th, 2015 at 02:57Nikhil MaratheMarch 11th, 2015 at 08:00J WMarch 17th, 2015 at 15:02Nikhil MaratheMarch 17th, 2015 at 15:29Alexander PetrovMarch 18th, 2015 at 01:12OliverMarch 19th, 2015 at 02:53Nikhil MaratheMarch 19th, 2015 at 10:16MAMarch 19th, 2015 at 08:40Josh HMarch 19th, 2015 at 20:25