---
title: Fully Loaded Node – A Node.JS Holiday Season, part 2 – Mozilla Hacks - the
  Web developer blog
url: https://hacks.mozilla.org/2012/11/fully-loaded-node-a-node-js-holiday-season-part-2/
author: Mozilla
...
[A Node.JS Holiday Season series]from Mozilla’s Identity team searches for an optimal server application architecture for computation heavy workloads.This is a prose version of a short talk given by
...
This post also introduces the [compute-cluster](https://github.com/lloyd/node-compute-cluster) module: a small Node.JS library that makes it easy to manage a collection of processes to distribute computation.

## The Problem

We chose Node.JS for [Mozilla Persona](https://persona.org), where we built a server that could handle a large number of requests with mixed characteristics.
...
**saturation**: The solution will be able to use every available processor.**responsiveness**: Our application’s UI should remain responsive. Always.**grace**: When overwhelmed with more traffic than we can handle, we should serve as many users as we can, and promptly display a clear error to the remainder.**simplicity**: The solution should be easy to incrementally integrate into an existing server.
...
Many examples exist, one being the excellent [bcrypt library](http://github.com/ncb000gt/node.bcrypt.js) from [Nick Campbell](http://github.com/ncb000gt).
...
The problem here is that the library is using NodeJS’s internal threadpool for a problem that it was not designed for, and this threadpool has a [hardcoded upper bound of 4](https://github.com/joyent/node/blob/e2bcff9aa75e51b9ba071330fe712180abed03e0/deps/uv/src/unix/threadpool.c#L31).

Deeper problems exist with this approach, beyond from these hardcoded limits:

- Flooding NodeJS’s internal threadpool with computation work can starve network or file operations, which hurts
**responsiveness**. - There’s no good way to control the backlog – If you have 5 minutes of computation work already sitting in your queue, do you really want to pile more on?
...
NodeJS 0.6.x and up offer a [cluster module](http://nodejs.org/docs/v0.8.14/api/all.html#all_how_it_works) which allows for the creation of processes which “share a listening socket” to balance load across some number of child processes.
...
We’ve generalized this solution in the [compute-cluster](https://github.com/lloyd/node-compute-cluster) library.

`compute-cluster`

spawns and manages processes for you, giving you a programatic means of running work on a local cluster of child processes.
...
events to handle incoming work:
...
It is possible to integrate `compute-cluster`

behind an existing asynchronous API without modifying the caller, and to start really performing work in parallel across multiple processors with minimal code change.
...
**simplicity**: Integration into an existing project is easy: By hiding the details of `compute-cluster`

behind a simple asynchronous API, calling code remains happily oblivious of the details.
...
Combining this knowledge with a client supplied parameter, `max_request_time`

, makes it possible to preemptively fail on requests that are likely to take longer than allowable.

This feature lets you easily map a user experience requirement into your code: “The user should not have to wait more than 10s to login”, translates into a `max_request_time`

of about 7 seconds (with padding for network time).
...
The future of [compute-cluster](https://github.com/lloyd/node-compute-cluster) may involve the ability to distribute work over multiple different tiers to maximally saturate available computation resources in times of load.
...
Or we may solve the problem a different way! If you have thoughts on an elegant way to enable [compute-cluster](https://github.com/lloyd/node-compute-cluster) to distribute work over the network while preserving the properties it has thus far, I’d love to hear them!

Thanks for reading, and you join the discussion and learn more about current scaling challenges and approaches in Persona on [our email list](https://groups.google.com/d/msg/mozilla.dev.identity/st_eL7kUpUw/hrOUUatl5i0J).

## 16 comments

Johan SundströmNovember 20th, 2012 at 11:18Lloyd HilaielNovember 20th, 2012 at 13:00Ryan DohertyNovember 20th, 2012 at 12:38Lloyd HilaielNovember 20th, 2012 at 13:46Ryan DohertyNovember 20th, 2012 at 15:20NNNovember 20th, 2012 at 17:23Lloyd HilaielJanuary 31st, 2013 at 10:09Andrew ChiltonNovember 20th, 2012 at 18:54Lloyd HilaielJanuary 31st, 2013 at 09:45alessioalexNovember 23rd, 2012 at 07:58dotnetcarpenterNovember 28th, 2012 at 15:09Ralf MuellerJanuary 29th, 2013 at 11:55Lloyd HilaielJanuary 31st, 2013 at 09:50gggeekJanuary 30th, 2013 at 13:04Lloyd HilaielJanuary 31st, 2013 at 10:08JorgeMarch 31st, 2013 at 21:52