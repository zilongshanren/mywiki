---
title: Implementing Private Fields for JavaScript – Mozilla Hacks - the Web developer
  blog
...
Private fields are a language feature being added to the JavaScript language through the [TC39](https://tc39.es/) [proposal process](https://tc39.es/process-document/), as part of the [class fields proposal](https://github.com/tc39/proposal-class-fields), which is at Stage 4 in the TC39 process. We will ship private fields and private methods in Firefox 90.
...
The most basic mental model one can have for private fields is what it says on the tin: [fields](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Classes/Public_class_fields), but private. Now, JS fields become properties on objects, so the mental model is perhaps ‘properties that can’t be accessed from outside the class’.

However, when we encounter proxies, this mental model breaks down a bit; trying to specify the semantics for ‘hidden properties’ and proxies [is challenging](https://github.com/zenparsing/es-abstract-refs/issues/11#issuecomment-65723350) (what happens when a Proxy is trying to provide access control to properties, if you aren’t supposed to be able see private fields with Proxies? Can subclasses access private fields? Do private fields participate in prototype inheritance?) . In order to preserve the desired privacy properties an alternative mental model became the way the committee thinks about private fields.
...
The actual specification changes are provided by the [class fields proposal](https://github.com/tc39/proposal-class-fields), specifically the [changes to the specification text](https://tc39.es/proposal-class-fields/). I won’t cover every piece of this specification text, but I’ll call out specific aspects to help elucidate the differences between specification text and implementation.

First, the specification adds the notion of [ [[PrivateName]]](https://tc39.es/proposal-class-fields/#sec-private-names), which is a globally unique field identifier. This global uniqueness is to ensure that two classes cannot access each other’s fields merely by having the same name.
...
The specification also adds a new [‘internal slot’](https://tc39.es/ecma262/#sec-object-internal-methods-and-internal-slots), which is a specification level piece of internal state associated with an object in the spec, called [ [[PrivateFieldValues]]](https://tc39.es/proposal-class-fields/#sec-private-names) to all objects.
...
This turns out to be possible, [a side effect of some of the specification’s handling of constructor return values](https://www.mgaudet.ca/technical/2020/7/24/investigating-the-return-behaviour-of-js-constructors). To be more concrete, the following is an example provided to me by André Bargull, which shows this in action.
...
Where it is possible to implement the steps of the specification directly, we prefer to do that, as it makes maintenance of features easier as specification changes are made. SpiderMonkey does this in many places. You will see sections of code that are transcriptions of specification algorithms, [with step](https://searchfox.org/mozilla-central/rev/3434a9df60373a997263107e6f124fb164ddebf2/js/src/vm/JSFunction.cpp#1143-1210) [numbers for comments](https://searchfox.org/mozilla-central/rev/3434a9df60373a997263107e6f124fb164ddebf2/js/src/builtin/Array.js#7-47). Following the exact letter of the specification can also be helpful where the specification is highly complex and small divergences can lead to compatibility risks.
...
The specification language for Private Names already almost matches the semantics around [ Symbols](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Symbol), which already exist in SpiderMonkey. So adding

`PrivateNames`

as a special kind of `Symbol`

is a fairly easy choice.#### Implementing Private Fields
...
- Accessing an a property on an object that doesn’t have it returns
`undefined`
...
algorithm - Setting a property on an object that doesn’t have it simply adds the property. Private fields will throw a
`TypeError`

in.`PrivateFieldSet`

- Adding a private field to an object that already has that field also throws a
`TypeError`

in. See “The Constructor Override Trick” above for how this can happen.`PrivateFieldAdd`


To handle the different semantics, we modified the bytecode emission for private field accesses. We added a new bytecode op, `CheckPrivateField`
...
is designed such that we can easily implement an [inline cache](https://www.mgaudet.ca/technical/2018/6/5/an-inline-cache-isnt-just-a-cache) using [CacheIR](https://jandemooij.nl/blog/2017/01/25/cacheir/). Since we are storing private fields as properties, we can use the Shape of an object as a guard, and simply return the appropriate boolean value. The Shape of an object in SpiderMonkey determines what properties it has, and where they are located in the storage for that object. Objects that have the same shape are guaranteed to have the same properties, and it’s a perfect check for an IC for `CheckPrivateField`
...
A IJune 11th, 2021 at 17:18Matthew GaudetJune 15th, 2021 at 09:25