---
title: Taming Configurations with node-convict – A Node.JS Holiday Season, part 7
  – Mozilla Hacks - the Web developer blog
url: https://hacks.mozilla.org/2013/03/taming-configurations-with-node-convict-a-node-js-holiday-season-part-7/
author: Mozilla
...
This is episode 7, out of a total 12, in the

[A Node.JS Holiday Season series]from Mozilla’s Identity team. Today it’s time to talk about configuration.
...
**What if a setting was configured incorrectly?**We can save headaches by detecting and reporting misconfigurations as early as possible.**How easily is it understood**by Ops/QA/and other collaborators that may need to adjust settings or diagnose issues? A more declarative format that also embeds documentation can make lives easier.
...
, etc.), or it can be a function to check a custom format. During validation, if a format check fails it will be added to the error report.**Default values**: Every setting*must*have a default value.**Environmental variables**: If the variable specified by`env`

has a value, it will overwrite the setting’s default value.**Documentation**: The`doc`
...
property out entirely, convict will check that the setting has the same type as the default value (according to [ Object.prototype.toString.call](http://perfectionkills.com/instanceof-considered-harmful-or-how-to-write-a-robust-isarray/)). For instance, the following three schemas are equivalent:
...
AaronMarch 8th, 2013 at 08:08Josh SchellMarch 8th, 2013 at 09:06