---
title: Multi-Value All The Wasm! – Mozilla Hacks - the Web developer blog
...
[Multi-value](https://github.com/WebAssembly/multi-value) is a proposed extension to core WebAssembly that enables functions to return many values, among other things. It is also a pre-requisite for [Wasm interface types](https://github.com/WebAssembly/interface-types/).
...
[The multi-value proposal](https://github.com/WebAssembly/multi-value) is an extension to the WebAssembly standard that lifts these arity restrictions. Under the new multi-value Wasm rules:
...
The multi-value proposal is currently at phase 3 of [the WebAssembly standardization process](https://github.com/WebAssembly/meetings/blob/master/process/phases.md).
...
Shrinking code size is great, and new instructions would be fancy, but here’s what I’m really excited about: [WebAssembly interface types](https://github.com/WebAssembly/interface-types/). Interface types used to be called “host bindings,” and they are the key to unlocking:

- direct, optimized access to the browser’s DOM methods on the Web,
- “shared-nothing linking” of WebAssembly modules, and
- defining language-neutral interfaces, like
[WASI](https://wasi.dev/).
...
: A library for transforming and rewriting WebAssembly modules, used by`wasm-bindgen`

‘s post-processor.`wasmparser`

: An event-style parser for WebAssembly binaries, used by`walrus`
...
[ wasmparser is an event-style parser for WebAssembly binaries.](https://github.com/bytecodealliance/wasmparser) It may seem strange that adding toolchain support for

*generating*multi-value Wasm began with

*parsing*multi-value Wasm. But it is necessary to make testing easy and painless, and we needed it eventually for Wasmtime anyways, which also uses
...
byte means there is a single`i32`

result - a
`0x7e`

byte means there is a single`i64`
...
The index is encoded as a signed variable-length integer, using [the LEB128 encoding](https://en.wikipedia.org/wiki/LEB128). If we interpret non-multi-value blocks’ optional result value type as a signed LEB128, we get:

`-64`

(the smallest number that can be encoded as a single byte with signed LEB128) means there is no result`-1`

means there is a single`i32`

result`-2`

means there is a single`i64`
...
‘s validation implementation is similar to [the validation algorithm presented in the appendix of the WebAssembly spec](https://webassembly.github.io/spec/core/appendix/algorithm.html): it abstractly interprets the Wasm instructions, maintaining a stack of types, rather than a stack of values. If any operation uses operands of the wrong type — for example the stack has an `f32`
...
[ walrus is a WebAssembly to WebAssembly transformation library.](https://github.com/rustwasm/walrus) We use it to generate glue code in

`wasm-bindgen`

and to polyfill WebAssembly features.`walrus`
...
edge in the tree, how do we know *which* of the child’s resulting values the parent wants to use? And also, if two different parents are each using one of the two values an instruction generates, we fundamentally don’t have a tree anymore, we have a [directed, acyclic graph (DAG)](https://en.wikipedia.org/wiki/Directed_acyclic_graph).

We considered generalizing our tree representation into a DAG, and labeling edges with *n* to represent using the *n th* resulting value of an instruction. We weighed the complexity of implementing this representation against what our current use cases in
...
performs, or that we foresee it doing in the future.Instead, we decided that within a block, representing instructions as a simple list is good enough for our use cases, so now our IR looks something like this:
...
[ wasm-bindgen facilitates high-level interactions between Wasm modules and their host.](https://github.com/rustwasm/wasm-bindgen) Often that host is a Web browser and its DOM methods, or some user-written JavaScript. Other times it is an outside-the-Web Wasm runtime, like Wasmtime, using WASI and interface types.

`wasm-bindgen`

acts as a polyfill for the interface types proposal, plus some extra batteries included for a powerful user experience.One of `wasm-bindgen`
...
[#1764: Introduce a multi-value transform](https://github.com/rustwasm/wasm-bindgen/pull/1764)[#1805: Always use multi-value when targeting interface types](https://github.com/rustwasm/wasm-bindgen/pull/1805)[#1839: Update binding metadata after multi-value transform](https://github.com/rustwasm/wasm-bindgen/pull/1839)
...
Enter [Wasmtime](https://wasmtime.dev/), the WebAssembly runtime built on top of [the Cranelift code generator](https://github.com/bytecodealliance/cranelift). Wasmtime translates WebAssembly into Cranelift’s IR with the `cranelift-wasm`
...
converting its mid-level intermediate representation (MIR) to LLVM’s IR.[2](https://hacks.mozilla.org#foot-cranelift-backend-for-rustc)

Cranelift’s IR is made up of (extended) basic blocks 3 containing code in

[single, static-assignment form (SSA)](https://en.wikipedia.org/wiki/Static_single_assignment_form). SSA, as the name implies, means that variables can only be assigned to when defined, and can’t ever be re-assigned:
...
that was initially reachable, and whose consequent arm also ends reachable, but whose alternative arm ends *un*reachable. Given that, should the block following the consequent and alternative be reachable? Yes, but we were incorrectly computing that it shouldn’t be.
...
Finally, this bug first manifested itself in a 39 KiB Wasm file, and figuring out what was going on was made so much easier thanks to tools like [ wasm-reduce](https://github.com/WebAssembly/binaryen/blob/d2550891e41ad0215b1cae46fa711bc1e264166a/src/tools/wasm-reduce.cpp) (a tool that is part of

[binaryen](https://github.com/WebAssembly/binaryen)) and

[(working on the WAT disassembly, rather than the binary Wasm). I forget which one I used this time, but I’ve successfully used both to turn big, complicated Wasm test cases into small, isolated test cases that highlight the bug at hand. These tools are real life savers so it is worth broadcasting their existence just in case anyone doesn’t know about them!](http://embed.cs.utah.edu/creduce/)

`creduce`

Translating multi-value Wasm into Cranelift IR happened in these pull requests:

[#1049: Translate multi-value Wasm into Cranelift IR](https://github.com/CraneStation/cranelift/pull/1049)[#1110: Correctly jump to the destination block at the end of the consequent](https://github.com/CraneStation/cranelift/pull/1110)[#1143: Fix reachability tracking for](https://github.com/CraneStation/cranelift/pull/1143)`if`
...
are the registers assigned to the parameter and return value by the calling convention.[4](https://hacks.mozilla.org#foot-locations-and-intel-syntax)
...
[#1147: Support many multi-value returns with struct return pointers](https://github.com/bytecodealliance/cranelift/pull/1147)[#1213:](https://github.com/CraneStation/cranelift/pull/1213)`legalize_signatures`
...
We can build our Wasm library with [ cargo wasi](https://github.com/bytecodealliance/cargo-wasi/):
...
tool from [WABT](https://github.com/WebAssembly/wabt):
...
We can see that the `function is exported as `"hello"` and that it has the multi-value type `(i32, i32) -> (i32, i32)`. This shim function is indeed the one introduced by our multi-value transform we added to `wasm-bindgen` to wrap the original`
...
0 Additionally, [Thomas Lively](https://github.com/tlively) and some other folks are already working on adding multi-value Wasm support directly to LLVM, so that is definitely coming in the future, and it made sense for me to focus my attention elsewhere. [↩](https://hacks.mozilla.org#back-llvm-multi-value)
...
DealnsalesNovember 24th, 2019 at 04:15macdevignNovember 28th, 2019 at 06:41