---
title: Localization community, tools & process, part 2 of 3 – A Node.js holiday season,
  part 10 – Mozilla Hacks - the Web developer blog
url: https://hacks.mozilla.org/2013/04/localization-community-tools-process-part-2-of-3-a-node-js-holiday-season-part-10/
author: Austin King
...
This is episode 10, out of a total 12, in the

[A Node.JS Holiday Season series]from Mozilla’s Identity team. Let’s talk some more localization!
...
does a few different things for you:
...
All these strings end up in text files that end with the `.po`

file suffix. I’ll refer to these as **PO files**.

### PO Files

.PO files are plain text files with a specific format that the Gettext tools can read, write, and merge.

An example snippet of a PO file named zh*TW/LC*MESSAGES/messages.po:
...
We’ll examine this in more detail below, but we can see that `msgid`

is the English String and `msgstr`

has the Chinese translation. Comments are anything that start with a `#`
...
The Mozilla community already uses many tools such as [POEdit](http://www.poedit.net/), [Verbatim](https://localize.mozilla.org/), [Translate Toolkit](https://github.com/translate/translate), and [Pootle](https://github.com/translate/pootle). Instead of forcing new tools on the community, we decided to make our tools work within their standards.
...
So our code is marked up with `gettext`
...
and other [GNU Gettext tools](http://www.gnu.org/software/gettext/) tools are a powerful way to manage catalogs of Strings. Only the String wrangler needs these tools, most developers (as well as Node.js) can remain blissfully ignorant of them.
...
To extract strings into a `locale`

directory, we would use this command.
...
creates a .POT file, which is a PO file Template.

This script will recursively look through your source code and extract Strings.

So how does `extract-pot`

create these POT files? You can use traditional GNU Gettext utilities, but we’ve also written a Node module `jsxgettext`

, which is a nice cross platform way to go.
...
The Gettext tool `msginit`

is used to create an empty PO file for a target locale, based on our POT file.
...
You can give your localizers access to this part of your codebase. The Spanish team will need access to `locale/es/LC_MESSAGES/messages.po`

for example. If you have a really big team, you might have `es-ES`

for Spain’s Spanish and `es-AR`

for Argentinian Spanish, instead of just a base `es`

for all Spanish locales.
...
We provide a wrapper shell script called `merge-po.sh`

which uses GNU Gettext’s `msgmerge`
...
Just like the first time… `extract-pot`

grabs all the Strings and updates the POT file. Next `merge_po.sh`

updates each locale’s PO file to match our codebase. You can now ask your L10n teams to localize any new or changed Strings.
...
, you’ll have to write your own merge tools. This is because if you have 30 locales, you’ll need to update 30 JSON files without losing the work they’ve already done.
