...
This is a response to the Rust [call for blogs 2021](https://blog.rust-lang.org/2020/09/03/Planning-2021-Roadmap.html) and also a followup to [last year’s entry](https://raphlinus.github.io/rust/druid/2019/10/31/rust-2020.html). It will be entirely focused on GUI.

There is considerable interest in GUI toolkits for Rust. As one data point, it was the 6th highest rated challenge for adoption in the [2019 Rust survey](https://blog.rust-lang.org/2020/04/17/Rust-survey-2019.html#rust-adoption---a-closer-look), just behind async I/O. There is also a fair amount of activity towards this goal, though as a community it still feels a bit unfocused. A characteristic sign is that a new GUI toolkit seems to pop up every couple of months or so.
...
The [Druid](https://github.com/linebender/druid) toolkit has made impressive progress in the last year, but is still nowhere near stable or complete. If you are looking for a GUI toolkit to develop your application today, Druid is not it.

We are developing the font editor [Runebender](https://github.com/linebender/runebender) as the primary motivating application, but, while a lot of pieces are in place, it is sadly not yet usable for day to day font creation work. One of my goals for the rest of the year is to start creating a font in it.

That said, I am very proud of the work that’s been done in the last year. To hit on some of the highlights, we’re just landing basic but capable [rich text layout](https://www.cmyr.net/blog/piet-text-work.html). The keyboard event is close to browser quality (based on [keyboard-types](https://crates.io/crates/keyboard-types)). There is incremental painting based on damage regions. Multi-window support is solid, with support for controlling window placement and dynamic hi-dpi. There is tab-focusing between text boxes. All of these are hard problems. Even more so, I am pleased that a lot of the work came from people in the community.
