...
[coreaudio](https://github.com/RustAudio/coreaudio-rs)[alsa](https://github.com/diwic/alsa-rs)[winapi](https://github.com/retep998/winapi-rs)[stdweb](https://github.com/koute/stdweb)[midir](https://github.com/Boddlnagg/midir)[coremidi](https://github.com/chris-zen/coremidi)[alsa](https://github.com/diwic/alsa-rs)[winapi](https://github.com/retep998/winapi-rs)[winit](https://github.com/tomaka/winit)[core-graphics](https://github.com/servo/core-foundation-rs)[wayland](https://github.com/Smithay/wayland-rs)/[x11](https://github.com/erlepereira/x11-rs)[winapi](https://github.com/retep998/winapi-rs)[(in winit)](https://github.com/tomaka/winit/tree/master/src/platform/emscripten)[gfx-rs](https://github.com/gfx-rs/gfx)[metal](https://github.com/gfx-rs/metal-rs)[ash](https://github.com/MaikKlein/ash)[d3d12](https://github.com/gfx-rs/d3d12-rs)[rand](https://crates.io/crates/rand)[libc](https://github.com/rust-lang/libc)[libc](https://github.com/rust-lang/libc)[winapi](https://github.com/retep998/winapi-rs)[stdweb](https://github.com/koute/stdweb)The near-magical result of this pattern is that, very often, it’s possible to `git clone`

a project, run `cargo run`

, and have it “just work,” even if running on a different platform than it was developed for.
...
Given a cross-platform abstraction for 2D graphics, several promising projects, both existing and future, could potentially be a back-end. The gfx team is starting discussions on [draw2d](https://github.com/draw2d/rfcs/issues/1), which would sit on top of the gfx-rs 3d abstraction.

In addition, [WebRender](https://github.com/servo/webrender) has a good chunk of 2D graphics rendering functionality, though itself is missing general Bezier path rendering. Both [Pathfinder](https://github.com/pcwalton/pathfinder) and [lyon](https://github.com/nical/lyon) provide the needed path functionality, using different approaches to use 3D graphics hardware.

### Other back-ends

For maximum compatibility, I imagine [cairo](https://cairographics.org/) is the most useful back-end when a system-provided library is not available. However, cairo is mostly a software renderer, so performance will be quite poor compared with what GPU hardware can do. (For full disclosure, there are other back-ends, but harder to interface, and none using state of the art rendering techniques, so this is unlikely to be a major effort.)
...
I’m making good, steady progress with xi-win-ui. In addition to xi-win, I’m also building the GUI for my [synthesizer](https://github.com/raphlinus/synthesizer-io) using it, and plan to use it for the game UI and interaction when I get to that.

Right now, I’m using [direct2d](https://github.com/Connicpu/direct2d-rs), and it’s going well. That said, I’m not happy about the fact that this code is Windows-only. I’d like this 2D graphics abstraction crate to exist sooner rather than later, so I can port the code over.

## Design choices

As is being discussed on the [draw2d](https://github.com/draw2d/rfcs/issues/1) thread, there are a number of design choices. Personally, I’d like to see the API generally close to Direct2D, not only because that minimizes porting cost, but also because it’s a relatively modern, performant implementation.
...
One project to look at is [resvg](https://github.com/RazrFalcon/resvg), a pure-Rust SVG implementation. It currently has both a [cairo](https://cairographics.org/) and a [Qt](https://www.qt.io/) backend. It is probably worth looking at its implementation to see what kind of interface it uses for multiple backends. SVG is also quite a rich graphical model. If the new crate is successful, resvg could perhaps be adapted to use it.
...
[HarfBuzz](https://www.freedesktop.org/wiki/Software/HarfBuzz/)is the gold standard here). -
Rasterizing the glyphs (traditionally

[FreeType](https://www.freetype.org/), but[rusttype](https://github.com/redox-os/rusttype)and[font-rs](https://github.com/raphlinus/font-rs)are also viable). -
