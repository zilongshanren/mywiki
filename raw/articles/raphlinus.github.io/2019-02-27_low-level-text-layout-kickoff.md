...
This post is to announce a new Rust library for low-level text layout, called “[skribo](https://github.com/linebender/skribo)” (the Esperanto word for “writing”). This has been a major gap in the Rust ecosystem, and I hope the new crate can improve text handling across the board.

## Multiple shaping engines

The skribo library doesn’t do shaping itself; it is basically a glue library on top of an actual shaping engine. Most users will use [HarfBuzz](https://www.freedesktop.org/wiki/Software/HarfBuzz/) for this, the industry-standard library. However, a goal for skribo is that it abstracts over the details of the shaping engine. Other choices include platform text (DirectWrite on Windows, CoreText on macOS and iOS). Also, if and when a Rust-native shaping library emerges, we want to make it easy to switch to that.
...
![CJK shaping example](../../assets/02214c5b27854f40.png)


## Expected users

This work is funded by Mozilla Research to be used in Servo. Doing text layout for the Web is a very complex problem. The higher level representation of rich text is very tightly bound to the DOM and CSS. All that logic needs to live in Servo, with the lower level providing a clean interface to lay out a span of text with a *single* style and a single [BiDi](https://en.wikipedia.org/wiki/Bi-directional_text) direction.

The second major use case is [piet](https://github.com/linebender/piet), the 2D graphics abstraction I’ve been working on. Currently for the Cairo back-end piet uses the “toy text API,” which is of course [inadequate](https://github.com/linebender/piet/issues/10). The idea is that skribo handles text layout for Cairo and all future low-level drawing back-ends (including any future back-end using WebRender or direct Vulkan drawing). A requirement for both Web layout and text editing within GUI apps (including potential front-end work for [xi-editor](https://xi-editor.io/)) is fine-grained measurement of text, including positioning of carets within text as well as width measurement of spans of text.

Additionally, just about every game engine currently being implemented in Rust uses a simplistic approach to text layout, just looking up each codepoint in the font’s cmap and then using its advance width to build a layout. I want to encourage all such engines to migrate to skribo, and want to make that experience smooth. In short, if you’re using [rusttype](https://github.com/redox-os/rusttype), you should consider using skribo.
...
![CJK shaping example](../../assets/7b4528c391dcf72e.png)


Other problems within scope are figuring out logic for “fake bold” and “fake italic” when the font collection doesn’t provide true versions, as well as adding [letter-spacing](https://developer.mozilla.org/en-US/docs/Web/CSS/letter-spacing).

Looking forward, [font variations](https://docs.microsoft.com/en-us/typography/opentype/spec/otvaroverview) affect layout, so one of the goals is to plumb this through to the underlying shaping engine, where supported.

## Performance

Unfortunately, OpenType shaping is a fairly expensive operation. A lot of optimization has gone into HarfBuzz, but it’s still potentially a major contribution to overall layout time. The Web has struggled with this. For a long time, Chrome had both a “simple path” and a “complex path”, where shaping was disabled in the simple path. This degrades the experience and exposes this detail to users. So a few years ago Chrome implemented a word cache to accelerate the complex path, making it suitable for all use, and the simple path was [removed](https://www.chromium.org/teams/layout-team/eliminating-simple-text) in October 2016. All this is described in a [Unicode presentation on Chrome text](http://www.unicodeconference.org/presentations/S5T2-R%C3%B6ttsches-Esfahbod.pdf) (PDF).
...
The scope of skribo is similar to [pango](https://www.pango.org/), which is well-established in the free software world. However, its implementation will hew even more closely to [Minikin](https://android.googlesource.com/platform/frameworks/minikin/), the low-level text layout library in Android. I’m biased of course, having written the first version of minikin, but I think there are many good reasons to use it as a basis, and am not alone; the [libtxt](https://github.com/flutter/flutter/issues/11092) layout engine used by Flutter is also based on a fork of Minikin.
...
I’ll be putting together a requirements document, then a design document. There’s a bit more detail on the [project roadmap](https://drive.google.com/open?id=1aw41q_izail-p99mN8dHrJeh9tMQ-Pldi54W6m7MHU8). I really want feedback on both of those, especially from potential users of the library. Then, over the next few weeks I will be intensively prototyping and refining the design.
