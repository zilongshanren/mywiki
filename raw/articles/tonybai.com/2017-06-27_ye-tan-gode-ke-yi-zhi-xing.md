---
title: 也谈Go的可移植性
url: https://tonybai.com/2017/06/27/an-intro-about-go-portability/
...
[Go](http://tonybai.com/tag/go)有很多优点，比如：[简单](http://tonybai.com/2017/04/20/go-coding-in-go-way/)、[原生支持并发](http://tonybai.com/2017/06/23/an-intro-about-goroutine-scheduler/)等，而不错的[可移植性](https://en.wikipedia.org/wiki/Software_portability)也是Go被广大程序员接纳的重要因素之一。但你知道为什么Go语言拥有很好的平台可移植性吗？本着“知其然，亦要知其所以然”的精神，本文我们就来探究一下Go良好可移植性背后的原理。
...
在[Go 1.7](http://tonybai.com/2016/06/21/some-changes-in-go-1-7/)及以后版本中，我们可以通过下面命令查看Go支持OS和平台列表：
...
从上述列表我们可以看出：从**linux/arm64**的嵌入式系统到**linux/s390x**的大型机系统，再到Windows、[linux](http://tonybai.com/tag/ubuntu)和darwin(mac)这样的主流操作系统、amd64、386这样的主流处理器体系，Go对各种平台和操作系统的支持不可谓不广泛。
...
![img{512x368}](../../assets/5d8096aa0f4ac568.png)


runtime是支撑程序运行的基础。我们最熟悉的莫过于libc（C运行时），它是目前主流操作系统上应用最普遍的运行时，通常以[动态链接库](http://tonybai.com/2010/12/13/also-talk-about-shared-library/)的形式(比如：/lib/x86_64-linux-gnu/libc.so.6)随着系统一并发布，它的功能大致有如下几个：

- 提供基础库函数调用，比如：
[strncpy](http://tonybai.com/2009/04/15/glibc-strncpy-source-analysis/)； - 封装syscall（注:syscall是操作系统提供的API口，当用户层进行系统调用时，代码会trap(陷入)到内核层面执行），并提供同语言的库函数调用，比如：malloc、fread等；
