...
url: https://tonybai.com/2014/11/07/golang-development-environment-for-vim/
...
虽说[sublimetext3](http://www.sublimetext.com/3)+[gosublime](https://github.com/DisposaBoy/GoSublime)+[gocode](https://github.com/nsf/gocode)是目前较为 流行的[Golang](http://tonybai.com/tag/go)开发环境组合，但作为一名VIMer，没有一套得心应手的[Vim](http://www.vim.org/) for Golang dev心里总是过不去的。Golang虽然年轻，但即便是从[Go 1版本发布](http://tonybai.com/2014/10/25/golang-history/)(2012年3月28日)算起，掐指算来也有小三年了。全世界的开发者已经为Golang贡献了较为成熟的Vim插件了。有了这些插件，搭建出 一套高效的Golang开发环境还是不难的，网上也有大量的资料可以参考，其中就有[vim-go](https://github.com/fatih/vim-go)作者自己发表的一篇文章《[Go development environment for Vim](http://blog.gopheracademy.com/vimgo-development-environment/)》。不过看别人 写的与自己搭建体验的还是有大不同的，于是想来想去还是把整个过程记录下来。

**一、一个干净的环境**

找个干净的基础环境，方便确认每个搭建步骤后的效果：

[Ubuntu 14.04](http://tonybai.com/2014/09/26/install-docker-on-ubuntu-server-1404/) x86_64
...
在Vim内执行 ![:P](../../assets/914c8d0637b6df78.gif)
...
此时，再次编辑hellogolang.go，__ 语法高亮__有了，

__（利用$GOBIN/gofmt）也有了，但其他高级功能，比如自动import缺失的 package、自动补齐仍然没有，我们还要继续安装一些东东。__

**保存时自动format****3、安装go.tools Binaries**
...
**1、安装 YCM(Your Complete Me)**

在~/.vimrc中添加一行：

Plugin 'Valloric/YouCompleteMe'

保存退出后，再打开~/.vimrc并执行 ![:P](../../assets/914c8d0637b6df78.gif)
