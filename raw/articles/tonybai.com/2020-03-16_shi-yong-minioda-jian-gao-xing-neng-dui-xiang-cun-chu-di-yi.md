---
title: 使用minio搭建高性能对象存储-第一部分：原型
url: https://tonybai.com/2020/03/16/build-high-performance-object-storage-with-minio-part1-prototype/
...
近期参与了一个项目，该项目有存储大量图片、短视频、音频等[非结构化数据](https://www.techrepublic.com/article/unstructured-data-the-smart-persons-guide/)的需求。于是我优先在[Go社区](https://tonybai.com/tag/go)寻找能满足这类需求的开源项目，[minio](https://github.com/minio/minio)就这样进入了我的视野。

![img{512x368}](../../assets/a016e87dc2bfbd5c.png)


其实三年前我就知道了minio，并还下载玩(研)耍(究)了一番，但那时minio的成熟程度与今天相比还是相差较远的(当时需求简单，于是选择了较为熟悉的[weedfs](https://tonybai.com/2015/08/22/intro-of-using-weedfs/))。而如今的minio在github上收获了广泛的关注，小星星也是蛮多的(20k+ star)。它不仅被Go社区使用，在其他语言社区也有着广泛应用。我可以**不负责任**的说：在对象存储领域，minio大有[kafka(java技术栈)](https://kafka.apache.org/)在消息队列领域舍我其谁的气概:)。

2019年[gopherchina大会](https://gopherchina.org/)上，探探工程师分享了[“基于MINIO的对象存储方案在探探的实践”](https://github.com/gopherchina/conference/blob/master/2019/2.1%20基于MINIO的对象存储方案在探探的实践%20-%20于乐.pdf)。虽然探探目前是否在生产中使用minio暂不得而知，但这又一次证明了minio在对象存储领域的强大影响力。

![img{512x368}](../../assets/fd424ac22c9c6e7c.png)


minio出品自一个有着多年网络文件系统开发经验的团队，其初始创始团队都来自于原[Glusterfs团队](https://www.gluster.org/)，该团队二次创业的产品minio的设计广泛吸取了glusterfs的经验和教训：
...
![img{512x368}](../../assets/bd25aeee9dbdc4f0.png)
...
![img{512x368}](../../assets/12bf1001512b6e20.png)


-
我们基于minio提供的

[distributed mode](https://docs.minio.io/docs/distributed-minio-quickstart-guide.html)，将位于多个host上的多块磁盘组成一个逻辑存储池，通过运行于不同host上的minio server实现一个高可用的对象存储方案； -
数据通过一个独立的上传服务(基于minio提供的sdk与minio集群通信)写入minio；

-
通过minio的

[mc工具](https://github.com/minio/mc)创建bucket，并将bucket的policy设置为”download”，以允许外部用户直接与minio通信，获取对象数据。中间不再设置除lb之外的中间层； -
...
![img{512x368}](../../assets/be6145c44deb4d37.png)
...
同样在单minio server的情况下，**erasure code mode**即为minio server实例传入多个本地磁盘参数。一旦遇到多于一个磁盘参数，minio server会自动启用**erasure code mode**。**erasure code**对磁盘的个数是有要求的，如不满足要求，实例启动将失败：
...
等价于`minio server data1 data2 data3 data4 data5 data6 data7 data8 data9 data10 data11 data 12 data13 data14 data15 data16 data17 data18`
...
minio同样会自动将这些disk drive划分为若干个**erasure coding set**。每个endpoint用`http://address/disk-drive-path`
...
![img{512x368}](../../assets/f6197ee149dca5ef.png)
...
我们使用浏览器访问一下`http://myminio.tonybai.com:9000/`

，登录后，你将看到如下页面：

![img{512x368}](../../assets/8b157bc1c125ede7.png)


选择左侧的”image” bucket，点击右下角的”+”号，我们可以上传一张图片：gopher-daily-logo.png，上传后，我们退出登录。然后通过地址`http://myminio.tonybai.com:9000/image/gopher-daily-logo.png`
...
一致都很担心这个东西的安全性: 管理端口和外部访问端口 (9000) 是同一个, 没有白名单或者别的防护.

不知有何解决方法.

这些端口都是可以自定义的啊，通过配置就可以改为其他端口号。

@bigwhite 翻了下文档 `端口` 确实可以配置: 统一更换, 但是不能单独设置 `管理接口端口` 的访问权限 (例如: 限制IP白名单).

因为 `管理端口` 统一在 `/minio/admin` 路径下, 可以在前端 `nginx 代理` 的时候设置防护.
