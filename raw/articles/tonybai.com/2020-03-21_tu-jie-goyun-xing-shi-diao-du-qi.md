---
title: 图解Go运行时调度器
url: https://tonybai.com/2020/03/21/illustrated-tales-of-go-runtime-scheduler/
...
本文翻译自[《Illustrated Tales of Go Runtime Scheduler》](https://medium.com/@ankur_anand/illustrated-tales-of-go-runtime-scheduler-74809ef6d19b)。

译注：原文章结构有些乱，笔者自行在译文中增加了一些分级标题，让结构显得更清晰一些:)。


多[goroutines](https://tonybai.com/2017/11/23/the-simple-analysis-of-goroutine-schedule-examples/)形式的[Go并发](https://tonybai.com/2015/06/23/concurrency-and-parallelism/)是编写现代并发软件的一种非常方便的方法，但是您的[Go](https://tonybai.com/tag/go)程序是如何高效地运行这些goroutines的呢？

在这篇文章中，我们将深入Go运行时底层，从设计角度了解Go运行时调度程序是如何实现其魔法的，并运用这些原理去解释在Go性能调试过程中产生的[Go调度程序跟踪信息](https://tonybai.com/2019/04/04/notes-about-fixing-a-go-panic-problem/)。

**所有的工程奇迹都源于需要**。因此，要了解**为什么需要一个Go运行时调度程序**以及**它是如何工作的**，我们可以让时间回到操作系统兴起的那个时代，回顾操作系统的历史可以使我们深入的了解问题的根源。如果不了解问题的根源，就没有解决它的希望。这就是历史所能做的。
...
- 这是一种多道程序设计，可以在作业之间快速切换。决定何时切换以及切换到哪个作业的过程就称为
**调度(scheduling)**。
...
![img{512x368}](../../assets/5c3e149ba8c61abb.png)



因此，使用一个包含多个线程的进程的效率更高，因为进程创建既耗时又耗费资源。但是随后出现了多线程问题:[C10k](https://en.wikipedia.org/wiki/C10k_problem)成为主要问题。

例如，如果**将调度周期定为10ms（毫秒）**，并且有2个线程，则每个线程将分别获得5ms。如果您有5个线程，则每个线程将获得2ms。但是，如果有1000个线程怎么办？给每个线程一个10μs（微秒）的时间片？错，这样做很愚蠢，因为您将花费大量时间进行上下文切换，但是真正要完成的工作却进展缓慢或停滞不前。

您需要限制时间片的长度。在最后一种情况下，如果最小时间片为2ms并且有1000个线程，则调度周期需要增加到2s（1000*2ms）。如果有10,000个线程，则调度程序周期为20秒(10000*2ms)。在这个简单的示例中，如果每个线程都将分配给它的时间片用完，那么所有线程都完成一次运行需要20秒。因此，我们需要一些可以使并发成本降低而又不会造成过多开销的东西。
...
![img{512x368}](../../assets/e34926df1df05beb.png)



goroutine是由Go运行时管理的轻量级线程（lightweight thread）。要启动一个新的goroutine，只需在函数前面使用`go`
...
![img{512x368}](../../assets/dfd38251a8c00e3f.png)
...
- 应该是并行、可扩展且公平的。
- 每个进程应可扩展到数百万个goroutine（
[C10M](http://c10m.robertgraham.com/p/manifesto.html)） - 内存利用率高。（RAM很便宜，但不是免费的。）
...
![img{512x368}](../../assets/27098f6e4d48d2f2.png)
...
![img{512x368}](../../assets/4742a8f944c213f2.png)
...
![img{512x368}](../../assets/e1615dc8c1b26169.png)
...
![img{512x368}](../../assets/3ef3014d59923882.png)
...
![img{512x368}](../../assets/0e994f4f31d4b2a6.png)
...
![img{512x368}](../../assets/2540983f0adfb84e.png)
...
非阻塞系统调用-将goroutine阻塞在[Integrated runtime poller](https://morsmachine.dk/netpoller)上 ，并释放线程以运行另一个goroutine。

![img{512x368}](../../assets/d11c8773f574525f.png)
...
![img{512x368}](../../assets/83736214b886eb83.png)
...
![img{512x368}](../../assets/e5b06eb2a345d656.png)
...
![img{512x368}](../../assets/791da90ebae7f888.png)
...
![img{512x368}](../../assets/69aac217055daac9.png)
...
![img{512x368}](../../assets/1a3a92451292a5a6.png)
...
![img{512x368}](../../assets/5d2a3028e7983f50.png)
...
![img{512x368}](../../assets/51fd414b276bc871.png)
...
![img{512x368}](../../assets/3657604964a21326.png)
...
但请小心无限循环，因为Go的调度程序不是抢先的（直到[Go 1.13](https://tonybai.com/2019/10/27/some-changes-in-go-1-13/)）。如果循环不包含任何抢占点（例如函数调用或分配内存），则它们将阻止其他goroutine的运行。一个简单的例子是:
...
使用`GODEBUG=schedtrace=DURATION`

环境变量运行Go程序以启用调度程序跟踪。（DURATION是以毫秒为单位的输出周期。）

![img{512x368}](../../assets/84621e6dbca0ae6c.png)



有关调度器跟踪的内容，[Go Wiki](https://github.com/golang/go/wiki/Performance#scheduler-trace)拥有更多信息。
...
首先，你可以在本地跑一下这个代码，你会发现 输出顺序跟demo不一样，是乱序的，因为你的机器是多核的，也就意味着有多个p，作者或者golang.org里面是单核的只有一个p，在本地代码中添加一个runtime.GOMAXPROCS(1) 你就会的到同样的结果了。
...
所有goroutine都在等待ffmpeg命令执行完毕才会继续，你top看到的负荷数值都是多个ffmpeg工作时的负荷。