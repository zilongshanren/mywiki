---
title: Golang测试技术
url: https://tonybai.com/2014/10/22/golang-testing-techniques/
...
本篇文章内容来源于[Golang](http://golang.org)核心开发组成员[Andrew Gerrand](http://nf.wh3rd.net/)在Google I/O 2014的一次主题分享“[Testing Techniques](https://talks.golang.org/2014/testing.slide#1)”，即介绍使用Golang开发 时会使用到的测试技术（主要针对[ 单元测试](http://tonybai.com/2005/11/08/the-design-and-implementation-of-c-unittest-framework/)），包括基本技术、高级技术（并发测试、

[mock](http://tonybai.com/2010/10/29/lcut-add-mock-support/)/fake、竞争条件测试、并发测试、内/外部测 试、vet工具等）等，感觉总结的很全面，这里整理记录下来，希望能给大家带来帮助。原Slide访问需要自己搭梯子。另外这里也要吐槽一 下：Golang官方站的slide都是以一种特有的

[golang artical](http://godoc.org/code.google.com/p/go.tools/present)的格式放出的（用这个工具http://go-talks.appspot.com/可以在线观看），没法像pdf那样下载，在国内使用和传播极其不便。
...
t.Errorf("Index(%q,%q) = %v; want %v", s, sep, got, want)//__注意原slide中____的got和want写反了__
...
我们可以使用Go的并发原语让那些奇怪不靠谱的sleep驱动的测试更加值得信赖。并发原语是什么意思呀？

我的理解是在设计并发测试时，以往我们常用sleep在两个或多个线程or goroutine之间进行“测试交互”，这里Andrew建议搭建使用go自身提供的并发原语，比如channel。

我的理解是在设计并发测试时，以往我们常用sleep在两个或多个线程or goroutine之间进行“测试交互”，这里Andrew建议搭建使用go自身提供的并发原语，比如channel。

感谢分享,之前http测试我还是自己写的http测试包用来给项目做api接口测试,也没去看httptest这个包,原来自带了都-_-||

请问最后一个测试为什么看不到

Crasher里的fmt的结果Going down in flames!

这个测试比较特殊，它本身被执行两次，一次是go test执行它，另外一个是在其内部cmd形式将go test编译出来的pkg.test在shell里重新执行一次。而你所说的“Going down in flames!”是在第二次>输出的，但是这个输出在 go test中是看不到的，除非将cmd.Run改为：
...
ok _/Users/tony/Test/Go/advtest 0.027s

请问下：起一个HTTP Server 之后，通过外部程序，如postman 、python写的测试case，调用server的接口，怎么收集这些case所覆盖的代码？

可以参考一下 这篇文章中提到的方法： https://blog.cloudflare.com/go-coverage-with-external-tests/