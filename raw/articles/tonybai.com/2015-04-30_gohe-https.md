---
title: Go和HTTPS
url: https://tonybai.com/2015/04/30/go-and-https/
...
近期在构思一个产品，考虑到安全性的原因，可能需要使用到[HTTPS](http://en.wikipedia.org/wiki/HTTPS)协议以及双向数字证书校验。之前只是粗浅接触过HTTP（[使用Golang开 发微信系列](http://tonybai.com/2014/12/18/access-validation-for-wechat-public-platform-dev-in-golang/)）。对HTTPS的了解则始于那次[自行搭建ngrok服务](http://tonybai.com/2015/03/14/selfhost-ngrok-service/)，在那个过程中照猫画虎地为服务端生成了一些私钥和证书，虽然结果是好 的：ngrok服务成功搭建起来了，但对HTTPS、数字证书等的基本原理并未求甚解。于是想趁这次的机会，对HTTPS做一些深度挖掘。主要途 径：翻阅网上资料、书籍，并利用golang编写一些实验examples。
...
打开浏览器，在地址栏输入"[http://localhost:8080](http://localhost:8080)"， 你会看到“ Hi, This is an example of http service in golang!"输出到浏览器窗口。
...
1、《[HTTP权威指南](http://book.douban.com/subject/10746113/)》第十四章

2、《[图解HTTP](http://book.douban.com/subject/25863515/)》第七章

3、阮一峰老师的两篇博文“[SSL/TLS协议运行机制的概述](http://www.ruanyifeng.com/blog/2014/02 /ssl_tls.html)"和"[图解SSL/TLS协议](http://http://www.ruanyifeng.com/blog/2014/09/illustration-ssl.html)"。
...
这让我想起了在《[搭建自己的ngrok服务](http://tonybai.com/2015/03/14/selfhost-ngrok-service/)》一文中为ngrok服务端、客户端生成证书的那几个步骤，我们来重温并分析一下每一步都在做什么。
...
通过上面的例子可以看出，使用golang开发https相关程序十分便利，Golang标准库已经实现了TLS 1.2版本协议。上述所有example代码均放在我的github上的[experiments/gohttps](https://github.com/bigwhite/experiments/tree/master/gohttps)中。

© 2015, [bigwhite](https://tonybai.com). 版权所有.

Related posts:

要验证对方的证书是否真实，自己需要存储一些常用CA证书(ca.crt)，就像浏览器一样，自带常见CA的证书；要证明自己的身份，就需要出示自己的由CA签发的证书与公钥，服务器端是(server.crt, server.key)，客户端就是(client.crt, client.key)。不知道有没有理解错误。

上述所有example代码均放在我的github上的experiments/gohttps中 的 github 链接错了
...
我实现了你的代码，可是抓包分析中并不能抓到SSL的包，这个client和Server真的建立了SSL连接吗？

呵呵。这还有假，golang标准库提供的方法，建立的应该是tls1.2的链接。

可是抓不到SSl握手过程，好奇怪，我也觉得程序就应该是这么写的

您好，我想问下，我现在自建ngrok的服务器想用https的形式访问，但是chrome浏览器拒绝访问。是不是我需要把ngrok里assets/client/tls/ngrokroot.crt 这个文件下到本地然后让浏览器新任这个证书，然后启动ngrok的时候在分别指定tlsCrt和tlsKey为assets/server/tls/snakeoil.crt和assets/server/tls/snakeoil.key ？

是你的浏览器不信任你的证书CA，你必须到浏览器合作的机构申请，它们信任的证书，替换你的自己生成的

有没有具体的步骤呢..关于ngrok的步骤,道理我大概懂 就是找不到具体该怎么做

https://loear.com/archives/391 可以看下我的博客

您好，沃通的证书我也申请过了，但是现在的问题是，我把证书已经加到我本地的iis里，通过https://localhost测试访问正常，但是如果用ngrok做一个映射比如:：https://test1.ngrok.testngrok.win:8000到https://localhost ，然后在浏览器里访问https://test1.ngrok.testngrok.win:8000 ，浏览器还是拒绝的。我现在想知道的是如果我的证书的域名是www.testngrok.win那么这个域名下的二级，三级域名是不是受浏览器信任呢？
...
请教一下，双向验证中，服务器的证书 server.cert 和 客户端的证书 client.crt 都是由 ca.crt 签发的，那这两边用的 ca.crt 必须要使用同一个么，还是说各自用各自的就可以？
...
你理解的没错，可以不同。我们的pc里，无论是mac还是windows都内置了大量可信CA组织的ca.crt。当我们访问https站点时，只要这个站点的证书的签发CA在我们的pc中存在即可验证通过。

谢谢！

谢谢作者的细心总结，最近就在搞golang https开发，这篇文章给了我很大的帮助。再次感谢。

您好，请问实现了最终代码后用Wireshark抓包只有TCP包是怎么回事呢？

tcp层上面的数据没有形成ssl层么？

写的很好，可操作，谢谢

请问一下，双向认证中，能给 client 端带上用户的唯一标识，server 端可以通过 client 请求体解析出证书中的该标识吗？

从证书的原理来看，证书是公钥+申请者身份信息的，可以从client端发送的公钥证书中提取身份信息。或者在建立安全通道后，在通道内直接传递身份验证数据。
...
实测用wireshark抓包确实 客户端发送的证书Certificates 没有内容

应该是我的问题，我把wireshark抓的包导出看到证书是对的，后来发现是服务端的问题

嗯，找到原因就好。

我有个疑问就是，现在很多抓包软件支持https 抓包，但是要安装证书，这个证书是不是就是上面生成的ca.crt , 当抓包软件抓不同的域名的https ，会生成不通域名的证书，这个过程是这样的吗？

抓https的原理是利用“中间人”攻击吧。即将抓包工具的ca.crt手工加入到浏览器端，这样浏览器就可以通过该ca.crt对“抓包工具”冒充的“中间人”予以信任。从而与抓包工具建立连接，抓包工具再与真正的服务端建连，并转发浏览器客户端的请求，转发服务端应答给浏览器，这期间抓包工具拥有两段的对称密钥，可以看到所有通信数据的明文。