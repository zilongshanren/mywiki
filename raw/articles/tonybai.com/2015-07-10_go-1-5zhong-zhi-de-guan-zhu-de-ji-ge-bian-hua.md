---
title: Go 1.5中值得关注的几个变化
url: https://tonybai.com/2015/07/10/some-changes-in-go-1-5/
...
在[GopherCon2015](http://www.gophercon.com/)开幕之 际，Google Go Team终于放出了Go 1.5Beta1版本的安装包。在go 1.5Beta1的发布说明中，Go Team也诚恳地承认Go 1.5将打破之前6个月一个版本的发布周期，这是因为Go 1.5变动太大，需要更多时间来准备这次发布（fix bug, Write doc）。关于Go 1.5的变化，之前Go Team staff在各种golang技术会议的[slide](https://talks.golang.org /2015/gogo.slide) 中暴露不少，包括：
...
go 1.5依旧兼容[Go 1 language specification](http://golang.org/ref/spec)，但修正了之前的一个“小疏忽”。

[Go 1.4](http://tonybai.com/2014/11/04/some-changes-in-go-1-4/)及之前版本中，我们只能这么来写代码：
...
1.5之前的版本要想实现[跨平台编译](http://tonybai.com/2014/10/20/cross-compilation-with-golang)，需要到$GOROOT/src下重新执行一遍make.bash，执行前设置好目标环境的环境变量(GOOS和 GOARCH)，Go 1.5大大简化这个过程，使得跨平台编译几乎与普通编译一样简单。下面是一个简单的例子：
...
至于Go 1.5的性能，官方的说法是，有的程序用1.5编译后会变得慢点，有的会快些。官方bench的结果是总体比1.4快上一些。但Go 1.5在性能方面主要是为了[减少gc延迟](https://sourcegraph.com/blog/live/gophercon2015/123574706480)，后续版本才会在性能上做进一步优化，优化空间还较大的，这次runtime、编译器由c变go，很多地方的go 代码并非是最优的，多是自动翻译，相信经过Go team的优化后，更idiomatic的Go code会让Go整体性能更为优异。
...
1.5之前的我没用过，1.5开始build确实比较慢

所说的动态库，标准说法是dynamic shared library，动态共享库，就是我们在linux上经常看到的.so。简称动态库，呵呵。还有另外一种就是static shared library，就是我们常见的.a文件。

不知道golang 1.8的plugin模式和go 动态库的主要区别是什么