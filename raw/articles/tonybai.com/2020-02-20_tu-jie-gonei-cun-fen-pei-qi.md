---
title: 图解Go内存分配器
url: https://tonybai.com/2020/02/20/a-visual-guide-to-golang-memory-allocator-from-ground-up/
...
当我刚开始尝试了解[Go](https://tonybai.com/tag/go)的内存分配器时，我发现这真是一件可以令人发疯的事情，因为所有事情似乎都像一个神秘的黑盒(让我无从下手)。由于几乎所有技术魔法都隐藏在抽象之下，因此您需要逐一剥离这些抽象层才能理解它们。
...
![img{512x368}](../../assets/a8d858af5bcda192.png)
...
![img{512x368}](../../assets/fa9ffb216158de17.png)
...
![img{512x368}](../../assets/df5ca6f386763d83.png)



- DRAM中的每个“字节(BYTE)”都被分配有唯一的
**数字标识符（地址）**。 但“物理字节的表示 != 地址线的数量”。（例如：16位Intel 8088，PAE） - 每条“地址线”都可以发送1bit值，因此它可以表示给定字节地址中指定“bit”。
...
![img{512x368}](../../assets/f791682840cf1452.jpeg)
...
![img{512x368}](../../assets/654a01b59beb8a1a.png)
...
![img{512x368}](../../assets/f2daa4454f157045.png)



![img{512x368}](../../assets/347c0d79f3364fb0.png)



程序通过`brk（sbrk/mmap等）`

系统调用请求更多内存。但内核实际上仅是更新了堆的VMA。
...
扩大堆，通常会请求大量内存。（默认情况下，对于malloc而言，大量的意思是 > MMAP_THRESHOLD字节-128kB）。

但是，内存分配器的责任不仅仅是更新`brk地址`

。其中一个主要的工作则是如何的降低内外部的内存碎片以及如何快速分配内存块。考虑按p1~p4的顺序，先使用函数`malloc`

在程序中请求连续内存块，然后使用函数`free(pointer)`

释放内存。

![img{512x368}](../../assets/64da4c89c559b556.png)
...
![img{512x368}](../../assets/cf08793842897927.png)



### 页堆(page heap)

TCMalloc管理的堆由页集合组成，其中一组连续页的集合可以用`span`

表示。当分配的对象大于32K时，将使用页堆进行分配。

![img{512x368}](../../assets/39e5f791c0ae0149.png)
...
![img{512x368}](../../assets/c62edf4827ea1d96.png)



Go默认采用8192B大小的页。如果这个页被分成大小为1KB的块，我们一共将拿到8块这样的页:

![img{512x368}](../../assets/ad6cd8b1caa2c09f.png)
...
![img{512x368}](../../assets/790df2b0a7e6e80c.png)
...
![img{512x368}](../../assets/23f1b6308ef6d816.png)
...
- 空的mspanList — 没有空闲内存的mspan或缓存在mcache中的mspan的列表
- 非空mspanList – 仍有空闲内存的span列表。
...
![img{512x368}](../../assets/315ca96acc6d310b.png)
...
![img{512x368}](../../assets/cc2ff67240e5ef7e.png)
...
padding确保将MCentrals以`CacheLineSize`

字节间隔开，以便每个MCentral.lock获得自己的缓存行，以避免错误的共享问题。
...
![img{512x368}](../../assets/39efe495467624cf.png)



因此，即使是简单的go程序，占用的虚拟空间也是大约100MB而RSS只有696kB。让我们尝试首先找出这种差异的原因。

![img{512x368}](../../assets/ebc27ca5a89a5f60.png)
...
![img{512x368}](../../assets/83a69ac1192997b0.png)
...
![img{512x368}](../../assets/07894350adf70741.png)
...
![img{512x368}](../../assets/4f82c2f5eda185d3.png)
