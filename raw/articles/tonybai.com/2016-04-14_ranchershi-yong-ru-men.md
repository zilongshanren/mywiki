---
title: Rancher使用入门
url: https://tonybai.com/2016/04/14/an-introduction-about-rancher/
...
上个月末，[Rancher Labs](http://rancher.com/)在其官方博客上宣布了 [Rancher 1.0正式版本发布](http://rancher.com/announcing-rancher-1-0-ga/)。 这是继[Apache Mesos](http://mesos.apache.org/)、 [Google Kubernetes](http://kubernetes.io/)以及[Docker](https://www.docker.com/) 原生 [Swarm](https://github.com/docker/swarm) 之后，又一个可用于Production环境中的容器管理和服务编排工具，而Rancher恰似这个领域的最后一张拼图（请原谅我的孤陋寡闻，如 果有其他 厂商在做这方面产品，请在评论中留言告诉我）。从Rancher Labs的官方about中我们可以看到：Rancher Labs致力于为[DevOps](https://en.wikipedia.org/wiki/DevOps) team打造一个最好的容器管理平台，让容器的部署和管理变得更加Easy。

本文将带大家与Rancher来个亲密接触，直观的体会一下Rancher的入门级使用方法。

注意：由于Rancher还在active development中，本文仅适用于刚刚发布的[v1.0.0](https://github.com/rancher/rancher/releases/tag/v1.0.0)版本，包括：
...
Rancher的各种容器管理理念均架构在由Rancher server和rancher agent构建的Infrastructure之上。Rancher server是Rancher的核心，其地位就类似于k8s、Docker swarm或mesos中的master，提供核心容器管理服务以及API服务。作为正式版发布的Rancher v1.0.0支持HA(high available)的[多节点rancher server](http://docs.rancher.com/rancher/installing-rancher/installing-server/multi-nodes/)集群，不过Install起来也的确复杂些，依赖的第三方组件也较多，什么[MySQL](http://www.mysql.com/)、[Redis](http://redis.io/)、[ZooKeeper](http://zookeeper.apache.org/)等统统都要额外部署。由于是入门，这里就偷个赖儿，我们就搭建一个单节点的Rancher Server。
...
![img{512x368}](../../assets/73bfea25ab9344f0.png)
...
可以看出里面有mysql、cattle、go-machine-service、rancher-compose-executor以及 websocket-proxy等。通过PID我们可以看出/usr/bin/s6-svscan是容器的第一个启动进程，/service这个 路径作为其命令行参数，估计这是一个类似于[supervisord](http://supervisord.org/)的进程控制程 序，由其 负责启动和管理Rancher server的两个重要服务：MySQL和cattle。注：单节点rancher server的数据都保存在其内部的MySQL中，而多节点rancher server则采用一个外部的MySQL存储数据。
...
![img{512x368}](../../assets/f2469abd1aef8aa2.png)
...
![img{512x368}](../../assets/a359fd01bb333c2d.png)
...
![img{512x368}](../../assets/3506b2e453baeb14.png)
...
![img{512x368}](../../assets/3be9d6c12ed9de68.png)
...
![img{512x368}](../../assets/9b440cb4dd7762a0.png)
...
![img{512x368}](../../assets/4a254a08864ac767.png)
...
我们来为dev environment添加两个主机资源：10.10.126.101和10.10.105.72。在”INFRASTRUCTURE”-> HOSTS中点击”Add Host”按钮添加主机资源。Rancher支持多种主机资源，包括Custom（本地自定义）、[Amazon EC2](http://aws.amazon.com/cn/ec2/)、 [Azure](http://azure.com/) 以及 [DigitalOcean](https://www.digitalocean.com/) 等。
...
![img{512x368}](../../assets/fd9dbfc19c0f2b2b.png)
...
![img{512x368}](../../assets/7bef30aef042e304.png)
...
![img{512x368}](../../assets/c9d0b09f051ce0f2.png)


点击stack名字，可以查看stack的详细信息：

![img{512x368}](../../assets/b4a33be95b5db13d.png)
...
![img{512x368}](../../assets/613ae02e4a5b5f13.png)
...
Rancher除了提供UI工具外，还提供了一个名为[rancher-compose](https://github.com/rancher/rancher-compose)的CLI工具，用于在一个stack的范围内管理各个services。rancher-compose的灵感来源于[docker-compose](https://github.com/docker/compose)，兼容docker-compose的配置文件格式，并有自己的扩展。此外与docker-compose不同的是rancher-compose支持跨多主机管理。
...
![img{512x368}](../../assets/53b4f4cbfabfae6c.png)


Upgrade后，nginx-alpine-service的详细信息如下：

![img{512x368}](../../assets/fcb528b0701a5326.png)
...
![img{512x368}](../../assets/778398345c792088.png)
...
另外国内的[云舒网络](http://www.cloudsoar.com/)与 Rancher Labs是深度的合作伙伴，云舒公司博客上的内容也值得大家认真参考。
...
目前dcos还没有研究过。dcos是基于mesos的技术的吧。mesos也未涉猎，不过后续肯定都会做些研究:)

很有用的文档，谢谢作者，收藏了。

添加host的几条docker run 命令是否均在 10.10.126.101上运行？

我回顾了一下，在ui上添加一个host后，ui上会针对这个host生成一条命令(docker run)，而这条命令需要在添加的那个host上去运行。比如：添加一个host: 10.10.126.101，那么那条命令就在10.10.126.101上运行。如果添加的host是10.10.105.71，那么docker run就在10.10.105.71上运行。

另外rancher早已经进化到2.0版本了吧，不知道我这篇文章是否已经outdate了。请参考rancher官方最新文档吧。

新版中，UI有所变化，大致雷同

新版本的 rancher 已经基于 k8s 了

是的。我首次接触rancher的时候，那时候rancher还刚born没多久。