---
title: Goroutine调度实例简要分析
url: https://tonybai.com/2017/11/23/the-simple-analysis-of-goroutine-schedule-examples/
...
前两天一位网友在[微博](https://weibo.com/bigwhite20xx)私信我这样一个问题：

抱歉打扰您咨询您一个关于

[Go]的问题：对于goroutine的概念我是明了的，但很疑惑goroutine的调度问题, 根据《[Go语言编程]》一书：“当一个任务正在执行时，外部没有办法终止它。要进行任务切换，只能通过由该任务自身调用yield()来主动出让CPU使用权。” 那么，假设我的goroutine是一个死循环的话，是否其它goroutine就没有执行的机会呢？我测试的结果是这些goroutine会轮流执行。那么除了[syscall]时会主动出让cpu时间外，我的死循环goroutine 之间是怎么做到切换的呢？
...
从运行结果输出的日志来看，尽管有deadloop goroutine的存在，main goroutine仍然得到了调度。其根本原因在于机器是多核多线程的（硬件线程哦，不是操作系统线程）。Go从[1.5版本](http://tonybai.com/2015/07/10/some-changes-in-go-1-5/)之后将默认的P的数量改为 = CPU core的数量（实际上还乘以了每个core上硬线程数量），这样case1在启动时创建了不止一个P，我们用一幅图来解释一下：

![img{512x368}](../../assets/929e0928a9cd8941.png)
...
![img{512x368}](../../assets/31fae84ee32bb70d.png)
...
“I got scheduled!”字样依旧没有出现在我们眼前！也就是说main goroutine没有得到调度！为什么呢？其实所谓的“有函数调用，就有了进入调度器代码的机会”，实际上是go compiler在函数的入口处插入了一个runtime的函数调用：runtime.morestack_noctxt。这个函数会检查是否扩容连续栈，并进入抢占调度的逻辑中。一旦所在goroutine被置为可被抢占的，那么抢占调度代码就会剥夺该Goroutine的执行权，将其让给其他goroutine。但是上面代码为什么没有实现这一点呢？我们需要在[汇编](http://tonybai.com/tag/Assembly)层次看看go compiler生成的代码是什么样子的。
...
关于在for loop中的leaf function是否应该插入morestack目前还有[一定争议](https://github.com/golang/go/issues/10958)，将来也许会对这样的情况做特殊处理。
...
为什么要这么做呢？按照go team的说法，是为了更好的利用现代CPU的[“static branch prediction”](https://github.com/golang/go/issues/10587)，提升执行性能。
...
文章内容很扎实,但是我还是看不太懂, 可能是基础知识不到位,看完不知道该总结出什么,难道是一些循环里尽量多封装一层以便能够多次交出CPU?

你好，我想请问一下这个博客的评论系统使用的是什么系统

我用的是Wordpress自带的评论系统啊。目前第三方的评论系统不是关闭服务，就是有qiang，或者服务十分不稳定。

我以为go的调度是复杂东西，被你这么一说觉得不可思议的简单
...
0×0000 00000 MOVQ (TLS), CX // 将该goroutine的g结构体地址放入CX寄存器

0×0009 00009 CMPQ SP, 16(CX) //比较SP与g.stackguard0的值, g.stackguard0存着该goroutine的栈顶值。
...
如果想要非要绑定某个线程执行某个函数，golang 能否做到呢？

runtime包中有一个函数LockOSThread可以将某个goroutine绑定到当前启动goroutine的线程上，不知道能否满足你的需要。

据说1.12要修改调度方式了, 不再每个函数插入调度代码了.

在自己的Mac电脑上运行case4.go，并没有打印输出，请教什么原因？和系统有关系吗？
...
不知道您用的是啥系统，go compiler啥版本

go 1.12.8版本测试，有问题，没法打印，看了下这个morestack也有

0x10937bd e8dec1fbff CALL runtime.morestack_noctxt(SB)

0x10937c2 ebbc JMP main.dummy(SB)

用go v1.14版本测试的，虽然设置了runtime.GOMAXPROCS(1)，但是main goroutine里的I got scheduled!依旧能够打印出来。

go 1.14加入了抢占式调度。利用系统信号驱动的。那个例子对于Go 1.14及后续版本不再适用。