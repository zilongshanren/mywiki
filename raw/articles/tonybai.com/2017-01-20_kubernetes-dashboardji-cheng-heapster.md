---
title: Kubernetes Dashboard集成Heapster
url: https://tonybai.com/2017/01/20/integrate-heapster-for-kubernetes-dashboard/
...
[默认安装后的Kubernetes dashboard](http://tonybai.com/2017/01/19/install-dashboard-addon-for-k8s/)如下图所示，是无法图形化展现集群度量指标信息的：

![img{512x368}](../../assets/6fa283d135b2d861.png)


图形化展示度量指标的实现需要集成k8s的另外一个Addons组件：[Heapster](https://github.com/kubernetes/heapster/)。

Heapster原生支持K8s(v1.0.6及以后版本)和[CoreOS](https://coreos.com/)，并且支持多种存储后端，比如：[InfluxDB](http://influxdb.com/)、[ElasticSearch](https://www.elastic.co/products/elasticsearch)、[Kafka](http://kafka.apache.org/)等，这个风格和k8s的确很像：功能先不管完善与否，先让自己在各个平台能用起来再说^0^。这里我们使用的数据存储后端是InfluxDB。

### 一、安装步骤

我们的Heapster也是要放在pod里运行的。当前，Heapster的最新stable版本是[v1.2.0](https://github.com/kubernetes/heapster/releases/tag/v1.2.0)，我们可以下载其[源码包](https://github.com/kubernetes/heapster/archive/v1.2.0.zip)到[K8s](http://tonybai.com/tag/k8s) cluster上的某个Node上。解压后，我们得到一个名为”heapster-1.2.0″的目录，进入该目录，我们可以看到如下内容：
...
以[InfluxDB为存储后端的Heapster](https://github.com/kubernetes/heapster/blob/master/docs/influxdb.md)部署yaml在deploy/kube-config/influxdb下面：
...
从apiserver的日志来看，heapster是通过apiserver的secure port连接的，由于我们的[API server设置有https client端证书校验机制](http://tonybai.com/2016/11/25/the-security-settings-for-kubernetes-cluster/)，因此两者连接失败。

### 三、通过insecure-port连接kube-apiserver

现在我们就来解决上述问题。

首先，我们会想到：能否让heapster通过kube APIServer的insecure-port连接呢？在《[Kubernetes集群的安全配置](http://tonybai.com/2016/11/25/the-security-settings-for-kubernetes-cluster/)》一文中我们提到过，kube-apiserver针对insecure-port接入的请求没有任何限制机制，这样heapster就可以获取到它所想获取到的所有有用信息。

在heapster doc中的“[Configuring Source](https://github.com/kubernetes/heapster/blob/master/docs/source-configuration.md)”中，我们找到了连接kube-apiserver insecure-port的方法。不过在修改yaml之前，我们还是要先来看看当前heapster的一些启动配置的含义：
...
因此，该域名在[k8s DNS](http://tonybai.com/2016/10/23/install-dns-addon-for-k8s/)中会被resolve为clusterip:192.168.3.1。外加https的默认端口是443，因此实际上heapster试图访问的apiserver地址是：https://192.168.3.1:443。
...
根据heapster官方源码文件和我的文章中的描述（其实只是改了很少的一点配置）应该可以写出你的yaml吧。

白老师写的很好，最近小弟也在研究k8s，目前有几个问题，希望白老师解答一下：

1. 除了dashboard，有没有其他方法查看heapster监控数据呢？

我是k8s 1.5.2，heapster:canary+influxdb:v0.5+grafana:v2.6.0-2

2. 我的kubernetes-dashboard-amd64:v1.5.1界面里pod列表处没有CPU，memory那一列

3. 安装好heapster，influxdb，grafana，打开dashboard很慢是什么原因呢？

1、heapster对外部提供api，可以参考其项目页面的说明：https://github.com/kubernetes/heapster/blob/master/docs/model.md ，不过我并没有用过。2、你查看一下启动后的heapster各个组件的日志，看是否有何异常。一般heapster启动后，需要一些时间才能显示出图形数据；3、肯定会慢一些，因为需要加载一些metric数据，但是要是特别慢的话，还得从日志分析，不过这个过程需要耐心，还有细心，不放过一丁点蛛丝马迹。

感谢白老师，今晚调试了一下，看heapster是有数据的。influxdb日志没有异常，granfana有关于node docker0网卡错误[I] Completed 172.17.73.1 – “GET /query HTTP/1.1″ 500 Internal Server Error 0 bytes in 17285us。granfana网页能连上influxdb，打开后要画图才能看到监控数据。influxdb和granfana是否正常运行还待确认。

请问白老师：

1.k8s dashboard显示CPU，memory原理是啥，通过heapster拿到的数据，还是通过granfana画的图？

2.granfana启动环境变量如何设置？我把示例yaml里granfana的环境变量都注释掉了，到granfana容器里发现run.sh启动脚本中有些变量是空值（如BASHBOARD_LOCATION，GRAFANA_PORT这类）不知是否影响正常使用。

3. 我的k8s集群hpa对象的CURRENT字段是，如何能够正常显示数值呢？因为我想使用弹性伸缩功能，这个字段不显示应该会对弹性伸缩功能有影响。

CURRENT字段是waiting

1. 原理不甚了解；2、granfana的环境变量都在yaml中填写啊。你注释掉了，环境变量值就无法传递给granfana的pod了，具体影响还要看granfana究竟是如何使用env var的； 3、一直没有用hpa，也就没法解答你的问题了。

好的，十分感谢白老师了！！！

一直报这个错误，怎么解决呢？

E0912 05:58:51.434913 1 reflector.go:203] k8s.io/heapster/metrics/heapster.go:319: Failed to list *api.Pod: Get https://kubernetes.default/api/v1/pods?resourceVersion=0: dial tcp 10.96.0.1:443: i/o timeout

E0912 05:58:51.434996 1 reflector.go:203] k8s.io/heapster/metrics/processors/namespace_based_enricher.go:84: Failed to list *api.Namespace: Get https://kubernetes.default/api/v1/namespaces?resourceVersion=0: dial tcp 10.96.0.1:443: i/o timeout

E0912 05:58:51.435056 1 reflector.go:203] k8s.io/heapster/metrics/processors/node_autoscaling_enricher.go:100: Failed to list *api.Node: Get https://kubernetes.default/api/v1/nodes?resourceVersion=0: dial tcp 10.96.0.1:443: i/o timeout

建议你仔细确认一下：你的问题和文中遇到的问题是否是一样的，如果是一样的，就按照文中的解决方法解决。

老师，请问下，heapster 是否可以监控k8s的service的情况，我这里测试只能显示pod的cpu和memory。

目前不能。只能看到service下的pods的资源占用情况。

老师，你好，我搭建了heapster以后，发现grafana只能显示一个节点的数据
...
heapster里面报无法连接到另外一个节点，我手动试了一下节点的端口是可以访问的。不知道中间有什么地方需要修改

似乎是无法访问某个节点上的kubelet的服务端口，你所说的“手动试了一下节点的端口是可以访问的”是从pod的container内部尝试访问kubelet所在的节点和端口么？

谢谢老师，已经找到原因了，calico里面的IPPOOL与机器上的IP属于相同的网段，然后修改了calico的IPPOOL，解决了这个问题。感谢老师百忙之中的回复

您好，我也遇到和您一样的问题，您是怎么解决的，能详细描述一下吗？

遇到哪个问题？文中说明的不够清楚？

container stats from Kubelet URL “http://192.168.21.13:10255/stats/container/”: Post http://192.168.21.13:10255/stats/container/: dial tcp 192.168.21.13:10255: getsockopt: connection timed out
...
W0410 14:03:25.000523 1 manager.go:147] Failed to get all responses in time (got 1/3)

我也修改了calico的IPPOOL 但是还是报错

我们遇到的似乎不是一个问题啊。我的是权限认证问题，而你这是pod无法连接到位于node上的kubelet service啊，网络问题。你可以先手工进入pod中，然后ping一下 kubelet的地址，看通不通，如果网络通，再看看是否能访问kubelet的服务端口，可以手工curl一下。