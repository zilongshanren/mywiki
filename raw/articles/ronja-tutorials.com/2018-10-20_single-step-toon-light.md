...
url: https://www.ronja-tutorials.com/post/031-single-step-toon/
...
I explain the basics of custom lighting models more indepth in [this tutorial](https://www.ronja-tutorials.com/post/013-custom-lighting/), but you should be fine if you know the basics of surface shaders in unity.

![](../../assets/f9d8aa4e75c417cf.gif)


## Antialiased Step [🔗︎](#antialiased-step)
...
Then we change the lighting model to our own and create a new function to match it. The function has to be called Lighting`OurLightingModelName`

. The lighting model then has the name of the function name after `Lighting`

. We’ll take the `SurfaceOutput`

of the surf function, the light direction, the view direction and the shadow attenuation as parameters for our lighing function to calculate the lighting.
...
![](../../assets/e80b728a7329216c.png)


Then, to get a hard cut for the toon effect, we can use a `step`

function, it’ll return 0 if the first argument is greater, otherwise 1. The dot function returns 0 when the vectors are orthogonal which is the case at the half point of our surface. The side thats away from the light has negative values and the side towards the light positive ones. That’s why we pass the `towardsLight`

parameter as the second parameter into the step function and 0 as the first.
