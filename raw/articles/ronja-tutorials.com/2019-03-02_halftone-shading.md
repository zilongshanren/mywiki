...
url: https://www.ronja-tutorials.com/post/040-halftone-shading/
...
This tutorial is on another common toon shading technique called halftone shading, unlike normal shading it only uses full lit or full unlit as colors, but it doesn’t create a hard cut either. Instead it uses a pattern to decide which pixels are lit and which aren’t and the chance of a pixel being lit gets higher the brighter the pixel would be with a normal lighting method. To understand this tutorial I recommend reading and understanding [the tutorial about custom lighting methods](https://www.ronja-tutorials.com/post/013-custom-lighting/) and [the tutorial about generating screenspace texture coordinates](https://www.ronja-tutorials.com/post/039-screenspace-texture/).

![](../../assets/a44e796b9a6cd713.png)


## Hard Step Halftone Shading [🔗︎](#hard-step-halftone-shading)

For the first simplest implementation we use the result of the [custom surface lighting tutorial](https://www.ronja-tutorials.com/post/013-custom-lighting/) as the base shader and start to modify that. First we make it simpler by removing the part where the shader reads the value of the toon ramp. Instead we multiply the shadow attenuation earlier with the `towardsLight`

variable and pass it to the `saturate`

function to clamp it between 0 and 1. Additionally the light intensity is now only saved as a one-dimensional float since we don’t read from a texture with colors anymore.
...
![](../../assets/97a30416856b238e.png)


Now that we have both the light intensity and the halftone comparison value we can compare them with the step function to get a binary 0 or 1 value based on the pattern of our texture. The first argument of the step function is the halftone value we just sampled from the texture and the second argument is the lightness value. If the lightness is brighter than the halftone texture value, the shader sets to pixel to be fully lit and if it’s less it’s treated as shadowed. The result of the step value is put back into the `lightIntensity`
...
![](../../assets/be0865bd8da48d16.png)


## Antialiased Halftone Pattern [🔗︎](#antialiased-halftone-pattern)

This works well, but we’re not actually limited by binary colors in our shaders. That means we can preserve the appearance of the halftone binary values while still using greyscale values between them to make the result look less choppy. For this we replace our `step`

function with a `smoothstep`

function and interpolate the colors over a single pixel. The first task is to figure out how much the value we compare to changes over a single pixel. Luckily shaders provide us with the `fwidth`
...
![](../../assets/95dd63f8fc252d6b.png)


## Remapping Comparison Values [🔗︎](#remapping-comparison-values)
