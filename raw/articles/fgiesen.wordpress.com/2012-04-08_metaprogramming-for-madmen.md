...
operators. We figured that a source code pretty-printer should understand at least that much about C++ code, and then we could add some extra tokens in strategic places and we’d be set.
...
in strategic locations (way easier than actually removing the code yourself – just let the compiler’s dead code elimination deal with it!).

To give an example of what it did: Here’s a short original fragment from GenMesh that deals with selection processing:

void GenMeshElem::SelElem(sU32 mask,sBool state,sInt mode) { switch(mode) { case MSM_ADD: if(state) Mask |= mask; break; case MSM_SUB: if(state) Mask &= ~mask; break; case MSM_SET: if(state) Mask |= mask; else Mask &= ~mask; break; case MSM_SETNOT: if(state) Mask &= ~mask; else Mask |= mask; break; } }

and here’s the same code after Lekktor was done with it (run using the kkrieger data):

void GenMeshElem::SelElem(sU32 mask,sBool state,sInt mode) { switch(mode) { { } case MSM_ADD: { if(state) { Mask|=mask; } } break; { } case MSM_SUB: { if(state) { } if(1) { Mask&=~ mask; } } break; { } case MSM_SET: { if(state) { Mask|=mask; } else { Mask&=~ mask; } } break; { } case MSM_SETNOT: { if(state) { } if(1) { Mask&=~ mask; } else { } if(0) { if(0) { Mask|=mask; } } } break; { } } }

Aha – looks like that for this function, the `MSM_SUB`

path only ever gets called with `state`

true, and same for the `MSM_SETNOT`

path, so we can get rid of the ifs. Yeah, that’s the kind of thing you would *never ever* consider doing manually; it’s obviously extremely brittle and it only saves a handful of bytes (a single compare and a conditional branch). But that’s exactly the point – saving hundreds of bytes in one place can be done well by human optimizers. Saving 3-5 bytes in a thousand places, not so much.

Note that for every `if`

, we keep track of whether it’s either never or always taken; we still need to evaluate the if expression (which might have side effects!), so the original `if`

stays, but we yank everything out of the body and instead put it into a `if (0)`

(or `if (1)`

) block. Again, let the compiler’s data-flow analysis deal with the rest to clean this mess up!
...
Thanks for sharing! It would be interesting to know how later advances in exe-compressors would affect the .kkrieger code size. In particular would Crinkler make a big difference?

In my project ZGameEditor (which wouldn’t exist if it wasn’t for the inspiration from .werkzeug) I use a related technique for dead code removal: When I generate the binary I know which components (a.k.a. operators) have been used in this particular project. With that information + debug info which I ship with the tool I can remove all the components that are NOT used by zeroing out particular offsets in the binary. The offsets corresponds to the lines of code that I know are unused. This will shrink the binary after it is run through compressor. Won’t be as efficient as the .lekktor approach but does not require recompilation so more suited for end-user tool.

Looking forward to more articles in this series.

Never tried Crinkler for this, but even with the newer kkrunchy versions, the lekktor version compressed down to less than 90k, and the un-lekktor’ed code comfortable fit within the 96k limit. Of course, that version of kkrunchy didn’t exist back then! :)

Great reading ryg :))), I wish you could refactor Windows code with it :P

“The way any self-respecting 4k intro, 64k intro or other size-limited program starts out is, well, too large.”

In our intros, we actually never managed to reach the 64k limit. Felix’s Workshop is about 62k (and we can easily save 2k in a few minutes). We’ve been adding stuff in it for months, but everything compresses so well thanks to kkrunchy (ie. the full chess game took about 700 bytes), and size has not been an issue yet. I can’t imagine how much work it would take us to reach 70 or 80kb, but we probably work in a different way than Farbrausch (optimizing our code to save 10kb sounds extremely difficult).

If you don’t start out exceeding 64k, you’re not pushing things hard enough :)

That said, all of our really serious 64k efforts were done with packers much worse than what you can use now. With a current version of kkrunchy, the original executable of fr-08 ends up below 48k, and Candytron about 54k.

Ryg, how did you do the coverage analysis? What comes up to my mind is either simply periodically suspending the intro thread, retrieving the IP and then looking up the currently executed file/line, or doing the same with the trap-bit set. The former one might miss some lines, where the latter one I guess is too slow. So what solution you came up with?

As described in the article, Lekktor actually generates modified source code. The “final” pass outputs the code shown in the article; the “analysis” pass does something like:


`if (cond)`

→`if (cond && COVERAGE(id)) { .. } else { COVERAGE(id+1); ... }`

where COVERAGE is a function that basically does`counters[id]++`

. That way we get “execution got here” (either of the counters is nonzero), “always taken” (counter for id is nonzero, counter for id+1 is zero) and “never taken” (counter for id is zero, counter for id+1 is nonzero). You get the idea.The lekktor approach indeed sounds dirty, but that was great read!

Wow, great read. I’m not exactly sure why you sort of buried the idea for intros. They are so much more deterministic compared to games (ok, apart from some parts of the 3d engine if you’re trying to support different codepaths for different cards). Or is it just because you didn’t want to go through the nightmare again trying to figure out what went wrong?

It wasn’t hard to figure out what went wrong, but since lekktor couldn’t parse structs/classes properly, running it required a bunch of extra work: you needed to provide a list of which names where type names, and make sure to write the code in a way that no type name was ever used as a variable name (and vice versa). This is necessary because parsing even a small subset of C/C++ correctly requires knowing which identifiers correspond to type names – consider the expression

`(a)*b`

. If`a`

is a type name, the interpretation is`cast<a>(operator*(b))`

; if not, it is`operator*(a,b)`

. Anyway, this meant that you couldn’t just run lekktor if you hadn’t done so for a few months – usually, some new types had been added and you had to add to them first (the typename list used for the original kkrieger is here). Similarly, while the missing proper declaration parsing was the biggest single hole, lekktor really didn’t understand much of the C++ grammar at all, and again, running it for the first time after a few months usually required teaching some new constructs to the parser. And generating an instrumented build required some fudging as well – point being, lekktor was never something you could “just run” on the code, getting it to work on the current version of the code base usually took something like 1-2 hours.The bigger problem, however, was just that most ops had both a static component that run during initialization / load time (the “Init” phase) and a component that ran at run time (the “Exec” phase) to handle animation and such. These were usually right next to each other in the same file. When we wrote kkrieger, there wasn’t much going on in the “Exec” phase for most operators, with the exception of the gameplay code which was special and in its own file (

`kkriegergame.cpp`

). Once we started using Werkkzeug3 for demos, we ended up with a lot more operators that had exec phases, and “exec” code wasn’t safe to remove with lekktor. If it had just been the`Exec_`

functions, that would’ve been reasonably easy to add as a rule to lekktor, but unfortunately any code that wascalledfrom within exec had to be manually marked up as “don’t process” – lekktor never had enough information to figure out which function calls which other functions statically. (Which, in C++, is another hard problem).Anyway, point being, at the point when were doing demos with Wz3, we had a bunch of code that wasn’t written in a “lekktor-friendly” way, with just as much potential for problems as the game code in kkrieger. And it wasn’t a pressing enough problem to spend time on – Wz3 wasn’t really suitable for 64k intros anyway, and we only ever made 3 intros with it: the Project Snowblind teaser (which had no set size limit), “fr-048: precision” and “fr-052: platinum”. We though about using lekktor on precision, platinum and debris, but decided against it every time. Just not worth the hassle and potential trouble. The only one that was tight in terms of size was “precision” – as I recall, the initial version I got from Visualice was 1.5k over the 64k limit. However, I managed to fit it into the regular 64k within 2 hours or so by manual optimization; I remember the biggest single change was to the V2 ASM code, which had several jump tables in the text segment that were seriously confusing dispack. Moving these to the data segment (a 2-minute fix once I had found the problem) saved close to 1k by itself.