...
url: https://fgiesen.wordpress.com/2012/02/21/half-edge-based-mesh-representations-theory/
...
call per mesh. - Generating primitives and simple vertex deformations are easy in this representation.
...
- Most importantly, anything
*but*the aforementioned primitive generators and simple vertex deformers was inconvenient in this representation, so we just didn’t support it at all.
...
![Half-edge links The half-edge data structure](../../assets/35b5e15231aae782.png)


This shows a random highlighted half-edge `e`

, going from vertex `a`

to vertex `b`

– half-edges are directed, so order matters. The corresponding half-edge in the opposite direction is denoted by `opposite(e)`

. Our edge `e`

is part of face `f`

(blue shaded area). The classic half-edge data structure keeps track of:

- Per half-edge
`e`

: Either its start or end vertex index (let’s pick`start(e)`

, the red vertex, here), a link to the face`face(e)`

it belongs to, links to the previous and next half-edges`prev(e)`

and`next(e)`

in the same face (in winding order), and a link to the corresponding opposite paired half-edge`opposite(e)`

. - Per vertex: A link
`vertEdge(v)`

to one of the outgoing half-edges – any will do. (Equivalently, you can keep track of an incoming half-edge; again, this is equivalent) - Per face: A link
`faceEdge(f)`

to any of the half-edges that make up the face.

It’s obvious that this structure lets us go from (half-)edge to (half-)edge since that’s what we store. It’s also obvious that we can enumerate all half-edges making up a face in-order just by following the `next`

(or `prev`

) links. And since we can get from half-edges to vertices, that means we can enumerate all vertices belonging to a face easily too. The one thing that’s not immediately obvious is that we can iterate around vertices, too: define

prevVert(e) := next(opposite(e)) nextVert(e) := opposite(prev(e))

(look at the diagram to see this does the right thing) – note these return a half-edge, not a vertex index. Again, we can use the `face`

link for the corresponding half-edges to enumerate all faces that touch a given vertex.
...
,`e = prev(next(e)) = next(prev(e)) = opposite(opposite(e))`

, and`face(e) = face(next(e))`
...
to update, new value `v`

.

**Find start of logical wedge**. Sweep`e`

clockwise around its start vertex until you hit an edge with crease flag for`a`

set or you’re back at the beginning.**Find end of logical wedge**. Now sweep`e`

counterclockwise from the start of the logical wedge found in the previous step, again until you hit a crease for`a`

or you’re back at the beginning.**Update logical wedge**. Loop over all physical wedges between the two extremes (if the two are equal, this reduces to just “all physical wedges”) and set their value for attribute`a`
...
So here’s the punchline: we don’t separate vertices and wedges. We just store an array of (physical) wedges, which also happens to be exactly what needs to go into the vertex buffer for rendering (eventually). Instead of a vertex ID, we just store the physical wedge ID as `start(e)`

in the corresponding half-edges. When looping around a vertex using `prevVertEdge`

and `nextVertEdge`

, we end up iterating over all wedges for a vertex, so we might end up seeing multiple values of `start(e)`

(that’s why I didn’t require consistent vertex numbers for this case). This should also explain why I defined `end(e)`

to be `start(next(e))`

– the alternative choice of `start(opposite(e))`

will still return something at the same position, but potentially corresponding to a different wedge.
...
I didn’t like the Hoppe scheme. I went simpler instead of more complex and said that vertices have zero creases/discontinuities. Then you get to use a much simpler half-edge representation (the one I assume you’re going to get to next post!). For those vertices & edges that do have creases, I had each non-creased vertex in a circular singly-linked list of vertices that share the same position, but not any of the other attributes. From that data you can fairly easily find things like coincident edges, or you could put the edges in circularly linked lists as well (I chose not to – didn’t seem to be a problem).

To say it another way, each of my simple “mesh” representations was entirely without creases or discontinuities, though they did have edges – they were not 2-manifold (and of course each could be rendered with a single draw call). Then the vertex circles stitched these “meshes” together into a more fully-featured “shape”. One nice side-effect is that different materials or different vertex types work fine.

Note this representation allows edges that are creased at one vertex but not at the other, e.g. it goes from smooth to sharp, or the UVs match at one vertex but not at the other. IIRC the Hoppe formulation had trouble with this type of edge (forgotten the details).

One thing it doesn’t magically handle, which I think the above does, is where only some of the attributes are discontinuous but others are (e.g. UV seams with smooth normals). Those needed extra code to ensure the different vertices had the same normal data before and after topology changes – slightly gunky, but not horrendous.

We used a much simpler representation in the end (I’ll get to that eventually). I don’t actually do the Hoppe thing, I just borrow the terminology. Hoppe doesn’t keep track of discontinuities per-attribute; most of the complexity near the end comes from actually tracking this properly. The main case where the distinction ended up making a difference was during Subdivision, where each edge can be a boundary or non-boundary for every single attribute (which results in different subdivision stencils). If you’re willing to relax that requirement things indeed get much simpler (the “new” mesh generator in Debris, which I’ll describe later, does just that).

The representation I describe here does support edges that go from smooth to sharp too (not sure if there’s special terminology for this; vertices where a seam stops are commonly called “darts”). These are vertices with exactly one incident crease edge. The update code I describe will find this edge as the start of the logical wedge, then do one full loop around the vertex trying to find the end of the wedge. The result will be that start and end edge are the same, i.e. “one physical wedge” – which is what should happen.

i am trying to implement a half edge based data structure. my implementation is working for closed meshes but for open meshes, there are some half edges with null as their twin. so i am not able to get 1-ring neighbours of boundary vertices. can you help me out.

Simple solution: disallow unpaired half-edges. Half-edges are

alwayscreated in opposite pairs. Along a boundary, you end up with half-edges that don’t belong to any real face (e.g. face(e) is null or some sentinel). Such edges are always linked in closed loops that effectively form a “face at infinity”.To give an example, if you have a mesh consisting of a single triangle, it has 3 edges and 6 half-edges. 3 of the half-edges link the vertices of the triangle in say counter-clockwise order and specify the triangle as their face. The other 3 then link the same vertices in clockwise order (i.e. exactly opposite!) and form an infinite-area face that corresponds to the “outside” of the triangle. This face can be either explicitly represented or just be a null pointer. The important part is that all half-edges now have well-defined next, previous and opposite partners, which keeps traversal uniform. Having a null face pointer doesn’t affect traversal and hence is much easier to deal with.