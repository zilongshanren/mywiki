---
title: Fast blurs 1
url: https://fgiesen.wordpress.com/2012/07/30/fast-blurs-1/
...
A bit of context first. As I explained [before](https://fgiesen.wordpress.com/2012/04/15/genthree-overview/), the texture generators in RG2 / GenThree / Werkkzeug3 / Werkkzeug4 all use 16 bits of storage per color channel, with the actual values using only 15 bits (to work around some issues with the original MMX instruction set). Generated textures tend to go through lots of intermediate stages, and experience with some older texture generation experiments taught me that rounding down to 8 bits after every step comes at a noticeable cost in both quality and user convenience.
...
// Compute box-filtered version of x with a (2*r+1)-tap filter, // ignoring boundary conditions. float scale = 1.0f / (2*r + 1); // or use fixed point // Compute sum at first pixel. Remember this is an odd-sized // box filter kernel. int sum = x[0]; for (int i=1; i <= r; i++) sum += x[-i] + x[i]; // Generate output pixel, then update running sum for next pixel. for (int i=0; i < n; i++) { y[i] = sum * scale; sum += x[i+r+1] - x[i-r]; }
...
One of the big problem with summed-area table is the number of bits you need to have if you want to retrieve every value perfectly, i.e. a 1Kx1K map (total of 1M pixels) would need 20 bits of precision per channel. There are tricks with binning intermediate sums, but then, you have to compensate and your S(j) – S(i-1) becomes more complicated (and introduce floor() calls).

Looking forward to how you implemented Gaussian filters in the moving average situation, though…