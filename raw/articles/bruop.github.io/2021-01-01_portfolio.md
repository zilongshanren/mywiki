---
title: Portfolio
url: https://bruop.github.io/projects
...
The projects I've worked on can basically be broken down into two categories: web development projects and personal computer graphics projects. Occasionally there is overlap, as with [Echoes](https://echoes.cbc.ca/), but for the most part I've only documented my computer graphics projects on this page.

The two main projects I've worked on in my spare time are [bae](https://github.com/bruop/bae) and [Zec](https://github.com/bruop/zec). Below are the "greatest hits" from those projects.
...
- Image based lighting with multiple scattering (see description below for
[bae](https://bruop.github.io#bae)and[this blog post](https://bruop.github.io/ibl)) - ISPC accelerated view frustum culling, as documented in
[another blog post](https://bruop.github.io/improved_frustum_culling/) - Clustered forward shading using compute shaders
...
[
](https://bruop.github.io/static/a80e868d81115b6193b4507887c1c86e/0d4f8/clustered_debug_forward.png)
![Shaded view. There are 1024 point lights and 128 spot lights distributed randomly throughout the scene Shaded view. There are 1024 point lights and 128 spot lights distributed randomly throughout the scene](../../assets/e628c197f568ffc0.png)




![Clustered Shading Debug view. There are 1024 point lights and 128 spot lights distributed randomly throughout the scene Clustered Shading Debug view. There are 1024 point lights and 128 spot lights distributed randomly throughout the scene](../../assets/b9e02559ebca5324.png)

One thing to note is that I used 32-byte `uint`

s to store light indices, but this is overkill for the number of lights I expect to support. I could easily get away with 16-bits for the index list entries BUT my laptop’s 960m lacks the feature support for 16-bit arithmetic 😢
...
[
](https://bruop.github.io/static/97d98e9c878f44b6813ca81a7a8604ae/f3baa/01-tonemapping.png)
![Image rendered using the Uchimura tone mapping operator Image rendered using the Uchimura tone mapping operator](../../assets/92e11f36fda602ad.png)
...
[Reinhard](http://www.cs.utah.edu/~reinhard/cdrom/tonemap.pdf)with variable white point- Unreal, based off ACES with built in gamma correction
[Lottes](http://32ipi028l5q82yhj72224m8j.wpengine.netdna-cdn.com/wp-content/uploads/2016/03/GdcVdrLottes.pdf)[Uchimura](https://www.desmos.com/calculator/gslcdxvipg)
...
[
](https://bruop.github.io/static/81bb0e0b8a6d5d319422a3d011fb9dd4/f3baa/02-forward-rendering.png)
![Forward rendered Sponza atrium Forward rendered Sponza atrium](../../assets/d1480a588049fba8.png)


The two examples `02-forward-rendering`

and `03-deferred-rendering`

are set up to be largely identical, but with a few key differences to highlight the strengths and weaknesses of forward vs deferred rendering.
...
[
](https://bruop.github.io/static/77ef246d1df02838b35d47ec2bcfaead/f3baa/03-deferred.png)
![Composite image showing different parts of the Gbuffer and final image Composite image showing different parts of the Gbuffer and final image](../../assets/fd870f70756264cd.png)
...
In both cases we increment the stencil buffer, and use a stencil test against `0`

(the starting value) for the second draw call.
...
[
](https://bruop.github.io/static/6b7135ce901f83cfc91f83cdb9935dba/f3baa/04-pbr-ibl.png)
![Flight Helmet model rendered using image based lighting Flight Helmet model rendered using image based lighting](../../assets/2b534b63875b3bb8.png)
