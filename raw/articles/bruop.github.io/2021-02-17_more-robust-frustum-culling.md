...
url: https://bruop.github.io/improved_frustum_culling/
...
[
](https://bruop.github.io/static/82b0a88e896ea8de3966da8ad742a499/1415a/intersection_example.png)
![An example of an edge-on-edge collision that would erroneously result in the OBB being culled using our previous method, despite it being visible. An example of an intersection the view frustum and bounding](../../assets/967c0f076af029f2.png)


## Separating Axis Theorem

As was pointed out to me on twitter, an alternative is to use the [Separating Axis Theorem](https://en.wikipedia.org/wiki/Hyperplane_separation_theorem). The theorem states that if two convex shapes are not intersecting, then there exists a separating plane which we could place between them, and perpendicular to this plane is the *separating axis*. If we project these volumes onto this separating axis, their extents along the axis will not overlap.

[
](https://bruop.github.io/static/746c100e050a10f3260f5ffaf6e52414/84390/separating_axis2.png)
![An overhead view of a frustum, an OBB, the separating line between them, and the separating axis An overhead view of a frustum, an OBB, the separating line between them, and the separating axis](../../assets/1a99597a107ffdd1.png)
...
[
](https://bruop.github.io/static/e7cb7e9c1b0ffdee4cc397b40f8902b3/dc666/OBB.png)
![An oriented bounding box with its center and its axes labels An oriented bounding box with its center and its axes labels](../../assets/7290ffb7d23f2738.png)




![Our view frustum with its normal and edge directions labelled. Note that the frustum's view direction is aligned with the negative z axis, and the up and right vectors are aligned with the y and x axes respectively. Our view frustum with it's normal and edge directions labelled](../../assets/da857d12b6951fe2.png)

Note that we can ignore directions that are anti-parallel to axes already under consideration, meaning that for the OBB we need only test three of its normal vectors, which will be parallel to its axes . For the view frustum, only its near and far planes have anti-parallel normal vectors so we’ll need to test five different normals for that volume.
...
To define this mathematically, consider an OBB with axes , half-extents and a frustum with up axis , right axis , normals and edges , then our 26 different axes will be the following set:

We can make things much simpler for ourselves by performing these calculation in view space, where our frustum’s origin is at and it’s up axis and right axis will just be aligned with the and axes, respectively, making it trivial to calculate our edge vectors and normal vectors . If our view frustum has near and far planes at and and the near plane has half-width and half-height then we can define both the normals and edges as the following:

The last question we have to solve is how to project our volumes onto the axis. For the OBB, it is fairly straightforward. We can project the center of the OBB onto the axis and then work out it’s extent along the axis from that center; let’s define that interval as where is the projected center of the OBB and is the projected radius. We can calculate these two terms using:

Where is the center of the OBB, is the ith axis of the OBB, and is the scalar extent of the OBB along the ith axis.
...
[
](https://bruop.github.io/static/a3a59cd9f0f360676366daac403d571b/47cac/frustum_overhead.png)
![An overhead view of a frustum projected onto a potential separating axis An overhead view of a frustum projected onto a potential separating axes](../../assets/2ae81c1a1e6110f6.png)


Here the axis is perpendicular with the direction of the view frustum , so we know that the extent will be defined by the vertices at the far plane of the frustum. But as the separating axis is rotated away from the view direction, we might see something like the following:

[
](https://bruop.github.io/static/58070dd3aec7416eb7e49c3085640ec3/47cac/frustum_overhead_2.png)
![An overhead view of an alternative orientation for the frustum, projected onto a potential separating axis An overhead view of an alternative orientation for the frustum, projected onto a potential separating axis](../../assets/8fa1193962b41845.png)


Here, the lower bound of the projection, , is still defined by a far plane vertex, but the upper bound is defined by a vertex on the near plane. So we can say that as the axis sweeps across relative to , we’ll need to consider either the projection of the near plane or the far plane, and we’ll have to do this for both bounds. [This document by David Eberly](https://www.geometrictools.com/Documentation/IntersectionBox3Frustum3.pdf) lays out the exact function for calculating and , reproduced here using our symbols and restricting the problem to taking place in view space:
...
Other than this initial setup, I was was surprised by how trivial it was to convert the code in my `SAT_visibility_test`
...
However, just like if we wanted to “SIMDify” this code ourselves, we’d need to make sure that for each multiply we used registers filled with all the `x`

components, all the `y`

components, etc. ISPC works the same way and it’ll opaquely load the `vec4`

data into a struct that looks more like:
...
But that means that if we pass our ISPC code an `Array<vec4>`

, it’s going to first have to load 8 different `vec4`

elements from the array, and then scatter their individual elements of `x, y, z, w`

into an instance of `vec4_SoA`

such that it can perform the SIMD multiplies.

Similarly, when it goes the store the data into `out_list[i]`

, it’ll have to scatter the data from the SIMD registers into the AoS `Array<vec4>`

after each operation. We’ll end up with *something like* the following:
