...
url: https://www.willusher.io/2015/05/15/porting-a-ray-tracer-to-rust-part-3/
...
It’s been a little while since my last post on tray_rust as I’ve been a busy with classes, but I’ve
...
We’ll start by putting together a `Boundable`

trait for types that can report their bounds in space:
...
here’s the intersect call of a `BVH<Instance>`

in scene.rs. Here our Fn takes a &mut Ray and &Instance and returns an Option
...
long as the BVH is alive. This is done with the `'a`

lifetime annotations in the intersect function signature.
...
and [here it is](https://crates.io/crates/tobj)! Adding this library as dependency to tray_rust can be done
by adding `tobj = "0.0.8"`

to the `[dependencies]`

list in the project’s Cargo.toml and the crate will be downloaded and available for use
...
convenient. I’ve started using this for tray_rust as well.
...
The current analytic models supported in tray_rust are [Lambertian](http://en.wikipedia.org/wiki/Lambertian_reflectance) (an ideal diffuse material),
...
by only having traits be in scope if you use them, eg. `use my_lib::CoolTrait`

will make CoolTrait’s methods available on implementing types.
...
![](https://cdn.willusher.io/img/fUEv6Au.webp)
...
![](https://cdn.willusher.io/img/9QU6fOU.webp)
...
There are still a few things left on my todo list for tray_rust. I need to fix my BVH construction so it doesn’t give some
