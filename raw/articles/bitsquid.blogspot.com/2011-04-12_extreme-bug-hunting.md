---
title: Extreme Bug Hunting
url: https://bitsquid.blogspot.com/2011/04/extreme-bug-hunting.html
author: Niklas
...
When a bug has a random component you can't be certain that a fix that made the bug disappear

*really*fixed the bug. It might have just masked it. Still, if you don't make any assumptions at all you won't get anywhere. Just as when you solve a difficult crossword puzzle, you may have to make some guesses to get started. So if the bug disappears when you disable a particular system and reappears when you enable it, you can assume as a

*working hypothesis*that the bug is caused by something in that system. But you should be ready to abandon that hypothesis if you find evidence to the contrary.
...
If the bug disappears in an earlier version you can do a binary search of the revisions until you find the point where the bug was introduced. Git even has a cool command for this:

*git bisect*. When you find the revision that introduced the bug it should be easy to spot the error.
...
Could the error be caused by uninitialized data? One way of finding out is to fill memory with specific values on malloc() and see if the behavior of the bug changes. This requires that you have implemented your own memory allocators, but you

[should do that anyway](http://bitsquid.blogspot.com/2010/09/custom-memory-allocation-in-c.html).
...
- Code that writes to a memory address after having called free().
- Code that allocates a buffer of a certain size and writes beyond that size (buffer overflow).
...
Having a working linux (and now, I think osx too) build enables you to run it under Valgrind, which is immensely useful for catching most of the memory related errors. You can even annotate custom memory pools for it.

ReplyDeleteOf course that isn't really useful if the problem is with spu code.

Thanks for sharing the best information with us and i loved it,

ReplyDeletecinema hd apk download

google 1713

ReplyDeletegoogle 1714
...
ReplyDeletecinema hd apk

cinema hd

cinema hd apk

Great stuff. Keep doing the good work.
