---
title: A Data-Oriented, Data-Driven System for Vector Fields -- Part 2
url: https://bitsquid.blogspot.com/2012/10/a-data-oriented-data-driven-system-for.html
author: Niklas
...
*bytecode*that gets executed by a virtual machine.
...
Execute that single instruction for
...
So to handle the AABB culling you generate a list of particles for each subfunction using a spatial query, add their result pointers to the registers list, and run just that subfunction on the VM?

ReplyDeleteNo the other way around... what I pass to evalute() is a list of points, as well as an AABB for those points.



ReplyDeleteThe first job of evalutate() is to find the effects that intersect the AABB of the points (i.e. I disregard any effect that cannot have any effect on any of the points).

I merge the bytecode of the relevant effects into a single bytecode function that I then run on all the particles.

Is there a reason you don't use the GPU? I was thinking that you're essentially writing HLSL and then processing it in batches like the GPU does, but you're doing it on the CPU.


ReplyDeleteAlso, I wonder if these ideas could provide any benefits to Lua.

The latency issues with GPU readback can be problematic if this is used to drive non-graphical systems (such as physics). Also, I'm not sure the way we dynamically merge bytecode would work on the GPU.



ReplyDeleteBut we are using a design close to HLSL so that we have the option of moving things to the GPU where that makes sense.

This is very informative. Yellowstone Coat

ReplyDeleteThe article I saw was interesting because of the offer. As I was expecting to see the count of upcoming posts, showing your plan to continue sharing such great things, I am glad to see you. Visit my latest post on How to solve the problem of a single mouse click that turns into a double click. It will be a pleasure to visit Fix The Mouse. Thank you for your concern.


ReplyDeleteThis is very interesting content! I have thoroughly enjoyed reading your points and have come to the conclusion that you are right about many of them. You are great.สล็อตวอเลท

ReplyDeleteYour content is nothing short of brilliant in many ways. I think this is engaging and eye-opening material. Thank you so much for caring about your content and your readers.สล็อตเว็บใหญ่

ReplyDeleteYour content is nothing short of brilliant in many ways. I think this is engaging and eye-opening material. Thank you so much for caring about your content and your readers.เกมสล็อต

ReplyDeleteService personnel are experts. Though they might be portrayed in a different way in films and upon television, they expect in the future into your house, handle the actual cleaning, as well as leave understanding they've amazed and held you pleased. If you take time to hire the best professionals for that job, you might be far happier using the results compared to you thought you may be. What steps in the event you take before securing a person to do these tasks for you personally. babysitting nanny services dubai
