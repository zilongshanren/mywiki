---
title: A Pragmatic Approach to Performance
url: https://bitsquid.blogspot.com/2011/12/pragmatic-approach-to-performance.html
author: Niklas
...
If you are writing a big program, some parts of the code will not be as fast as theoretically possible. Sorry, let me rephrase. If you are writing a big program,

*no part*of the code will be as fast as theoretically possible. Yes, I think it is reasonable to assume that every single line of your code could be made to run a little tiny bit faster.

Writing fast software is not about maximum performance all the time. It is about

*good performance where it matters*. If you spend three weeks optimizing a small piece of code that only gets called once a frame, then that’s three weeks of work you could have spent doing something more meaningful. If you had spent it on optimizing code that actually mattered, you could even have made a significant improvement to the game’s frame rate.
...
*actually*spending its time, a top down profiler is an invaluable tool. We use explicit profiler scopes in our code and pipe the data live over the network to an external tool that can visualize it in various ways:
...
Niklas, thanks for the insightful post(once again!). Of all your guidelines, I guess, in your blog posts you haven't covered only:



ReplyDelete* Avoid ”do nothing” updates -- instead, keep track of active objects

* If the system handles many objects, support data parallelism

Can't wait :)

Nice article! One simple thing I'd add to your list, which can make a difference in 'hot' loops:





ReplyDeleteOrder complex/expensive conditionals with most common cases first:

if (commoncasetest() || rarecasetest()) {

// do operation...

}

or the inverse with negation:

if (!rarecasetest() && commoncasetest()) {

continue;

}

@willjcroz Yes, that's always a good idea.

ReplyDeleteNice journal with terribly attention-grabbing and helpful data on your web site. Thanks for sharing the journal and this nice data that is unquestionably planning to facilitate us...
...
Pet Door Installation Rancho Santa Fe CA

You guys try it for the best Home Inspection services.

Indibet login commitment to ongoing innovation enables it to modify and enhance its products on a regular basis in order to accommodate its consumers' shifting requirements and tastes. To improve the betting experience and remain on top of the game, the platform releases upgrades, additions, and new features on a regular basis. Indibet is always working to deliver its customers the greatest possible betting experience, whether that means adding new sports markets, improving the user interface, or streamlining the platform for maximum efficiency. Indibet guarantees that its customers always have access to state-of-the-art betting features and technology by being at the forefront of innovation

ReplyDeleteA pragmatic approach to performance starts with smart fixes—like using Punch-Free Screw Stickers for hassle-free, damage-free wall mounting!

ReplyDeleteFinding stable wifi for rv is a game-changer if you travel full-time. I recently checked out Monthly Internet, and their plans look pretty flexible compared to others I’ve seen.
