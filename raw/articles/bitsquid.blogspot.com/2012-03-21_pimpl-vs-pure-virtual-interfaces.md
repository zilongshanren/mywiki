---
title: PIMPL vs Pure Virtual Interfaces
url: https://bitsquid.blogspot.com/2012/03/pimpl-vs-pure-virtual-interfaces.html
author: Niklas
...
Factory functions are not needed, you can use

*new()*,*delete()*or create objects on the stack.The

*SoundWorld*class can be subclassed.The interface methods are not virtual, so calling them

*might be*faster. (On the other hand, we need an extra memory fetch to get to the implementation object.)PIMPL can be introduced in an existing class without changing its external interface or its relation to other classes.
...
ReplyDeleteCouldn't agree more. As I start using things like POD types and arrays and not using virtual inheritance very much, I start to ask myself why I don't just write the whole thing in C.

this is very informative. Yellowstone Coat

ReplyDeleteI loved the actionable steps you provided. They’re simple but so effective. Can’t wait to give them a try!
