---
title: Sensible Error Handling — Part 3
url: https://bitsquid.blogspot.com/2012/02/sensible-error-handling-part-3.html
author: Niklas
...
One idea I've had for this problem is to make warnings errors unless explicitly silenced. In this way you can grandfather in old stuff but prevent *new* warnings from creeping in.


ReplyDeleteYou'd also like to have a way you can review all silenced warnings before ship.

This is off topic, but I would like to now your opinion on singletons. Do you use singletons in the Bitsquid engine?

ReplyDeleteFirst, I don't like the name "singleton". That seems to indicate that we are dealing with something "fancier" and "better" than a global variable, which let's face it, we are not.
...
ReplyDeleteSecond, I think they are overused. I've seen a lot of cases where people use singletons for things like Camera or World. Why should there be just one camera or just one world?

Third, if you are going to use singletons, please have explicit methods to create and destroy them. I've seen many code examples where singletons get created "automatically" and never destroyed. This means you will run into all kinds of startup-order issues when singletons depend on other singletons. It also means you can't do a clean complete shutdown of your system. (We do a clean shutdown to verify that we don't have any memory leaks.)

In my opinion it is much cleaner and nicer to explicitly create and pass objects than to use "singletons". We explicitly pass many types of "global" objects to subsystems which I suspect are singletons in other engines, such as

Application,Window,ThreadManager,ResourceManager, etc.We use singletons/global variables for a few things though, for example

ProfilerandLogging. Since they can be used in any function and are not directly related to what the function actuallydoes, it would be cumbersome and ugly to pass them explicitly.For these systems we always have functions for setting them up and shutting them down in a clean way, something like:

void setup_global_profiler();

void shutdown_global_profiler();

bool has_global_profiler();

Profiler &global_profiler();

Thank you for guiding me how to resolve this error. Yellowstone Coat

ReplyDeletethank you for publishing this article. Great help!
