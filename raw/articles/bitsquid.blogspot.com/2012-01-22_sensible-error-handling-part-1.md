---
title: 'Sensible Error Handling: Part 1'
url: https://bitsquid.blogspot.com/2012/01/sensible-error-handling-part-1.html
author: Niklas
...
*expected error*I mean any kind of error that happens in a situation where the caller can reasonably expect that something might go wrong and has a plan for dealing with that. The most typical example is network code. Since the network may die at anytime, the caller cannot just call

*fetch_web_page()*and assume that she will get a valid result. She must always check for and be prepared to handle errors.

An

*unexpected error*is an error that happens when the caller has no reason to assume that something might go wrong. A typical example might be a

*NULL*pointer returned by an allocator that is out of memory or a corrupted internal state caused by a buffer overflow problem.

What errors can be considered ”expected” depends on context. When opening a saved game or a user config file,

*File Not Found*might be an expected error, because we can expect the user to muck around with those files. When opening our main .

*pak*bundles,

*File Not Found*is an unexpected error, because we don’t expect the user to partially delete an installed game. And besides, there is not much we can do beyond displaying an error message if our data isn’t there.

A

*warning*happens when someone has done something that is kind-of sort-of bad, probably, but we are able to continue running without any ill effects. An example might be a call to a deprecated function.
...
By crashing, the API is

*taking full responsibility*for performing what the caller asks of it. We are saying: either we will do what you wanted or, if there is a problem with that, we will deal with that too. In either case, you don’t have to worry about it.
...
Note the curious absence of any error codes. If the caller passes a malformed path, we crash, we do not return an

*E_INVALIDARGUMENT*error. If the file doesn’t exist, we crash. The caller is responsible for using

*exist()*to check for files that might not exist. There are no errors for the caller to handle and the code will be clean and readable.
...
*StalkWalk64*to generate the call stack and the

*Sym**functions to translate it.
...
If there is a parse error, it would be very helpful for the user to know in which file the error occurred. But the

*parse_json_number*function doesn’t know that. It doesn’t even know if there is a file. It might have been asked to parse data from network or memory.
...
the problem of the proposed api design is that crashes lose session data. When game designers start using tech to create very complex data sets in mass production the cost of getting all of it corrupted goes sky high very quickly.

ReplyDeleteNot for us. Our tools use the engine as a visualizer over a TCP/IP connection. The state of the edited data is held by the C# application. So the engine can crash and be restarted, without any data being lost in the tool.


ReplyDeleteAlso I think the "crash early" philosophy is a weapon against corruption, rather than the other way around. If the engine crashes on corrupt data, it gets noticed and fixed quickly. If the engine doesn't crash the corrupt data can spread and infect more of the content.

This comment has been removed by the author.

ReplyDeleteNiklas, may there be any cases when an out of memory error is (and should be treated as) an expected one?

ReplyDeleteI don't think there is any sensible way for a game to handle a general out-of-memory error. (Where a random allocation anywhere in the code fails.)



ReplyDeleteThere might be ways to handle specific out-of-memory situations, though. For example a game might allocate 50 MB for sounds and if it runs out of sound memory it might handle that by throwing out some old sounds. But I would regard that as a system working as designed, rather than error handling.

For some non-game applications it might make sense to handle general out-of-memory errors. For example, such an application might free a "rainy-day fund" of memory that it has saved for just this purpose, so that it can continue to run long enough to put up a message box that alerts the user to the problem and allows her to save her files.

As someone following the same phylosophy, I agree on all you just said.



ReplyDeleteThe way we handle session lost data is by catching exceptions (including SEH) and attempting to save everything. This mechanism isn't fail-safe as the data or even the CRT lib may be in a corrupted safe; which is why it is removed from the final game. 95% of the times though, it works.

As for exceptions, it can be a performance optimization. The idea of being "sloooooooooow" is just old fashioned thinking. The problem is,; people say it's "slow", but... compared to what? Performance is relative, not absolute.

The explanation can be read here: http://forums.pcsx2.net/Thread-blog-C-exceptions-can-be-an-optimization

Like the link says, in some cases it is slow; but in some cases it isn't.

Errata: corrupted *state*; not "safe"

ReplyDeleteI like the crash philosophy a lot and especially the minimization of error mgmt on the client side.

ReplyDeleteThere is still one situation when I'm more in favor of a "No crash-No effect-Error notification", it is when a function is supposed to be exposed to a scripting system (directly wrapped). I consider that a script should never be able to crash the system, it should just report the erroneous state.

@Jeremie I wholeheartedly agree. We have a special assert for this (SCRIPT_ASSERT) which triggers lua_error().


ReplyDeletelua_error() doesn't crash the engine. Instead, it prints an error message to our internal console and freezes the game -- giving the gameplay programmer a chance to fix, reload & continue.

Thank you for sharing this wonderful information. Yellowstone Coat
