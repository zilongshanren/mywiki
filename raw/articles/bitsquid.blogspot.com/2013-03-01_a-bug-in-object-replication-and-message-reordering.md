---
title: A Bug in Object Replication and Message Reordering
url: https://bitsquid.blogspot.com/2013/03/a-bug-in-object-replication-and-message.html
author: Niklas
...
*We could impose a global message ordering.*We could make sure that the reliable message streams are globally ordered to prevent "paradoxes" of this kind. I.e., if*HOST*sends*M_ab*after receiving*C*, no peer should receive*M_ab*before*C*. Unfortunately, this is not as easy as it sounds. For example, what if*A*dies before it has sent*C*to*X?*In that case, that failed delivery will also block the channels*HOST → X*and*B → X*, since they are not allowed to deliver any messages before*X*has received*C*.*We could use a migration handshake.*We could do some kind of handshake procedure to make sure that everybody has received*M_ab*, before*B*takes over ownership. But this would require a lot of extra messages and temporarily put the object in limbo.*We could fix the ACKs.*We could make it so that*X*doesn't*ACK**M_ab*until*C*has arrived, thus forcing*HOST*to keep resending it, until we are ready to receive it. This would work, but would require us to implement ACKing of individual messages. Currently, we just ACK an entire UDP packet (containing many messages) on reception, which is simpler and more performant.*We could create an internal message queue.*We could queue up migration, create and delete messages in some sort of internal queue if they arrive out of order and try to fix things up later. This is a truly horrible "solution" that increases code complexity and is likely to cause lots of confusing bugs in the future.
...
*MIGRATE*act as a*CREATE*message, if we get*MIGRATE*for an object that doesn't exist.We ignore "old"

*CREATE*messages. (The*C*that arrives after*M.)*
...
Nice article! I'm implementing network system over UDP and I need some tips about object replication. Could you suggest me a good starting point(books, blogs...)? Thanks

ReplyDeleteThanks for the article! I do have one question though: Why is the creation message so special in your protocol? Wouldn't it be possible to just send all required information for object creation together with the current update data until you get an ack? This would reduce the wait period between create and the first update and make the protocol and implementation easier. Think of the 'create' as just another update from no state to the first valid one. The same works for the not shown destroy step. Or did I miss anything?
...
ReplyDeleteOne way to implement ownership migration is to have the new owner just take over and update the object and send update data to the other clients. The other clients will notice that and will just use the new data from the new owner.

To resolve conflicts between multiple clients trying to get ownership of the same object at the same time (which shouldn't happen too often) one could use a very simple static resolve strategy. Both clients have to be able to decide how to resolve the conflict without any additional communication. The simplest is to use the client-id or some other fixed number associated with each client (The higest/lowest id wins). The loosing end just drops the ownership of the object when it gets the first update from the winner (and tries hard to make the transition look good ;)

@Julien Mostly to ensure that the create messages get into the same ordered stream as the RPC calls, so that if you send an RPC message talking about a specific object, you can be sure that it exists on the receiving end.

ReplyDeleteThis article is very amazing. Yellowstone Coat

ReplyDeleteconflicts can be solved if put some more efforts
...
The Bitsquid network framework upholds a distributed model with relocation of organization objects - i.e.,Help Me With My Homework changing the proprietor of an organization object starting with one friend then onto the next. This as of late lead to an uncommon race condition.

ReplyDeleteA plagiarism free essay generator is a valuable tool for students seeking inspiration and assistance with their writing. These generators utilize advanced algorithms to generate unique content, ensuring originality and minimizing the risk of unintentional plagiarism. However, always remember to properly cite and reference any external sources you use to maintain academic integrity and give credit to the original authors.

ReplyDeleteAnyone know where I can get a dupe of the travis kelce green suit? I need that for my next event.
