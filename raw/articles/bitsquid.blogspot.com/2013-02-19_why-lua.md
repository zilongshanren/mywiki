---
title: Why Lua?
url: https://bitsquid.blogspot.com/2013/02/why-lua.html
author: Niklas
...
**Simplicity**. (A small, manageable codebase with a minimalistic, modular design.)**Flexibility**. (A completely data-driven engine that is not tied to any particular game type.)**Dynamism**. (Fast iteration times, with hot reload of everything on real target platforms.)**Speed**. (Excellent multicore performance and cache-friendly data-oriented layouts.)
...
Lua is a pretty nice lightweight language. It's very much like JavaScript but without the badly designed parts.


ReplyDeleteRegarding type inference, I don't think even IDEs for mainstream languages support this very well.

Lack of type safety and type inference is the one thing that makes me very leery about using Lua, despite all other benefits.


ReplyDeleteIt would be an interesting project to see if you could make a statically typed Lua, or at least one that is more easily analyzed for type correctness, and still keep the language simple. I like Dart for this reason, but the language is super complex, and not as dynamic as Lua.

Lua is type safe. I guess what you're referring to is dynamic typing. There's merit to both dynamic and static typing. When coming from a static language, dynamic typing feels scary and unsafe, but you often gain in development speed. For scripting and small programs dynamic typing is often at an advantage. IMO of course.

DeleteThe IntelliJ stuff for PHP and Javascript do a lot of work to offer some completion and type checking features, with varying degrees of success depending on the library you are using. Perhaps one day they'll do the same for Lua.


ReplyDeletehave you looked into this? haven't done so myself, but they make big claims (of course...):


ReplyDeletehttp://unknownworlds.com/decoda/

I recently heard of Squirrel, a Lua-like language written by one of the engineers at Crytek to overcome some of the issues they found with Lua particularly as it applies to game engine scripting.




ReplyDeleteAre you familiar with Squirrel? And if so is there a reason that you stuck with Lua as opposed to going with Squirrel?

/mike.

To me personally, Lua has a more beautiful syntax. Otherwise, I'd say the languages are basically the same except that Lua is older and thus has rather more libraries. Also, I had the impression that the wiki of Squirrel is not that good.

DeleteQUOTE:


ReplyDeletefunction ::= function funcbody

It seems there's a recursion, however the second "function" is just the "function" keyword.

Hi there, I found your blog via Google while searching for such kinda informative post and your post looks very interesting for me Print Production

ReplyDeleteThis information is very amazing. Yellowstone Coat

ReplyDeleteI must say this blog is worth reading as it is stuffed with great many suggestive pieces
...
Your blog is a breath of fresh air in a sea of generic content. I appreciate your unique perspective and the valuable insights you provide. Also, I am sharing a Profile based on color vision test. Color blind tests serve as a reminder that even though we might see things differently, we can still appreciate the beauty in all its forms.

ReplyDeleteb2b spa near me like Bellaspa can help improve skin health by cleansing, exfoliating, and moisturizing the skin.

ReplyDeleteThis comment has been removed by the author.

ReplyDeleteThis was such a well-written piece. I love how you mix personal experience with professional advice!
