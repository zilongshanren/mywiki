---
title: Strings Redux
url: https://bitsquid.blogspot.com/2011/06/strings-redux.html
author: Niklas
...
- Using the same encoding everywhere means there is never any confusion about what encoding a certain string or file should be in. If it is not in UTF-8, then it is
*wrong*. Period. - UTF-8 uses the standard C data types for strings:
*char **and*char []*. - ASCII strings look like ASCII strings and all functions, parsers, etc that operate on ASCII strings work on UTF-8 strings without modification.
...
In the few cases where you

*do*need to convert to glyphs (for example for rendering) you typically do that

*sequentially*, from the start to the end. This is still a fast operation, it is only

*random access*of glyphs that is significantly slower with UTF-8 than with UTF-32. Another interesting thing to note is that since all continuation bytes in UTF-8 follow the pattern 10xxxxxx you can quickly find the start and end of the next or previous glyph given a

*char **to anywhere within a UTF-8 string.
...
For code that deals with static strings you should always use

*const char **rather than

*const string &*. The former is more flexible. It allows the caller to store her strings however she likes rather than adhering to some memory model imposed by the string class. It also means that if you call the function with a static string it doesn’t get pointlessly converted to a
...
This only leaves debugging. We use formatted strings for informative assert messages when something goes wrong. Our profiler and monitoring tools use

[interned strings](http://altdevblogaday.org/2011/05/26/monitoring-your-game/)to identify data. Our game programmers use debug-prints to root out problems. Of course, non of this affects the end user, since the debugging strings are only used in debug builds.

Hashes can be problematic when debugging. If there is an error in the resource 0x3e728af10245bc71 it is not immediately obvious that it is the object

*vegetation/trees/larch_3.mesh*that is at fault.
...
ReplyDeletegood post. Utf-8 is a good way to store string.

For 64 bits hash algorithm, which algorithm did you used ?

@James I use the 64-bit murmur hash.

ReplyDeleteThis might be a stupid noob question, but how do you enforce UTF-8 encoding? I know that C++11 have the u8 for literal strings, but that doesn't work in VS2010, so how do you do it?

ReplyDeleteOur C++ source files are written as 7 bit clean ASCII, without any special characters (such as å), so all the strings there are already UTF8-encoded.


ReplyDeleteStrings that are displayed on screen are edited with our localization tool (so that they can be translated to different languages) and that tool outputs UTF8-encoded JSON files with the string data.

FWIW, many of the higher-performing substring searching algorithms require effectively-random access to the glyphs within the string. See for example: https://www.cs.utexas.edu/users/moore/best-ideas/string-searching/fstrpos-example.html

ReplyDeleteThe fact that random access glyphs are rare makes sense, since ASCII-strings can be used without any changes in most of the cases. However, one exception to this is rendering, which is done sequentially and is only efficient for sequential processing.


ReplyDeleteMoreover, when it comes to optimization and efficiency; let’s look at ViQure Coupon Code to find out about its exclusive offers. As this conversation emphasizes the need for caution in utilizing memory ViQure has a variety of items to choose from as far as health-conscious lifestyle is concerned. This ensures great value for your very good state.