---
tags: [人物, 作者, unity]
date: 2026-04-19
sources: 5
---

# Ted Sie（阿祥的开发日常）

台湾 Unity 开发者，博客 tedsieblog.wordpress.com（"阿祥的开发日常"），以 Unity 实战案例为主题的中文技术博客作者。内容覆盖 shader、image effect、程序化生成、DOTS/ECS、游戏系统实作，风格偏 tutorial，每篇文章围绕一个具体功能从概念到实作完整走一遍，并附完整源码。

## 相关

- [[unity-complexity-patterns]]
- [[ecs]]
- [[unity-dots-tween-system]]
- [[dynamic-split-screen]]
- [[l-system-fractals]]
- [[l-system-lightning-bolts]]
- [[unity-crowd-waypoint-system]]

## Sources

- [[sources/tedsie-dynamic-split-screen]]
- [[sources/tedsie-l-system-fractals]]
- [[sources/tedsie-l-system-lightning-bolts]]
- [[sources/tedsie-crowd-simulation]]
- [[sources/tedsie-dots-tween-system]]
